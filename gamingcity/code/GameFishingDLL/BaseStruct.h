...
struct CS_stChangeCannon {
  CS_stChangeCannon() { Clear(); }
  ~CS_stChangeCannon() { Clear(); }
  void Clear() {
    chair_id = 0;  //椅子ID
    add = 0;       //玩家时间
...
struct CS_stChangeCannonSet {
  CS_stChangeCannonSet() { Clear(); }
...
struct CS_stTreasureEnd {
  CS_stTreasureEnd() { Clear(); }
