...
void EffectMgr::Clear()
{
	...
	{
		EffectFactory::instance()->Recovery((*it)->GetEffectType(), *it);
		++it;
	}
	m_effects.clear();
}
...
int64_t  EffectMgr::Execute(std::shared_ptr<T1> pTarget, std::list<std::shared_ptr<T2>>& list, bool bPretreating){
	int64_t  Score = 0;
