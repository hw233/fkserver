...
class CGameConfig : public Singleton<CGameConfig> {
 public:
  bool LoadSystemConfig(lua_State* L);

  bool LoadFish(lua_State* L);

  bool LoadVisual(lua_State* L);

  bool LoadCannonSet(lua_State* L);

  bool LoadBoundBox(lua_State* L);

  bool LoadScenes(lua_State* L);

  bool LoadFishSound(lua_State* L);

  bool LoadSpecialFish(lua_State* L);

  bool LoadBulletSet(lua_State* L);
 ...
 public:
  ...
  int nWidth;          //宽度
  int nHeight;         //高度
  //当前房间的游戏币与渔币的兑换比例为%d游戏币兑换%d渔币 nChangeRatioUserScore
  //nChangeRatioFishScore
