...
class MoveCompent : public MyComponent {
 public:
  ...
  void SetPosition(int x, int y) {
    m_data.m_Position.x_ = x;
    m_data.m_Position.y_ = y;
  }
  ...
  void SetPathID(int pid, bool bt = false) {
    m_nPathID = pid;
    m_bTroop = bt;
  }
  ...
  void SetOffest(const MyPoint& pt) { m_Offest = pt; }
  ...
  void SetTarget(std::shared_ptr<MyObjMgr> pm, uint32_t id) {
    m_pObjMgr = pm;
    m_dwTargetID = id;
  }
 ...
 protected:
  ...
  std::shared_ptr<MyObjMgr> m_pObjMgr;    //对象管理
  bool m_bTroop;          //是否队伍
};
