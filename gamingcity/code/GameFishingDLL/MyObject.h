...
public:
	...
	void SetMgr(std::shared_ptr<MyObjMgr> mgr){m_Mgr = mgr;}
	std::shared_ptr<MyObjMgr> GetMgr(){return m_Mgr;}
...
protected:
	std::shared_ptr<MyObjMgr> m_Mgr;            //管理器指针
	uint32_t id_;
	int objType_;               //对象类型
...
protected:
	typedef std::map< const uint32_t, std::shared_ptr<MyComponent> >	Component_Table_t;     //附加控件列表
	typedef std::list< std::shared_ptr<CComEvent> > CCEvent_Queue_t;          //事件列表
...
public:
	void ProcessCCEvent(std::shared_ptr<CComEvent>);//即时处理的事件
	void ProcessCCEvent(uint32_t idEvent, int64_t nParam1 = 0, void* pParam2 = 0);

	void PushCCEvent(std::shared_ptr<CComEvent>& evnt);//延迟处理的事件
	void PushCCEvent(uint32_t idEvent, int64_t nParam1 = 0, void* pParam2 = 0);

	template<class T>
	std::shared_ptr<T> GetComponent(const int family_id);

	template<class T>
	void SetComponent( std::shared_ptr<T> newComponent);
