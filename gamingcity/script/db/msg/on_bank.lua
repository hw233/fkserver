...
function on_sd_bank_set_password(game_id, msg)
	log.info(string.format("player [%d] set password",msg.guid))
...
--重置 银行密码
function on_sd_resetpw( game_id , msg)	
	log.info(string.format("player [%d] reset password",msg.guid))
	...
	log.info(string.format("guid [%d] DS_ResetPW : %d",msg.guid,ret))
	return {
		guid = guid_,
...
function on_sd_bank_change_password(game_id, msg)
	log.info(string.format("player [%d] change password",msg.guid))
	local guid_ = msg.guid
...
function on_change_bank(game_id, msg)
	-- body
	local gameid = game_id
	local guid = msg.guid
	...
	log.info(string.format("on_change_bank gameid[%d] guid[%d] optType[%d] changemoeny[%d] oldmoney[%s] newmoney[%s]", gameid ,guid , optType , money_ , tostring(oldmoney) , tostring(newmoney)))
	if tonumber(optType) == 3 then
		...
		if not data then
			...
			log.info(string.format("on_change_bank  mysql faild :[%d]  guid[%s]"  , notify.retcode, notify.guid))
		else
			data = data[1]
			log.info(string.format("on_change_bank guid [%s]  data.ret [%d]",notify.guid , data.ret))
			if tonumber(data.ret) ~= 1 then  -- 2 金币不足 4 金币为空 5 update 金币失败
				log.info(string.format("on_change_bank faild ,guid[%d] data.ret[%d] changemoeny[%d]",notify.guid,data.ret,money_))
				...
				if tonumber(optType) == 2 then
					...
					db_log:query(sql)
					log.info(string.format("on_change_bank guid [%s] log_money_type is [%d]",notify.guid,log_money_type))
					local log_money_= {
						guid = notify.guid,
						old_money = oldmoney,
						new_money = newmoney,
...
function  on_banklog_new(game_id, msg)
...
function on_SD_CheckBankTransferEnable(gameid, msg)
	...
	if not data then
		log.error(string.format("on_SD_CheckBankTransferEnable faild recv_guid[%d] pay_guid[%d]" ,msg.recv_guid, msg.pay_guid))
		return notify
	end
...
--扣除银行金币成功，插入提现记录
function do_PlayerBankTransfer_Success(game_id,msg,oldbank,newbank)
	local guid_ = msg.guid
	...
	log.error(string.format("do_PlayerBankTransfer_Success error guid [%d]",guid_))
...
function on_SD_PlayerBankTransfer(game_id, msg)
	-- body
	log.info(string.format("on_SD_PlayerBankTransfer begin ------- guid[%d] money[%d]",msg.guid,msg.money))
	...
	if tonumber(data.ret) == 1 then
		--成功
		do_PlayerBankTransfer_Success(game_id,msg,data.oldbank,data.newbank)
	else
		return {
			guid = guid_,
			result = data.ret,
...
function on_SD_ValidateboxFengIp(game_id, msg)
	dbopt.account:query([[REPLACE INTO t_validatebox_feng_ip set ip='%s']], msg.ip)
end

function on_SD_GetBonusPoolMoney(game_id, msg)
	local sql = string.format([[
		SELECT money FROM t_bonus_pool WHERE bonus_pool_name = '%s'
		]], msg.bonus_pool_name)

	local data = dbopt.game:query(sql)[1]
	local bonus_money = 0
	if data and data.money then
		bonus_money = tonumber(data.money)
	end

	redisopt.default:set(msg.bonus_pool_name, bonus_money)
end

function on_SD_UpdateBonusPool(game_id, msg)
