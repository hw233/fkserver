...
function on_sd_send_mail(game_id, msg)
	...
	if not player then
		log.warning(string.format("on_sd_send_mail guid[%d] not find in db", mail_.send_guid))
		return
	end
...
function on_sd_del_mail(game_id, msg)
	...
	if not player then
		log.warning(string.format("on_sd_del_mail guid[%d] not find in db", msg.guid))
		return
	end
...
function on_sd_receive_mail_attachment(game_id, msg)
	local player = find_player(msg.guid)
	if not player then
		log.warning(string.format("on_sd_receive_mail_attachment guid[%d] not find in db", msg.guid))
		return
	end
