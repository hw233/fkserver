...
require "game.banker_ox.banker_table"
...
banker_room = base_room

-- 初始化房间
function banker_room:init(tb, chair_count, ready_mode, room_lua_cfg)
	base_room.init(self, tb, chair_count,ready_mode,room_lua_cfg)
end

-- 创建桌子
function banker_room:create_table()
	return banker_table:new()
end

-- 坐下处理
function banker_room:on_sit_down(player)
...
-- 快速坐下
function banker_room:auto_sit_down(player)
...
-- 坐下
function banker_room:sit_down(player, table_id_, chair_id_)
...
-- 站起
--function banker_room:stand_up(player)
...
-- 玩家掉线
--[[function banker_room:player_offline(player)
...
-- 站起并离开房间
function banker_room:stand_up_and_exit_room(player)
	print("banker_room:stand_up_and_exit_room=============================")
