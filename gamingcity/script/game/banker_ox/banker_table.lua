...
function banker_table:init(room, table_id, chair_count)
	...
	if def_first_game_type == 14 then
		...
		log.info(string.format("banker_ox:-------->type[%d]:bet_times_option:[%d,%d,%d,%d]",def_second_game_type,bet_times_option[1],bet_times_option[2],bet_times_option[3],bet_times_option[4]))
	end
	self:robot_init()
end
...
function banker_table:load_lua_cfg()
	...
	log.info(string.format("banker_table: game_maintain_is_open = [%d]",self.room_.game_switch_is_open))
	...
	if banker_config then
		...
		if banker_config.BANKER_OX_GRAND_PRICE_BASE then
			...
			log.info(string.format("#########BANKER_OX_GRAND_PRICE_BASE:[%d]",BANKER_OX_GRAND_PRICE_BASE))
		end

		if banker_config.BASIC_BET_TIMES then
			bet_times_option = banker_config.BASIC_BET_TIMES
			log.info(string.format("######### banker_ox:bet_times_option:[%d,%d,%d,%d]",bet_times_option[1],bet_times_option[2],bet_times_option[3],bet_times_option[4]))
		end
...
function banker_table:send_player_cards()
	...
	--	if player.pb_base_info.money < self.room_:get_room_limit() then
	--		player:forced_exit()
			...
			if cards_num_total == 5 then
				...
				if ox_type_ >= BANKER_CARD_TYPE_ONE then --有牛就将能凑牛的牌放在前面可观看的4张中
					if sort_cards_ ~= nil and getNum(sort_cards_) == 5 then
						log.info(string.format("222222222222 before exchange cards:player guid[%d],ox_type_[%d] sort_cards_[%s]" , _player.guid ,ox_type_, table.concat(sort_cards_, ", ")))
						--交换牌顺序
						self.b_player[_player.guid].cards = {}
						for i_key = 1,5 do
							table.insert(self.b_player[_player.guid].cards,sort_cards_[i_key])
						end
						log.info(string.format("333333333333333 after exchange cards:player is [%d]  cards [%s]" , _player.guid , table.concat( self.b_player[_player.guid].cards, ", ")))
					end
				end
			end
		end
...
function banker_table:decide_banker()
	...
	log.info(string.format("banker_candidate is [%d]", getNum(banker_candidate)))
	local msg = {}
	...
	log.info(string.format("self.b_banker.ratio = [%d],guid = [%d]", self.b_banker.ratio,self.b_banker.guid))
	...
	log.info(string.format("Cur Banker guid = [%d], ratio = [%d]",self.b_banker.guid,self.b_banker.ratio))
end
...
function banker_table:send_result()
	...
	--	end
	--end
    log.info(string.format("banker_lose_money [%d] banker_win_money [%d] banker_old_money [%d] banker_win_lose_money[%d]", banker_lose_money, banker_win_money, banker_old_money, banker_win_lose_money))
	...
	for _guid, b_player in pairs(self.b_player) do
		if _guid ~= self.b_banker.guid and b_player.cards then
			...
			if win == true then
				...
				log.info(string.format("3333player guid[%d] win_money = [%d] banker_win_lose_money = [%d] banker_old_money = [%d]",l_player.guid,win_money,banker_win_lose_money,banker_old_money))
				if banker_old_money < 2 then
					...
					--  按比例获得 金币
						...
						log.info(string.format("4444banker_ox:guid[%d] win_money [%d] banker_old_money [%d] banker_win_lose_money [%d]",_guid,win_money ,banker_old_money ,banker_win_lose_money))
					end
				end
				...
				if banker_ox_player_stdard_award >= BANKER_OX_GRAND_PRICE_BASE and l_player.is_player ~= false then
					log.info(string.format("player guid[%d] nickname[%s]in banker ox game earn money[%d] upto [%d],broadcast to all players.",l_player.guid,l_player.nickname,banker_ox_player_stdard_award,BANKER_OX_GRAND_PRICE_BASE))
				...
				--  按比例获得 金币
					...
					log.info(string.format("66666banker_ox:guid[%d] lose_money [%d] banker_old_money [%d] banker_win_lose_money [%d]",_guid,lose_money ,banker_old_money ,banker_win_lose_money))
				end
	...
	if self.b_pool > 0 and banker_win_lose_money < 0 then
		...
		log.info(string.format("99999player[%d] increment_money = [%d] tax = [%d]",l_player.guid,pb_banker.increment_money,pb_banker.tax))
		if pb_banker.increment_money > s_old_money then
			log.info(string.format("00000guid[%d] banker win_money [%d] s_old_money[%d]",l_player.guid,pb_banker.increment_money ,s_old_money))
			pb_banker.increment_money = s_old_money
		end
		log.info(string.format("aaaplayer[%d] increment_money = [%d] s_old_money = [%d]",l_player.guid,pb_banker.increment_money,s_old_money))
		if l_player then
			l_player:add_money(
				{{ money_type = ITEM_PRICE_TYPE_GOLD,
		...
		if banker_ox_banker_stdard_award >= BANKER_OX_GRAND_PRICE_BASE and l_player.is_player ~= false then
			log.info(string.format("player guid[%d] nickname[%s]in banker ox game earn money[%d] upto [%d],broadcast to all players.",l_player.guid,l_player.nickname,banker_ox_banker_stdard_award,BANKER_OX_GRAND_PRICE_BASE))
		...
		log.info(string.format("ccccguid[%d] b_pool = [%d] banker_ox.increment_money = [%d] lose_money = [%d]",l_player.guid,self.b_pool,pb_banker.increment_money,lose_money))
			if DEBUG_MODE then
...
--[[
	...
	log.info(string.format("banker_bet player[%d] bankerMoney [%s] playerPayMoney[%s] #self.b_player[%d] msg.bet_times [%s]",player.guid,tostring(bankerMoney) ,tostring(playerPayMoney) , self.b_player_count, tostring(msg.bet_times)))
	...
	log.info(string.format("banker_bet player[%d] playerPayMoney [%s] l_money[%s] self_bet_max[%s] msg.bet_money[%s]",player.guid,tostring(playerPayMoney) ,tostring(l_money) ,tostring(self_bet_max) ,tostring(msg.bet_money)))
	...
	if msg.bet_money > self_bet_max then
		...
		log.info(string.format("new bet_money is  [%s] bet_times [%d] " , tostring(msg.bet_money) , tostring(msg.bet_times)))
	end
--]]

	local cur_player_money = player:get_money()
	log.info(string.format("player guid[%d] bet_money = [%d] bet_times = [%d] cur_player_money = [%d] ,banker ratio = [%d]",player.guid,msg.bet_money,msg.bet_times,cur_player_money,self.b_banker.ratio))
	...
	if msg.bet_times == player_cur_max_times then --玩家选择最低倍数始终可选不用判断
		...
		msg.bet_money = msg.bet_times * self.b_bottom_bet * self.b_banker.ratio
		log.info(string.format("player guid[%d] bet_money = [%d] acturl_bet_times = [%d] cur_player_money = [%d] ,banker ratio = [%d], acturl_player_bet_max_times = [%d]",player.guid,msg.bet_money,msg.bet_times,cur_player_money,self.b_banker.ratio,acturl_player_bet_max_times))
	end
...
function banker_table:banker_contend(player, ratio)
	if player and player.guid then
		log.info(string.format("banker_table:banker_contend ========== player [%d] ratio[%d]" , player.guid, ratio))
	end
	...
	if not self:isPlayer(player) then
		if player then
			log.info(string.format("banker_contend player is not in game guid[%d]" , player.guid))
		else
			log.info(string.format("banker_contend player is nil"))
		end
		return
	end
	if ratio < 0 then
	...
	if self.b_player[player.guid].ratio ~= -1 then
		log.info(string.format("banker_contend player ratio is not nil guid[%d] ratio[%d]" , player.guid , self.b_player[player.guid].ratio))
		return
	end
	...
	log.info(string.format("banker_ox:banker_contend ========== player [%d] ratio[%d] curMoney = [%d] cur_max_ratio = [%d]" , player.guid, ratio,player_curMoney,cur_max_ratio))
	if ratio > cur_max_ratio then
		ratio = cur_max_ratio
	end
	--抢庄倍数异常时，默认为1
	if ratio > 4 then
		log.warning(string.format("banker_ox:player guid[%d] banker contend expection ratio[%d] will be set ratio = 1" , player.guid, ratio))
		ratio = 1
	end
...
function banker_table:reconnection_play_msg(player)
...
function banker_table:player_sit_down(player, chair_id_)
	...
	for i,v in pairs(self.player_list_) do
		if v == player then
			log.info(string.format("GameInOutLog,banker_table:player_sit_down return, guid %s,chair_id %s",tostring(player.guid),tostring(chair_id_)))
...
function banker_table:playerReady( player )
	...
	if player then
		...
		elseif self.b_table_busy == 1 and player.banker_enterflag == true and self.b_status ~= STATUS_WAITING then
			if player.is_player ~= false then
				log.error(string.format("playerReady error player banker_enterflag is true guid[%d]",player.guid))
			end
			--player:forced_exit()
			return
		end
	...
	log.info(string.format("banker_table:playerReady room[%d] table[%d] chair[%d] player[%d]",self.room_.id, self.table_id_ , player.chair_id, player.guid))
...
function banker_table:check_black_user(cards)
	...
	log.info(string.format("max_cards_in_black_player [%s] min_cards_in_white_player[%s] max_chair_id[%d] min_chair_id[%d]",tostring(max_cards_in_black_player),tostring(min_cards_in_white_player),max_chair_id,min_chair_id))
