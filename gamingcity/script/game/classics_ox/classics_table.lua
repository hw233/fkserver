...
function classics_table:init(room, table_id, chair_count)
	...
	if def_first_game_type == 24 then
		...
		log.info(string.format("-------->type[%d]:bet_times_option:[%d,%d,%d,%d]",def_second_game_type,bet_times_option[1],bet_times_option[2],bet_times_option[3],bet_times_option[4]))
	end
	self:robot_init()
end
...
function classics_table:load_lua_cfg()
	...
	log.info(string.format("classics_table: game_maintain_is_open = [%d]",self.room_.game_switch_is_open))
	...
	if classics_config then
		if classics_config.SYSTEM_BEAST_PROB then
			...
			log.info(string.format("######### classics_configSYSTEM_BEAST_PROB:[%f]",SYSTEM_BEAST_PROB))
		end

		if classics_config.SYSTEM_FLOAT_PROB then
			SYSTEM_FLOAT_PROB = classics_config.SYSTEM_FLOAT_PROB
			log.info(string.format("#########classics_config SYSTEM_FLOAT_PROB:[%f]",SYSTEM_FLOAT_PROB))
		end
		...
		if classics_config.CLASSIC_OX_GRAND_PRICE_BASE then
			...
			log.info(string.format("######### CLASSIC_OX_GRAND_PRICE_BASE:[%d]",CLASSIC_OX_GRAND_PRICE_BASE))
		end

		if classics_config.BASIC_BET_TIMES then
			bet_times_option = classics_config.BASIC_BET_TIMES
			log.info(string.format("######### bet_times_option:[%d,%d,%d,%d]",bet_times_option[1],bet_times_option[2],bet_times_option[3],bet_times_option[4]))
		end
...
function classics_table:can_enter(player)
	...
	if player then
		log.info (string.format("classics_table player have date guid[%d] room[%d] table[%d] ",player.guid, self.room_.id,self.table_id_))
	else
...
-- local _idx = 1
	...
	for _key, _player in pairs(notify.pb_player) do
		...
		for i = 1,5 do
			...
			if cards_[i] then
				if setCount - user_cards_idx >= SYS_CARDS_VALUE then
					...
					if haveTenCards then
						...
						--log.info(string.format("=================================================end[%d]",idx))
						if idx == 0 then
							haveTenCards = false
						end
					end
					...
					--	log.info(string.format("1. upCards[%d][%d] ",self.b_player[_player.guid].cards[i - 1] , upCards ))
					--	local x = 1
					--	while( x < setCount - user_cards_idx and idx == 0 ) do
					--		log.info(string.format("x is [%d]",x))
					--		local thisCards = get_value(self.t_card_set[x]) + 1
					--		log.info(string.format("2. thisCards[%d][%d] ",self.t_card_set[x] , thisCards ))
					--		if thisCards + upCards == 10 then
					--			log.info(string.format("3. upCards[%d][%d] t_card_set[%d][%d]",self.b_player[_player.guid].cards[i - 1] , upCards , self.t_card_set[x] , thisCards ))
					--			idx = x
					--		end
					--		x = x + 1
					--	end
			...
			self.t_card_set[getNum(self.t_card_set) - user_cards_idx] = card
			user_cards_idx = user_cards_idx + 1
			--log.info(string.format("self.t_card_set is [%s]",table.concat(self.t_card_set,",")))
		end
		-----test-----
		...
		log.info(string.format("player is [%d] chair_id [%s]" , _player.guid , _player.chair))
...
-- end
		...
		if player.is_player then
			table.insert(player_list_temp, player)
	...
	if b_change_cards then
		...
		log.info(string.format("------------------------T :[%d] [%d]", random_chair, mix_chair_id))
		--最小牌
		...
		if v and v.is_player == false and random_chair ~= mix_chair_id and player_num ~= 0 then
			...
			local player_temp = player_list_temp[random_chair]
			if player_temp then
				local _ =  self.b_player[player_temp.guid]
...
function classics_table:decide_banker()
	...
	if getNum(self.b_contend_count) == 0 then
		...
		for _guid, b_player in pairs(self.b_player) do
			...
			self.game_log.classics_contend[b_player.chair] = 1
			log.info(string.format("not find  player contend banker."))
	...
	log.info(string.format("banker_candidate is [%d]", getNum(classics_candidate)))
	local msg = {}
	...
	log.info(string.format("self.b_classics.ratio = [%d],guid = [%d]", self.b_classics.ratio,self.b_classics.guid))
	...
	log.info(string.format("Cur Banker guid = [%d]",self.b_classics.guid))
end
...
function classics_table:send_result()
	...
	local banker_old_money = banker_player.pb_base_info.money
	...
	for _guid, b_player in pairs(self.b_player) do
		...
		if _guid ~= self.b_classics.guid and b_player.cards then
			...
			if win == true then
				...
				log.info(string.format("1111player[%d] betmoney = [%d],win_money_ = [%d] banker_win_lose_money = [%d]",l_player.guid,b_player.bet,win_money_,banker_win_lose_money))
			else
				...
				log.info(string.format("2222player[%d] betmoney = [%d],lose_money_ = [%d] banker_win_lose_money = [%d]",l_player.guid,b_player.bet,lose_money_,banker_win_lose_money))
			end
		end
	end
	...
	for _guid, b_player in pairs(self.b_player) do
		if _guid ~= self.b_classics.guid and b_player.cards then
			...
			if win == true then  --闲家赢了
				...
				log.info(string.format("3333win_money = [%d] banker_win_lose_money = [%d] banker_old_money = [%d]",win_money,banker_win_lose_money,banker_old_money))
				if banker_old_money < 2 then
					win_money = 0 			-- 异常情况 玩家作弊
					log.error(string.format("classics_ox:player [%d] Cheat" , _guid))
				else
					if banker_win_lose_money > 0 and banker_win_lose_money > banker_old_money then
					--  按比例获得 金币
						win_money =  math.floor(win_money * lose_coeff)
						log.info(string.format("4444classics_ox:guid[%d] win_money [%d] banker_old_money [%d] banker_win_lose_money [%d]",_guid,win_money ,banker_old_money ,banker_win_lose_money))
					end
				end
				...
				log.info(string.format("55555player[%d] betmoney = [%d],win_money = [%d] banker_win_lose_money = [%d]",l_player.guid,b_player.bet,win_money,banker_win_lose_money))
				...
				if classic_ox_player_stdard_award >= CLASSIC_OX_GRAND_PRICE_BASE and l_player.is_player ~= false then
					log.info(string.format("player guid[%d] nickname[%s]in classics ox game earn money[%d] upto [%d],broadcast to all players.",l_player.guid,l_player.nickname,classic_ox_player_stdard_award,CLASSIC_OX_GRAND_PRICE_BASE))
				...
				log.info(string.format("~~~~~~~~~will lose lose_money = [%d] player money = [%d]",lose_money,s_old_money))
				if banker_win_lose_money < 0 and (-banker_win_lose_money) > banker_old_money then
				--  按比例获得 金币
					lose_money =  math.floor(lose_money * win_coeff)
					log.info(string.format("66666classics_ox:guid[%d] lose_money [%d] banker_old_money [%d] banker_win_lose_money [%d]",_guid,lose_money ,banker_old_money ,banker_win_lose_money))
				end
				...
				log.info(string.format("77777player[%d] betmoney = [%d],acturl lose_money = [%d] banker_win_lose_money = [%d]",l_player.guid,b_player.bet,lose_money,banker_win_lose_money))
				pb_player = {
					chair = b_player.chair,
					money = 0,
					tax = 0,
	...
	log.info(string.format("==================>b_pool = [%d] banker_win_lose_money = [%d]",self.b_pool,banker_win_lose_money))
	if self.b_pool > 0  and banker_win_lose_money < 0 then --庄家赢了
		log.info(string.format("88888b_pool = [%d] banker_win_lose_money = [%d]",self.b_pool,banker_win_lose_money))
		...
		log.info(string.format("99999player[%d] increment_money = [%d] tax = [%d]",l_player.guid,pb_classics.increment_money,pb_classics.tax))
		if pb_classics.increment_money > s_old_money then
			log.info(string.format("00000guid[%d] banker win_money [%d] s_old_money[%d]",l_player.guid,pb_classics.increment_money ,s_old_money))
			pb_classics.increment_money = s_old_money
		end
		log.info(string.format("aaaplayer[%d] increment_money = [%d] s_old_money = [%d]",l_player.guid,pb_classics.increment_money,s_old_money))
		if l_player then
			l_player:add_money(
				{{ money_type = ITEM_PRICE_TYPE_GOLD,
		...
		if classic_ox_banker_stdard_award >= CLASSIC_OX_GRAND_PRICE_BASE and l_player.is_player ~= false then
			log.info(string.format("bbbplayer guid[%d] nickname[%s]in classics ox game earn money[%d] upto [%d],broadcast to all players.",l_player.guid,l_player.nickname,classic_ox_banker_stdard_award,CLASSIC_OX_GRAND_PRICE_BASE))
		...
		log.info(string.format("ccccguid[%d] b_pool = [%d] pb_classics.increment_money = [%d] lose_money = [%d]",l_player.guid,self.b_pool,pb_classics.increment_money,lose_money))
			if DEBUG_MODE then
	...
	log.info(string.format("record: banker[%d] header_icon[%d] nickname[%s] cards_type[%d] money_change[%d]",banker_record.guid,banker_record.header_icon,banker_record.nick_name,banker_record.cards_type,banker_record.money_change))
...
function classics_table:classics_bet(player, msg)
	...
	if self.b_classics.guid == player.guid then
		log.warning(string.format("banker guid[%d] can't bet money.",player.guid))
		return
	end
--[[
	-- 算出实际下注金额
	...
	log.info(string.format("bankerMoney [%s] playerPayMoney[%s] #self.b_player[%d] msg.bet_times [%s]",tostring(bankerMoney) ,tostring(playerPayMoney) , self.b_player_count, tostring(msg.bet_times)))
	...
	log.info(string.format("classics_table playerPayMoney [%s] l_money[%s] self_bet_max[%s] msg.bet_money[%s]",tostring(playerPayMoney) ,tostring(l_money) ,tostring(self_bet_max) ,tostring(msg.bet_money)))

	if msg.bet_money > self_bet_max then
		log.info(string.format("old bet_money is  [%s] bet_times [%s] " , tostring(msg.bet_money) , tostring(msg.bet_times)))
		local bet_times = 1
		msg.bet_money = self.b_bottom_bet * bet_times
		msg.bet_times  = bet_times
		log.info(string.format("new bet_money is  [%s] bet_times [%s] " , tostring(msg.bet_money) , tostring(msg.bet_times)))
	end
--]]
	local cur_player_money = player:get_money()
	log.info(string.format("player guid[%d] bet_money = [%d] bet_times = [%d] cur_player_money = [%d] ,banker_times = [%d]",player.guid,msg.bet_money,msg.bet_times,cur_player_money,self.b_classics.ratio))
	...
	if msg.bet_times == player_cur_max_times then --玩家选择最低倍数始终可选不用判断
		...
		msg.bet_money = msg.bet_times * self.b_bottom_bet * self.b_classics.ratio
		log.info(string.format("player guid[%d] bet_money = [%d] acturl_bet_times = [%d] cur_player_money = [%d] ,banker_times = [%d], acturl_player_bet_max_times = [%d]",player.guid,msg.bet_money,msg.bet_times,cur_player_money,self.b_classics.ratio,acturl_player_bet_max_times))
	end
...
function classics_table:classics_guess_cards(player)
	if player then --and next(self.b_player[player.guid]) ~= nil
		log.info(string.format("player guid = [%d] show cards cur_status = [%d]",player.guid,self.b_status))
...
function classics_table:classics_contend(player, ratio)
    if player and player.guid then
		log.info(string.format("classics_table:banker_contend ========== player [%d] ratio[%d]" , player.guid, ratio))
	end
	...
	if not self:isPlayer(player) then
		if player then
			log.info(string.format("classics_contend player is not in game guid[%d]" , player.guid))
		else
			log.info(string.format("classics_contend player is nil"))
		end
		return
	end
	if ratio < 0 then
	...
	if self.b_player[player.guid].ratio ~= -1 then
		log.info(string.format("classics_contend player ratio is not nil guid[%d] ratio[%d]" , player.guid , self.b_player[player.guid].ratio))
		return
	end
	local player_curMoney = player:get_money()
	--最大可抢倍数校验
	local cur_max_ratio = math.floor(player_curMoney/self.b_bottom_bet/25)
	log.info(string.format("classics_table:banker_contend ========== player [%d] ratio[%d] curMoney = [%d] cur_max_ratio = [%d]" , player.guid, ratio,player_curMoney,cur_max_ratio))
	if ratio > cur_max_ratio then
		ratio = cur_max_ratio
	end
	--抢庄倍数异常时，默认为1
	if ratio > 4 then
		log.warning(string.format("classics_table:player guid[%d] classics contend expection ratio[%d] will be set ratio = 1" , player.guid, ratio))
		ratio = 1
	end
...
function classics_table:reconnection_play_msg(player)
...
function classics_table:playerReady( player )
	...
	if player then
		...
		elseif self.b_table_busy == 1 and player.classics_enterflag == true and self.b_status ~= STATUS_WAITING then
			if player.is_player ~= false then
				log.error(string.format("playerReady error player classics_enterflag is true guid[%d]",player.guid))
			end
			player:forced_exit()
			return
		end
	...
	log.info(string.format("classics_table:playerReady room[%d] table[%d] chair[%d] player[%d]",self.room_.id, self.table_id_ , player.chair_id, player.guid))
	...
	if  self.b_player_count > 1 and self.b_player_count < 6 then
		...
		--if self.b_player_count >= 2 then
			...
			log.info(string.format("---------------------------------------------self.b_player_count:[%d]",self.b_player_count))
		--end
	else
		self.b_timer = get_second_time()
	end
...
function classics_table:check_black_user()
	...
	--	local player = self:get_player(b_player.chair)
	--	if player then
	--		log.info(string.format("player guid[%d] chair_id[%d]",player.guid,player.chair_id))
...
--
	...
	-- end
--
	-- log.info(string.format("max_player_cards_guid--------->[%d] chair_id = [%d]", max_player_cards_guid,max_chair_id))
...
--
	...
	-- 			--最大牌已经在非黑名单玩家手里
	-- 			log.info(string.format("max cards belong to while player guid[%d] chair_id[%d]",player.guid,player.chair_id))
	-- 			return
	-- 		end
	-- 	end
	-- end
...
--
	...
	-- log.info(string.format("black list players guid[%d] chair_id[%d]", max_player_cards_guid, max_chair_id))
	-- log.info(table.concat(self.b_player[max_player_cards_guid].cards, ','))
...
--	end
	...
	if #player_white_list_temp == 0 or #player_black_list_temp == 0then
	...
	if max_cards_in_black_player and min_cards_in_white_player then
		...
		log.info(string.format("black list players guid[%d] chair_id[%d]", max_cards_player_info.guid, max_cards_player_info.chair_id))
	...
	elseif min_cards_in_white_player then
		...
		log.info(string.format("black list players guid[%d] chair_id[%d]", min_cards_player_info.guid, min_cards_player_info.chair_id))
	...
	if self.robot_switch == 1 then
		for _,v in pairs(self.robot_info) do
			if v.is_use then
				...
				log.info(string.format("self.room_:get_room_limit() %d v.android:get_money() %d  guid %d",self.room_:get_room_limit() , v.android:get_money() ,v.android.guid))
...
function classics_table:reset_robot(android)
	...
	android.cur = 0
	android.pb_base_info.money = self.room_:get_room_limit() * random.boost_integer(2,20)
	...
	--else
	--	android.pb_base_info.money = self.room_:get_room_limit() * random.boost_integer(100,200)
	--end
	log.info(string.format("def_second_game_type %d , android.pb_base_info.money [%d] " ,def_second_game_type , android.pb_base_info.money))
	android:reset_show()
end
