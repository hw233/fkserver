...
function land_cards.convert_aicards_cards(aicards,player_cards)
	...
	--log.error(string.format("ai_play_cards------>%s",table.concat(ai_play_cards, ',')))
	--log.error(string.format("player_cards------>%s",table.concat(player_cards, ',')))
	--log.error(string.format("player_out_cards------>%s",table.concat(player_out_cards, ',')))
...
function land_cards.convert_cards_aicards(out_cards,card_list)
	...
	--log.error(string.format("convert_cards_aicards cardstype[%d] cardsval[%d] card_list[%s]",cardstype,cardsval,table.concat(card_list, ',')))
    ...
    if val ~= nil then
        if cards.val == val and val < 13 then
    		...
    		if cards.index > 0 and cards.val ~= nil then
    			if cards.index >= cards.max then
    				-- 不可能 不处理
    				log.error(string.format("analyseb_cards_new_sorting error cards.index[%d] cards.max[%d]" , cards.index , cards.max))
    			else
