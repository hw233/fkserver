...
local good_cards_array = good_cards_array
...
function land_table:init(room, table_id, chair_count)
	...
	for k=1,10 do
		...
		if numCount == 100000 then
			log.info(string.format("shuffle_cheat_cards--------------->all is ok"))
		end
	end
	--]]
end
...
function land_table:player_stand_up(player, is_offline)
	...
	log.info(string.format(" player_stand_uptable id[%d] guid[%s]",self.table_id_,tostring(player.guid)))
	local ret = base_table.player_stand_up(self,player, is_offline)
	if ret then
		if self.status == LAND_STATUS_START_COUNT_DOWN then
			log.info(string.format(" ================ play start count down clean ==================table id[%d]",self.table_id_))
			self.status = LAND_STATUS_FREE
		end
	end
	return ret
...
function land_table:check_cancel_ready(player, is_offline)
	log.info(string.format("check_cancel_ready guid [%d] status [%d] is_offline[%s]",player.guid and player.guid or 0,self.status,tostring(is_offline)))
...
function land_table:load_lua_cfg()
    log.warning(string.format("land_table:load_lua_cfg : [%s] ",self.room_.room_cfg))
    local land_config = json.decode(self.room_.room_cfg)
    if land_config then
        if land_config.GameLimitCdTime then
            log.info(string.format("GameLimitCdTime : [%s]" , land_config.GameLimitCdTime))
            self.GameLimitCdTime = land_config.GameLimitCdTime
        end
        if land_config.basic_coeff then
            log.info(string.format("basic_coeff : [%s]" , land_config.basic_coeff))
            self.BASIC_COEFF = land_config.basic_coeff
        end

        if land_config.float_coeff then
            log.info(string.format("float_coeff : [%s]" , land_config.float_coeff))
            self.FLOAT_COEFF = land_config.float_coeff
        end

        if land_config.broad_money then
            log.info(string.format("broad_money : [%s]" , land_config.broad_money))
        ...
        if land_config.playWithAndroid then -- self.playWithAndroid.status 0 关 1 全开 2 按渠道开 3 按渠道关
            log.info(string.format("playWithAndroid : [%s]" , land_config.playWithAndroid))
            self.playWithAndroid = land_config.playWithAndroid
            if self.playWithAndroid then
                if self.playWithAndroid.status then
                    log.info(string.format("self.playWithAndroid.status [%d] ", self.playWithAndroid.status))
            ...
            log.info(string.format("self.playWithAndroid [%s] self.playWithAndroid.status[%s] self.playWithAndroid.status [%d]", tostring(self.playWithAndroid) , tostring(self.playWithAndroid.status) , self.playWithAndroid.status))
        end

        if land_config.system_bad_card_coeff then
            log.info(string.format("system_bad_card_coeff : [%s]" , land_config.system_bad_card_coeff))
	...
	for i=1,self.good_cards_total do
		...
		for j=1,54 do
			...
			if value_key ~= cards[j] then
				log.error(string.format("~~~~~~~~~~~~index = [%d]",i))
				log.info(table.concat(cards, ','))
				return
			end
		end
...
function land_table:start()
	log.info(string.format(" ================ play start count down ==================table id[%d]",self.table_id_))
...
function land_table:do_ai_grab_landlord(chair_id)
	log.info(string.format("ai_grab_landlord table_id[%d] chairid[%d]",self.table_id_,chair_id))
	local player = self.player_list_[chair_id]
	...
	ai_GrabLandlord(stringbuffer, function(reply)
		...
		log.info(string.format("ai_grab_landlord table_id[%d] score[%d]",self.table_id_,score))
...
function land_table:ai_grab_landlord2()
	...
	ai_GrabLandlord2(stringbuffer, function(reply)
		local score = tonumber(reply)
		log.info(string.format("ai_grab_landlord2 score[%d]",score))
	end)
end
...
function land_table:do_ai_play_card(chair_id)
	...
	ai_AiPlayCard(stringbuffer, function(reply)
		...
		--log.error(string.format("ai_play_card type[%d] alone_1[%d] alone_2[%d] alone_3[%d] alone_4[%d] pb_combo_list[%s]",landAiPaiMove.type,landAiPaiMove.alone_1,landAiPaiMove.alone_2,landAiPaiMove.alone_3,landAiPaiMove.alone_4,table.concat(landAiPaiMove.pb_combo_list.cards, ",")))
		...
		if ai_time > 14 then
			log.error(string.format("do_ai_play_card table [%d] time [%d]",self.table_id_,ai_time))
		end
...
function land_table:do_ai_play_card_passive(chair_id)
	...
	ai_AiPlayCardPassive(stringbuffer, function(reply)
		local landAiPaiMove = pb.decode("LandAiPaiMove", reply)
		--log.error(string.format("ai_play_card_passive type[%d] alone_1[%d] alone_2[%d] alone_3[%d] alone_4[%d] pb_combo_list[%s]",landAiPaiMove.type,landAiPaiMove.alone_1,landAiPaiMove.alone_2,landAiPaiMove.alone_3,landAiPaiMove.alone_4,table.concat(landAiPaiMove.pb_combo_list.cards, ",")))
		local newCards =  land_cards.convert_aicards_cards(landAiPaiMove,player_cards)
		if #newCards == 0 then
		...
		if ai_time > 14 then
			log.error(string.format("do_ai_play_card_passive table [%d] time [%d]",self.table_id_,ai_time))
		end
...
function land_table:statrGameBegin( ... )
    ...
    log.info(string.format("table_id [%d] playWithAndroid_isopen [%s] bad_card_coeff [%s]", self.table_id_ , tostring(self.playWithAndroid_isopen) , tostring(bad_card_coeff) , tostring(self.system_bad_card_coeff)))
	if self.playWithAndroid_isopen and bad_card_coeff < self.system_bad_card_coeff then
		if self:shuffle_cheat_cards() == true then --做差牌成功
			log.info(string.format("shuffle_cheat_cards-------------------->ok."))
			self.bad_card_make_flag = 1
			log.info(string.format("befor change cards ----->last three cards [%d][%d][%d]",self.cards[52], self.cards[53], self.cards[54]))
			self.cards = {}
			...
			log.info(string.format("after change cards ----->last three cards [%d][%d][%d]",self.cards[52], self.cards[53], self.cards[54]))
		end
	end
	...
	if self.bad_card_make_flag == 0 and good_cards_coeff <  this_time_coeff then --满足概率，随机做1副好牌
		...
		--log.info(string.format("~~~~~please send good cards............"))
	else
		--log.info(string.format("~~~~~please send normal cards............"))
	end
	if self.good_cards_flag == 1 then
		...
		if self.this_good_cards ~= nil and self:check_this_good_cards(self.this_good_cards) == true then --抽好牌成功
			...
			--log.info(string.format("last_three_cards[%d] [%d] [%d]",self.three_cards[1][1],self.three_cards[1][2],self.three_cards[1][3]))
	...
	if self.good_cards_flag == 1 then
		...
		if android_count == 2 and player_index == 1 then
			...
			log.info(string.format("player change cards index [%d]" ,change_index))
		end
		...
		for i,v in ipairs(self.player_list_) do
			if v then
				...
				--log.info(string.format("v.chair_id: [%d]" ,v.chair_id))
		...
		if self.bad_card_make_flag == 1 then
			...
			for i,v in ipairs(self.player_list_) do
				if v then
					...
					msg.cards = cards
					send2client_pb(v, "SC_LandStart", msg)
					log.info(string.format("make bad cards-------->guid[%d]  chair_id: [%d]." ,v.guid,v.chair_id))
					log.info(table.concat(msg.cards, ','))
	...
	log.info(string.format("first call soure chairid : %s ", tostring(self.first_turn)))
	...
	--机器人叫分
	if self.player_list_[self.cur_turn].is_android then
...
function land_table:send_land_cards(player)
	...
	log.info(string.format("==========send_land_cards: [%s]" ,tostring(self.cur_call_score_chair_id)))
	...
	log.info(string.format("landcards [%s]",table.concat(cards_,",")))
	log.info(string.format("befor landplayercards [%s]",table.concat(self.land_player_cards[self.cur_call_score_chair_id].cards_,",")))
	...
	log.info(string.format("after number [%d] landplayercards [%s]",#self.land_player_cards[self.cur_call_score_chair_id].cards_,table.concat(self.land_player_cards[self.cur_call_score_chair_id].cards_,",")))
...
function land_table:call_double(player,is_double)
	...
	log.info(string.format("call_double begin ---- %s",tostring(is_double)))
...
function land_table:startGame( ... )
	...
	log.info(string.format("table_game_id is %s",self.table_game_id))
	self:next_game()

	log.info(string.format("get_now_game_id is %s",tostring(self:get_now_game_id())))
	-- 获取开始时间
	self.time0_ = get_second_time()
...
function land_table:set_LandLastTrusteeship(player,msg)
	...
	if msg then
		...
		log.info(string.format("msg is nil"))
		msg = { isTrusteeship = false }
	end
	log.info(string.format("player guid [%d] set LastTrusteeship [%s]" , player.guid , tostring(msg.isTrusteeship)))
	...
	log.info(string.format("player guid [%d] player.LastTrusteeship [%s]" , player.guid , tostring(player.LastTrusteeship)))
...
function land_table:call_score(player, callscore)
	log.info(string.format("==========call_score status[%d]",self.status))
	if self.status ~= LAND_STATUS_CALL then
		log.warning(string.format("land_table:call_score guid[%d] status error", player.guid))
		return
	end

	if player.chair_id ~= self.cur_turn then
		log.warning(string.format("land_table:call_score guid[%d] turn[%d] error, cur[%d]", player.guid, player.chair_id, self.cur_turn))
		return
	end

	if callscore < 0 or callscore > 3 then
		log.error(string.format("land_table:call_score guid[%d] score[%d] error", player.guid, callscore))
		return
	end

	if callscore > 0 and callscore <= self.cur_call_score then
		log.error(string.format("land_table:call_score guid[%d] score[%d] error, cur[%d]", player.guid, callscore, self.cur_call_score))
		return
	end
	-- 记录叫分 日志
	local call_log = {
	...
	log.info(string.format("callscore is : %s",tostring(callscore)))
	if callscore == 3 then
		local notify = {
			cur_chair_id = self.cur_turn,
			call_chair_id = player.chair_id,
...
function land_table:out_card(player, cardslist, flag)
	--log.info(string.format("player: %d" , player.chair_id))
	log.info(table.concat(cardslist,","))
	if self.status ~= LAND_STATUS_PLAY then
		log.warning(string.format("land_table:out_card guid[%d] status error", player.guid))
		return
	end

	if player.chair_id ~= self.cur_turn then
		log.warning(string.format("land_table:out_card guid[%d] turn[%d] error, cur[%d]", player.guid, player.chair_id, self.cur_turn))
		return
	end

	local playercards = self.land_player_cards[player.chair_id]
	if not playercards:check_cards(cardslist) then
		log.error(string.format("land_table:out_card guid[%d] out cards[%s] error, has[%s]", player.guid, table.concat(cardslist, ','), table.concat(playercards.cards_, ',')))
		return
	end
	...
	log.info(string.format("gameid[%s] cardstype[%s] cardsval[%s]" , tostring(self.table_game_id) , tostring(cardstype) , tostring(cardsval) ))
	if not cardstype then
		log.error(string.format("land_table:out_card guid[%d] get_cards_type error, cards[%s]", player.guid, table.concat(cardslist, ',')))
		return
	end
...
function land_table:pass_card(player, flag)
	if self.status ~= LAND_STATUS_PLAY then
		log.warning(string.format("land_table:pass_card guid[%d] status error", player.guid))
		return
	end

	if player.chair_id ~= self.cur_turn then
		log.warning(string.format("land_table:pass_card guid[%d] turn[%d] error, cur[%d]", player.guid, player.chair_id, self.cur_turn))
		return
	end

	if not self.last_out_cards then
		log.error(string.format("land_table:pass_card guid[%d] first turn", player.guid))
		return
	end
...
function land_table:Next_Player_Proc( ... )
	...
	if  self.status == LAND_STATUS_CALL then
		...
		elseif self.player_list_[self.cur_turn].is_android then
			--机器人叫分
			self:ai_grab_landlord(self.cur_turn)
		end
	elseif self.status == LAND_STATUS_PLAY then
		--log.info("========================================Next_Player_Proc")
		if self.player_list_[self.cur_turn].Dropped or self.player_list_[self.cur_turn].isTrusteeship or self.player_list_[self.cur_turn].LastTrusteeship then
...
function  land_table:is_play( ... )
	log.info(string.format("land_table:is_play : [%s]",tostring(self.status)))
	-- body
...
--请求玩家数据
function land_table:reconnection_play_msg(player)
	-- body
	log.info(string.format("player online : %s " , tostring(player.chair_id)))
...
function  land_table:inList(player)
	...
	if channellist then
		for _,v in pairs(channellist) do
			log.info(string.format("v [%s] player.channel_id [%s]" , v , player.channel_id))
...
function  land_table:canAddAndroid(player)
	-- body
	log.info(string.format("player.is_android[%s] self:get_player_count() [%d] elf:isAndroidWithPlay(player) [%s]", tostring(not player.is_android) ,self:get_player_count() , tostring(self:isAndroidWithPlay(player))))
...
function land_table:player_sit_down(player, chair_id_)
	...
	if self:canAddAndroid(player) then
		log.info(string.format("add_android ============================================== [%d]",player.guid))
...
function land_table:ready(player)
	...
	log.info(string.format("land_table:ready ======= :%s guid[%s]",tostring(player.table_id),tostring(player.guid)))
	if self:is_play() then
		log.info(string.format("land_table: is play  can not ready ======= tableid: %s",tostring(player.table_id)))
		return
	end
...
function  land_table:recoveryplayercard(player)
	...
	if self.status == LAND_STATUS_PLAY or self.status == LAND_STATUS_DOUBLE then
		...
		log.info(string.format("chairid[%d] cards[%s]",player.chair_id,table.concat( self.land_player_cards[player.chair_id].cards_, ", ")))
...
function  land_table:player_offline( player )
	...
	log.info(string.format("player offline : guid[%d] chairid[%s]",player.guid,player.chair_id))
	-- body
	if self.status == LAND_STATUS_FREE then
		-- 等待开始时 掉线则强制退出玩家
		player:forced_exit()
...
--		self:next_game()
		...
		for i,v in ipairs(self.player_list_) do
			log.info(string.format("======== chairid is %d",v.chair_id))
			if self:isDroppedline(v) then
				log.info(string.format("this player is offline: %d",v.chair_id))
			end
			local s_type = 1
		...
		if self.flag_land == player.chair_id then
			...
			log.info(string.format("score_multiple[%d] room_cell_score[%d]",score_multiple,room_cell_score))
			...
			for i,v in ipairs(self.player_list_) do
				...
				if self.flag_land == v.chair_id then
					...
					if self:isDroppedline(v) and offlinePunishment_flag then
						s_type = 3
						log.info(string.format("land win chair_id[%d] but offline",v.chair_id))
						...
						s_tax = math.ceil(notify.pb_conclude[v.chair_id].score * self.room_:get_room_tax())
						-- 收取%5 税收 math.ceil 可能没必要
						log.info(string.format("ceil befor : %s",tostring(notify.pb_conclude[v.chair_id].score)))
						notify.pb_conclude[v.chair_id].score = notify.pb_conclude[v.chair_id].score - s_tax
						log.info(string.format("ceil after : %s",tostring(notify.pb_conclude[v.chair_id].score)))
						v:add_money({{money_type = ITEM_PRICE_TYPE_GOLD, money = notify.pb_conclude[v.chair_id].score}}, LOG_MONEY_OPT_TYPE_LAND)
						log.info(string.format("land win add money: %s",tostring(notify.pb_conclude[v.chair_id].score)))
					...
					self:save_player_collapse_log(v)
					log.info(string.format("farmer loss cost money: %s",tostring(notify.pb_conclude[v.chair_id].score)))
				end
				self.gamelog.playInfo[v.chair_id].tax = s_tax
				self.gamelog.playInfo[v.chair_id].new_money = v.pb_base_info.money
				notify.pb_conclude[v.chair_id].tax = s_tax
				notify.pb_conclude[v.chair_id].flag = self.room_.tax_show_
				log.info(string.format("game finish playerid[%d] guid[%d] money [%d] tax[%d]",v.chair_id,v.guid,v.pb_base_info.money,s_tax))
				self:player_money_log(v,s_type,s_old_money,s_tax,notify.pb_conclude[v.chair_id].score,self.table_game_id)
			end
		else
			log.info("land loss")
			...
			for i,v in ipairs(self.player_list_) do
				...
				if self.flag_land == v.chair_id then
					...
					log.info(string.format("land loss cost money: %s", tostring(notify.pb_conclude[v.chair_id].score)))
					self:save_player_collapse_log(v)
				else
					s_type = 2
					local farmer_score = 0
					...
					if not self:isDroppedline(v) or not offlinePunishment_flag then
						...
						s_type = 3
						log.info(string.format("chair_id[%d] win but offline",v.chair_id))
						notify.pb_conclude[v.chair_id].score = 0
						self.gamelog.playInfo[v.chair_id].gameEndStatus = "farmer win but offline"
					end
					log.info(string.format("farmer win add money: %d",notify.pb_conclude[v.chair_id].score))
				end
				self.gamelog.playInfo[v.chair_id].tax = s_tax
				self.gamelog.playInfo[v.chair_id].new_money = v.pb_base_info.money
				log.info(string.format("game finish playerid[%d] guid[%d] money[%d] tax[%d]",v.chair_id,v.guid,v.pb_base_info.money,s_tax))
	...
	-- end
	log.info("game end")
	log.info(string.format("chuntian [%d] ,fanchuntian [%d]",notify.chuntian,notify.fanchuntian))
	for _,var in ipairs(notify.pb_conclude) do
		log.info(string.format("score [%d] ,bomb_count [%d], cards[%s]",var.score,var.bomb_count,table.concat( var.cards, ", ")))
	end
	...
	--	else
	--		log.info(string.format("v is nil: %d",i))
	--	end
	--end
...
function  land_table:isDroppedline(player)
	...
	if player then
		...
		if player.chair_id then
			log.info(string.format("land_table:isDroppedline:guid[%d] table[%d] chair[%d]",player.guid,player.table_id,player.chair_id))
		end
...
function land_table:can_enter(player)
	...
	if not player.is_android and self:get_player_count() > 0 then
		...
		if self:isAndroidWithPlay(player) then
			...
			log.info(string.format("===============================================land_table:can_enter false [%d] " ,player.guid))
			return false
		end
	end
	log.info("===============================================land_table:can_enter true")
	...
	for _,v in ipairs(self.player_list_) do
		if v and v.guid ~= player.guid then
			...
			if player:judge_play_times(v,LimtCDTime) then
				log.info(string.format("tableid[%d] judge_play_times is true playerGuid[%d] otherGuid[%d]",self.table_id_,player.guid,v.guid))
				v.ipControlTime = get_second_time()
			else
				-- 再判断
				if v:judge_play_times(player,LimtCDTime)	then
					...
					log.info(string.format("tableid[%d] judge_play_times is false playerGuid[%d] otherGuid[%d]",self.table_id_,player.guid,v.guid))
					return false
				end
			end
...
function land_table:tick()
					...
					if t then
						if curtime -  t >= LAND_TIME_IP_CONTROL then
							...
							if self:isDroppedline(v) then
								...
								log.info(string.format("tableid is %s",tostring(v.table_id)))
								v.ipControlflag = true
								log.info("===============land_table tick")
								--[[]]
								if self.ready_list_[v.chair_id] then
									log.info(string.format("guid[%d] tableid [%d] chairid [%d] chair is ready",v.guid,self.table_id_,v.chair_id))
								else
									log.info(string.format("guid[%d] tableid [%d] chairid [%d] chair is not ready",v.guid,self.table_id_,v.chair_id))
								end
								log.info(string.format("player[%d] readyPlayer[%d]",self:get_player_count() ,#self.ready_list_))
	...
	elseif self.status == LAND_STATUS_PLAY then
		...
		if curtime - self.time0_ >= self.time_outcard_ then
			...
			if player and player.chair_id then
				log.info(string.format("time out : %d" ,player.chair_id))
				if not player.TrusteeshipTimes then
					player.TrusteeshipTimes = 0
				end
				player.TrusteeshipTimes = player.TrusteeshipTimes + 1
				log.info(string.format("player.is_android [%s] player.TrusteeshipTimes [%d] player.isTrusteeship[%s] player.LastTrusteeship[%s]",tostring(player.is_android),player.TrusteeshipTimes,tostring(player.isTrusteeship),tostring(player.LastTrusteeship)))
		...
		if self.status == LAND_STATUS_PLAY and curtime - self.gamelog.start_game_time > LAND_TIME_OVER then
			self:finishgameError()
			log.warning(string.format("LAND_TIME_OVER gameTableid [%s]",self.table_game_id))
		end
	elseif self.status == LAND_STATUS_CALL then
		local curtime = get_second_time()
		if curtime - self.time0_ >= LAND_TIME_CALL_SCORE then
			...
			local player = self.player_list_[self.cur_turn]
			if player then
				log.info(string.format("call_score time out call 0: [%d]",player.chair_id))
				self:call_score(player, 0)
			else
	...
	elseif self.status == LAND_STATUS_PLAYOFFLINE then
		...
		-- 游戏结束 进行结算
			log.info(string.format("LAND_TIME_WAIT_OFFLINE time out time0[%d] curtime[%d]",self.time0_ ,curtime))
			self:finishgame()
		end
	end
end
...
function land_table:shuffle_cheat_cards()
	...
	while (iBadCardsCount < 13) do
		...
		if arrSmallCardStat[iColor][iValueIndex] == 1 and iValueIndex ~= iLackValue1 and iValueIndex ~= iLackValue2 then
			if arrSmallCardStat[0][iValueIndex] + arrSmallCardStat[1][iValueIndex] + arrSmallCardStat[2][iValueIndex] + arrSmallCardStat[3][iValueIndex] > 1 then
				...
				--log.info(string.format("%d is  iValueIndex[%d] iColor[%d]" , iBadCardsCount - 1 , iValueIndex , iColor))
			end
		end
	end
	...
	--log.info(table.concat(bad_cards, ','))
	-- log.info(string.format("bad_cards count---------------->[%d]", getNum(bad_cards)))
	-- log.info(table.concat(self.new_cards, ','))
	local left_cards = {}
	...
	-- log.info(string.format("left_cards count---------------->[%d]", getNum(left_cards)))
