...
local base_player = require "base_player"
...
base_bonus_activity_manager = base_bonus_activity_manager or { active_bonus_activities = {}}

base_bonus_activity = class("base_bonus_activity")
...
--	log.info("base_bonus_activity:tick")
	...
	if self:is_totally_elasped() and not self.broadcast_when_totally_end then
		log.info(string.format("base_bonus_activity:tick bonus [%d] end",self.id))
...
function base_bonus_activity_manager:load_activity(id)
	send2db_pb("SD_ReqQueryBonusActivity",{id = id})
end

function base_bonus_activity_manager:remove(id)
	self.active_bonus_activities[id] = nil
end

function base_bonus_activity_manager:foreach(func)
	table.walk(self.active_bonus_activities,func)
end

function base_bonus_activity_manager:find(id)
	return self.active_bonus_activities[id]
end

function base_bonus_activity_manager:new_activity(id,start_time,end_time,platform_id,cfg)
	self.active_bonus_activities[id] = base_bonus_activity.new(id,start_time,end_time,platform_id,cfg)
end

function base_bonus_activity_manager:filter_by_platform(platform_id)
	if not platform_id or platform_id < 0 then return self.active_bonus_activities end
	local activities = {}
	for id,activity in pairs(self.active_bonus_activities) do
...
function base_bonus_activity_manager:latest_activity(platform_id)
	local latest_activity = nil
	for _,activity in pairs(self.active_bonus_activities) do
...
function base_bonus_activity_manager:tick()
	self:foreach(function(activity) 
		activity:tick()
		if activity:is_totally_elasped() then
				base_bonus_activity_manager:remove(activity.id)
		end
	end)
end


base_bonus = class("base_bonus")
...
function base_player:increase_play_info(money)
	...
	base_bonus_activity_manager:foreach(function(activity)
...
function base_player:send_bonuses()
	local latest_activity = base_bonus_activity_manager:latest_activity(tonumber(self.platform_id))
...
function base_player:check_and_switch_to_next_bonus(activity_id)
	local activity = base_bonus_activity_manager:find(activity_id)
	if not activity then 
		log.error(string.format("base_player:check_and_switch_to_next_bonus not find activity id [%d]",activity_id))
		return 
	end
...
function base_player:check_and_create_bonus()
	...
	base_bonus_activity_manager:foreach(function(activity)
		...
		if not table.elements_or_operation(activity.cfg.game_types,function(first_game_type) 
			return first_game_type == def_first_game_type
		end) then
			log.info(string.format("check_and_create_bonus can not match first_game_type [%d]",def_first_game_type))
			return
		end
		...
		then
			...
			log.warning(string.format("get bonus guid [%d] money [%d] activity_id[%d] index[%d]",self.guid,hongbao_money,activity.id,cur_bonus_index))
