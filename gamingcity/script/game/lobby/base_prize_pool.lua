...
base_prize_pool = base_prize_pool or {}
...
function base_prize_pool:prize_pool_init()
	...
	self.game_log_id = {}
	if prize_pool_robot_name and type(prize_pool_robot_name) == "table" and prize_pool_robot_name[def_first_game_type] and type(prize_pool_robot_name[def_first_game_type]) and prize_pool_robot_name[def_first_game_type][def_second_game_type] and type(prize_pool_robot_name[def_first_game_type][def_second_game_type])  == "table" then
		self.robot_name = prize_pool_robot_name[def_first_game_type][def_second_game_type]
	end

	--dump(self.robot_name)
end
...
function base_prize_pool:prize_pool_is_open()
	if self.prizepool_cfg.open == 1 then
		...
		--log.info(string.format("start[%d] end[%d] now[%d]" , self.prizepool_cfg.time_start , self.prizepool_cfg.time_end  , now))
		...
		elseif self.run_flag then
			...
			log.info(string.format("prize_pool_is_open =============================== %s",self.player_list))
			for room_id , room in pairs(self.player_list) do
				log.info(string.format("roooid [%d] ============== prize pool close table_count[%d]" , room_id,getNum(room)))
				for table_id , _ in pairs(room) do
					log.info(string.format("room_id[%d] table[%d] send SC_PrizePool_show = 0 ",room_id , table_id))
...
function base_prize_pool:prize_pool_show(room_id ,table_id)
	...
	--通知更新奖池
	log.info(string.format("============SC_PrizePool_show room_id[%d] table_id[%d] [%d]",room_id , table_id,self.prizepool.show))
...
function base_prize_pool:broadcast2client(room_id , table_id, msg_name, pb)
	...
	for i, p in pairs(self.player_list[room_id][table_id] ) do
		if p and type(p) == "table" then
			log.info(string.format("============send guid[%d]",p.guid))
...
function base_prize_pool:send_prize_pool_money(table_id , table_self , user_list)
	for k,v in pairs(user_list) do
		...
		log.info(string.format("=========================1  guid[%d]",k))
		if v then
			local smoney = v.player:get_money()
			local money = user_list[v.player.guid]

			log.info(string.format("=========================1  guid[%d] smoney[%d] money[%d]",k ,smoney , money))
			if money > 100 then
				local notify = {}
				...
				log.info(string.format("guid[%d] old_money[%d] new_money[%d]" , k, smoney, v.player:get_money()))
				...
				log.info(string.format("guid[%d] name[%s] money[%d]" ,v.player.guid ,  v.player.nickname , money))
				send2client_pb(v.player,"SC_Prize_Pool_Result",notify)
			end
		else
			log.info(string.format("=========================1  guid[%d] not find",k))
		end
	end
...
function base_prize_pool:set_table_player_list(room_id , table_id , player_list)
	-- body
	log.info(string.format("room_id[%d] , table_id [%d]" , room_id , table_id))
	...
	log.info(string.format("set_table_player_list roooid [%d] ============== prize pool open table_count[%d]" , room_id,getNum(self.player_list[room_id])))
end
...
function base_prize_pool:load_lua_cfg(room_id , cfgstr )
	-- body
	log.info(string.format("%s" , cfgstr))
	if game_name_list[def_game_name] == 1 then
		...
		log.info(string.format("%s" , jsonStr))
		...
		if cfg and type(cfg) == "table" and cfg.prizepool and type(cfg.prizepool) == "table" then
			...
			if is_open and not self:prize_pool_is_open() then
				log.info(string.format("close prize_pool ===============================1 %s",self.player_list))
				for room_id , room in pairs(self.player_list ) do
					log.info(string.format("roooid [%d] ============== prize pool close table_count[%d]" , room_id,getNum(room)))
					for table_id , _ in pairs(room) do
						log.info(string.format("room_id[%d] table[%d] send SC_PrizePool_show = 0 ",room_id , table_id))
			...
			elseif not is_open and self:prize_pool_is_open() then
				log.info(string.format("close prize_pool ===============================2 %s",self.player_list))
				for room_id , room in pairs(self.player_list ) do
					log.info(string.format("roooid [%d] ============== prize pool open table_count[%d]" , room_id,getNum(room)))
					for table_id , _ in pairs(room) do
						log.info(string.format("room_id[%d] table[%d] send SC_PrizePool_show ",room_id , table_id))
						self:prize_pool_show(room_id ,table_id)
					end
				end
			end
...
function base_prize_pool:send_player_show( player )
	--通知更新奖池
	if self:prize_pool_is_open() then
		log.info(string.format("============SC_PrizePool_show guid[%d]",player.guid))
...
function base_prize_pool:game_start(room_id , table_id , game_log_id)
	-- body
	if self:prize_pool_is_open() then
		log.info(string.format("game_start table_id [%d] game_log_id [%s] " , table_id , game_log_id))
		self.game_log_id[table_id] = game_log_id
	elseif self.prizepool_cfg.open == 0 then
		self:prize_pool_init()
		log.info(string.format("room_id[%d] table_id[%d] send SC_PrizePool_show = 0 ",room_id , table_id))
		self:broadcast2client(room_id , table_id , "SC_PrizePool_show",{money = 0})
	end
end
...
function base_prize_pool:input_bet_list( table_id , player, bet_money )
	...
	if self:prize_pool_is_open() and self.game_log_id[table_id] then
		log.info(string.format("input_bet_list table_id [%d] guid [%d] bet_money[%d]" , table_id , player.guid , bet_money))
		if not self.player_bet_info[table_id] then
			self.player_bet_info[table_id] = {}
		end
...
function base_prize_pool:game_end(table_id ,table_self, money , gamelog)
	if self:prize_pool_is_open() and self.game_log_id[table_id] then
		...
		log.info(string.format("input_bet_list table_id [%d] end_money [%d] " , table_id , money ))
		...
		log.info(string.format("self.prizepool.pool end by table[%d]  [%d]====> [%d]" ,table_id , money_old , self.prizepool.pool))
	end
end