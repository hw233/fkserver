...
function on_ds_query_bonus_activities(msg)
	...
	table.walk(msg.pb_activities,function(activity)
		base_bonus_activity_manager:new_activity(activity.id,activity.start_time,activity.end_time,activity.platform_id,activity.cfg)
	end)

	base_players:foreach(function(p)
		local activity = base_bonus_activity_manager:latest_activity(tonumber(p.platform_id))
		dump(activity)
		if activity then
...
function on_cs_query_bonus_activities(player,msg)
	player:send_unelasped_activities()
end

function on_cs_query_bonus(player,msg)
	player:send_bonuses()
end
...
function on_ls_load_bonus_config(msg)
	...
	base_bonus_activity_manager:load_activity(msg.activity_id)
end
