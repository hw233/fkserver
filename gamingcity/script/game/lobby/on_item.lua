...
function on_cs_buy_item(player, msg)
	if msg.item_num <= 0 then
		log.warning(string.format("guid[%d] item num[%d] <= 0", player.guid, msg.item_num))
	...
	if not goods then
		log.error(string.format("guid[%d] item id[%d] not find in item market table", player.guid, msg.item_id))
		send2client_pb(player, "SC_BuyItem", {
			item_id = msg.item_id,
			item_num = msg.item_num,
	...
	if not item_details_table[msg.item_id] then
		log.error(string.format("guid[%d] item id[%d] not find in item details table", player.guid, msg.item_id))
		send2client_pb(player, "SC_BuyItem", {
			item_id = msg.item_id,
			item_num = msg.item_num,
			result = ITEM_OPERATE_RESULT_ITEMID_ERR,
	...
	if not player:cost_money({{money_type = goods.price.money_type, money = msg.item_num * goods.price.money}}, LOG_MONEY_OPT_TYPE_BUY_ITEM) then
		log.error(string.format("guid[%d] item id[%d] money not enough", player.guid, msg.item_id))
		send2client_pb(player, "SC_BuyItem", {
			item_id = msg.item_id,
			item_num = msg.item_num,
