...
function on_gs_recharge(msg)
	-- body
	log.info(string.format("on_gs_recharge  begin----------------- guid[%d] type[%d] money[%d] orderid[%d]" , msg.guid,msg.type,msg.money,msg.orderid))
	local notify = {
		guid = msg.guid,
		type = msg.type,
		money = msg.money,
	...
	if player and player.pb_base_info then
		local beforbank = player.pb_base_info.bank
		local bRet = player:change_bank(msg.money, msg.type, true)
		if bRet == true then
			log.info(string.format("on_gs_recharge success guid[%d] type[%d] money[%d] orderid[%d] beforbank[%d] afterbank[%d]" , msg.guid,msg.type,msg.money,msg.orderid,beforbank,player.pb_base_info.bank))
			local notify_T = {
				guid = msg.guid,
				orderid = msg.orderid,
				loginid = msg.loginid,
				...
				after_bank = player.pb_base_info.bank
			}
			log.info(string.format("on_gs_recharge success guid[%d] type[%d] money[%d] orderid[%d] beforbank[%d] afterbank[%d]" , notify_T.guid,msg.type,msg.money,notify_T.orderid,notify_T.befor_bank,notify_T.after_bank))
...
function on_changmoney_deal(msg)
	local info = msg.info
	log.info(string.format("on_changmoney_deal  begin----------------- player  guid[%d]  money[%g] type[%d] order_id[%d]", info.guid, info.gold, info.type_id, info.order_id))
	...
	if player and player.pb_base_info then
		...
		log.error(string.format("on_changmoney_deal no find player  guid[%d]", info.guid))
		fmsg = {
		web_id =  msg.web_id,
		login_id = msg.login_id,
		info = msg.info,
...
function on_GS_UpdatePlayerBank(msg)
	...
	log.info(string.format("on_GS_UpdatePlayerBank  begin----------------- player  guid[%d]  opt_type[%g] old_bank[%d] new_bank[%d]", msg.guid, msg.opt_type, msg.old_bank, msg.new_bank))
	local player = base_players[msg.guid]
	...
	if player and player.pb_base_info then
		...
		log.warning(string.format("on_GS_UpdatePlayerBank no find player  guid[%d]", msg.guid))
	end
end
...
function on_cash_false_deal(msg)
	local info = msg.info
	log.info(string.format("on_changmoney_deal  begin----------------- player  guid[%d]  money[%g]  order_id[%d]", info.guid, info.coins, info.order_id))
	local player = base_players[info.guid]
	local nmsg = {
		web_id = msg.web_id,
		result = 1,
	...
	if player and  player.pb_base_info then
		...
		if bRet == false then
			nmsg.result = 6
			log.warning(string.format("on_cash_false_deal..............................%d add money false player", info.guid))
		end
	else
		nmsg.result = 5
		log.warning(string.format("on_cash_false_deal..............................%d no find player", info.guid))
	end
	...
	redis_cmd_query(string.format("get %s",var_platform), function (reply)
		if type(reply) == "string" then
			...
			if switchforcashvalue and switchforcashvalue[switch_key] then
				log.info(string.format("---------------->cash switch key[%s] value[%s]", tostring(switch_key),tostring(switchforcashvalue[switch_key])))
...
function get_player_platform_cashswitch(player,msg, remain_money)
	...
	log.info(string.format("player guid[%d] platform_info [%s] cash type [%s]",player.guid, tostring(platform_info), tostring(cash_key)))
...
function do_cs_cash_money_do(player, msg, remain_money)
	-- body
	log.info(string.format("on_cs_cash_money  begin----------------- player  guid[%d]  money[%d] seniorpromoter[%d]", player.guid, msg.money ,player.seniorpromoter))
	local nmsg = {
		result = 1,
		bank = player.pb_base_info.bank ,
		money = player.pb_base_info.money,
	...
	--	log.error(string.format("cash maintain ===============cash_switch [%d]",cash_switch))
	...
	if player.disable == 1 then
		log.error(string.format("cash is disable [%d]",player.guid))
		send2client_pb(player,"SC_CashMoneyResult", nmsg)
		return
	end
	...
	if (player.alipay_account == nil or player.alipay_account == "") and (player.alipay_name == nil or player.alipay_name == "") then
		log.error (string.format("alipay is empty [%d]",player.guid))
		nmsg.result = 9
		send2client_pb(player,"SC_CashMoneyResult", nmsg)
		return
	end
	--银行账号
	if msg.cash_type == 4 then
		log.info(string.format("do_cs_cash_money_do bank_card ------- guid[%d] bank_card_num[%s] bank_card_name[%s] bank_name[%s]",player.guid, player.bank_card_num, player.bank_card_name, player.bank_name))
		if (player.bank_card_num == nil or player.bank_card_num == "**") or (player.bank_card_name == nil or player.bank_card_name == "**") or (player.bank_name == nil or player.bank_name == "")then
			log.error (string.format("bank_card is empty [%d]",player.guid))
			nmsg.result = 10
			send2client_pb(player,"SC_CashMoneyResult", nmsg)
			return
		end
	...
	if player.pb_base_info.bank + player.pb_base_info.money < msg.money + remain_money then
		...
		log.error(string.format("msg.money[%d] all_money_t_ [%d]",msg.money,all_money_t_))
		send2client_pb(player,"SC_CashMoneyResult", nmsg)
		return
	end
	...
	if player.pb_base_info.bank < msg.money then
		money_cost = msg.money - player.pb_base_info.bank
...
function do_cs_cash_money (player, msg)
	...
	redis_cmd_query(string.format("GET cash_remain_money"), function (reply)
		if type(reply) == "string" or type(reply) == "number" then
			...
			log.info(string.format("remain_money = [%d]", remain_money))
			get_player_platform_cashswitch(player, msg, remain_money)
		else
			log.error(string.format("do_cs_cash_money player[%d] get cash_remain_money error..............................",player.guid))
			if player then
					local nmsg = {
					result = 2,
					bank = player.pb_base_info.bank ,
...
function on_SD_CheckCashTime(msg)
	...
	local player = base_players[msg.guid]
	if not player then
		log.warning(string.format("on_SD_CheckCashTime guid[%d] not find in game", msg.guid))
		return
	end
	...
	elseif msg.cash_max_count and msg.cash_max_count == 2 then		
		log.info(string.format("on_SD_CheckCashTime guid[%d] msg.cash_max_count[%d]",player.guid,msg.cash_max_count))
		local nmsg = {
			result = 30,
			bank = player.pb_base_info.bank ,
			money = player.pb_base_info.money,
...
function on_cs_cash_money(player, msg)
	...
	if player.table_id ~= nil or player.room_id ~= nil then
		log.error(string.format("player guid[%d] on_cs_cash_money error.",player.guid))
	    return
	end
	local cash_type_ = 1
...
function on_ds_cash_money_type( msg )
	...
	if player then
		...
		log.warning(string.format("on_ds_cash_money_type..............................%d no find player", msg.guid))
	end
...
function on_DS_WithDrawCash(msg)
	log.info (string.format("on_DS_WithDrawCash begin  guid[%d]  ret[%d]", msg.guid, msg.result))
	local player = base_players[msg.guid]
	if player and  player.pb_base_info  then
		...
		log.warning(string.format("on_ds_cash_money..............................[%d] no find player", msg.guid))
	end
