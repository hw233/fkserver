...
function multi_showhand_table:init(room, table_id, chair_count)
	...
	self.game_max_bet = self.room_.cell_score_ * 257 --默认该局梭哈上限为底注的256倍(底注×256倍)
...
function  multi_showhand_table:check_maintain()
	...
	if self.cur_state_FSM == FSM_S.WAITING or self.cur_state_FSM == FSM_S.PER_BEGIN or next_state == FSM_S.XI_PAI then --准备时间
		if self:check_single_game_is_maintain() == true or game_switch == 1 then
			log.info(string.format("Game will maintain..game_switch=[%d] self.room_.game_switch_is_open[%d].....................",game_switch,self.room_.game_switch_is_open))
...
function multi_showhand_table:load_lua_cfg()
	log.info(string.format("multi_showhand_table: game_maintain_is_open = [%d] game_switch[%d]",self.room_.game_switch_is_open,game_switch))
	self:check_maintain()
	...
	if multi_showhand_config then
		...
		log.info(string.format("max_call = [%d] A_is_7 = [%d] MULTI_SHOWHAND_PRICE_BASE = [%d] CELL_SCORE[%d] black_rate = [%d].",MAX_CALL,A_is_7,MULTI_SHOWHAND_PRICE_BASE,CELL_SCORE,BLACK_RATE))
	end
...
function multi_showhand_table:can_enter(player)
	local next_state = self.cur_state_FSM - FSM_S.GAME_IDLE_HEAD
	log.info(string.format("player guid[%d] can_enter next_state[%d] cur_state_FSM[%d]",player.guid,next_state,self.cur_state_FSM))
	if self.table_busy == 1 and next_state == FSM_S.XI_PAI and self.last_action_change_time_stamp <= get_second_time() + 1 then
		log.warning(string.format("Game jast begin can not enter, player guid[%d] can not enter.",player.guid))
		return false
	end

	if player.vip == 100 and self:getNum(self.player_list_) < 5 then
		return true
	end
	
	if self:getNum(self.player_list_) >= 5 then
...
function  multi_showhand_table:check_player_number()
	...
	if self.cur_state_FSM == FSM_S.WAITING or self.cur_state_FSM == FSM_S.PER_BEGIN or next_state == FSM_S.XI_PAI then
		if self:getNum(self.player_list_) < 3 then
			self:reset()
		end
	end
end
...
function multi_showhand_table:check_cancel_ready(player, is_offline)
	...
	if player.cur_status == 0  then --观众
		log.info(string.format("player guid[%d] check cancel ready true,cur_status[%s]", player.guid,tostring(player.cur_status)))
...
function multi_showhand_table:is_play( player )
	...
	if self.do_logic_update then
		if player.is_dead == true or player.cur_status == 0 then
			log.info(string.format("player guid[%d] is_dead[%s] cur_status[%d] can exit",player.guid, tostring(player.is_dead),player.cur_status))
			return false
		end
		local next_state = self.cur_state_FSM - FSM_S.GAME_IDLE_HEAD
		log.info(string.format("player guid[%d] game_cur_status[%d] next_state[%d]",player.guid, self.cur_state_FSM,next_state))
		if self.cur_state_FSM == FSM_S.XI_PAI or self.cur_state_FSM == FSM_S.GAME_ROUND or self.cur_state_FSM == FSM_S.GAME_BALANCE or next_state == FSM_S.GAME_ROUND or next_state == FSM_S.GAME_BALANCE then
			if not player.is_dead and not player.cur_status then
				log.info(string.format("player guid[%d] can exit, is_dead[%s] cur_status[%s]",player.guid, tostring(player.is_dead),tostring(player.cur_status)))
				return false
			else
				log.info(string.format("player guid[%d] is play true, cur_state = [%d],next_state[%d],can not exit.",player.guid,self.cur_state_FSM,next_state))
				return true
			end
		else
			self:check_player_number()
	...
	log.info(string.format("player guid[%d] is not play, cur_state = [%d] can exit",player.guid, self.cur_state_FSM))
	self:check_player_number()
	return false
end
...
function multi_showhand_table:player_stand_up(player, is_offline)
	self.player_count = self:getNum(self.player_list_)
	log.info(string.format("player guid[%d] stand up player_count[%d] player_list num[%d]",player.guid,self.player_count,self:getNum(self.player_list_)))
	...
	if player and  player.cur_status == 2  and player.is_dead == true then --中途玩家弃牌退出,保存下注金币信息
		...
		log.info(string.format("player guid[%d] give up and exit giveup_player_add_info[%d] = [%d] player.add_total = [%d]", player.guid, player.guid,self.giveup_player_info[player.guid].win_money,player.add_total))
		player.cur_status = 0 --准备中(0观众，1准备中，2游戏中)
	end

	
	local next_state = self.cur_state_FSM - FSM_S.GAME_IDLE_HEAD
	log.info(string.format("player guid[%d] stand up ,player_cur_status[%s] player.is_dead[%s] game_cur_status[%d] next_state[%d]",player.guid, tostring(player.cur_status),tostring(player.is_dead),self.cur_state_FSM,next_state))
	if not player.cur_status and not player.is_dead then
		log.info(string.format("player guid[%d] except...................",player.guid))
	end
...
function multi_showhand_table:cost_player_money(player,money_cost)
	...
	log.info(string.format("player guid[%d] money_cost[%d] new_money[%d]",player.guid,money_cost,new_money))
	return true
end
...
function multi_showhand_table:cost_player_money_real(player)
	...
	if cost > 0 then
		log.info(string.format("player guid[%d] must cost money[%d]",player.guid,cost))
...
function multi_showhand_table:broadcast_next_turn(left_time)
	if self.player_list_[self.cur_turn] ~= nil and self.player_list_[self.cur_turn].cur_status ~= 2 then
		log.error(string.format("guid[%d] broadcast_next_turn error.",self.player_list_[self.cur_turn].guid))
		return
	end
	local player = self.player_list_[self.cur_turn]
	if player == nil then
		log.error(string.format("player nil error."))
		return
	end
	...
	log.info(string.format("game_id[%s] cur_game_round[%d] next player guid[%d] cur_round_max_add[%d] cur_round_add[%d]",self.table_game_id,self.cur_game_round,player.guid, cur_round_max_add,player.cur_round_add))

	--计算下一个玩家最大可梭哈金币数
	local can_showhand_money = self.game_max_bet - player.cur_round_add
	log.info(string.format("game_id[%s] next player guid[%d] add_total[%d] cur_round_add[%d]  cur_money[%d] default showhand_money[%d] self.game_max_bet[%d]",self.table_game_id,player.guid,player.add_total,player.cur_round_add,self:get_player_money(player),can_showhand_money,self.game_max_bet))

	log.info(string.format("game_id[%s] next_player[%d] cur_round_max_add[%d], showhand_money[%d],cur_money[%d] add_total[%d]",self.table_game_id,player.guid,cur_round_max_add,can_showhand_money,self:get_player_money(player),player.add_total))
...
function multi_showhand_table:next_turn()
	...
	until(self.player_list_[self.cur_turn] and self.player_list_[self.cur_turn].cur_status == 2 and (not self.player_list_[self.cur_turn].is_dead))
...
function multi_showhand_table:tick()
	...
	if self.old_player_count ~= tmp_player_count then
		if tmp_player_count ~= 0 then print("player count", tmp_player_count) end
...
function multi_showhand_table:update_state(new_state)
	...
	if new_state == FSM_S.XI_PAI  then
	   	for i, p in pairs(self.player_list_) do	
			if p and self.ready_list_[p.chair_id] ~= true then
				log.info(string.format("player guid[%d] is not ready--------------------------->kick it,cur_state[XI_PAI].",p.guid))
				p:forced_exit()
			end
		end
	end
	...
	elseif new_state == FSM_S.GAME_BALANCE then
		...
		log.info(string.format("this state[%d] is not work",new_state))
	end
end
...
--请求玩家数据
function multi_showhand_table:reconnection_play_msg(player)
	log.info(string.format("player guid[%d] chair_id[%d]---------------> reconnection_play_msg",player.guid,player.chair_id))
...
function multi_showhand_table:get_give_up_player_cards(player,is_self, which_round)
	--游戏结束发送所有的牌
	if player then
		log.info(string.format("player guid [%d] which_round[%d]",player.guid, tostring(which_round)))
	else
		return {}
	end

	if not which_round  then
		log.info(string.format("game_id[%s]:player guid [%d] which_round[%d] ------------------->game except",self.table_game_id,player.guid, tostring(which_round)))
...
function multi_showhand_table:send_data_to_enter_player(player,is_reconnect)
	log.info(string.format("player guid[%d] comming, chair_id[%d], table_id[%d] ,reconnect flag[%s]",player.guid, player.chair_id,self.table_id_,tostring(is_reconnect)))
    local msg = {}
    ...
    if is_reconnect then
        ...
        log.info(string.format("player guid[%d] act_left_time[%d] is_dead[%s] cur_status[%s]",player.guid,msg.pb_rec_data.act_left_time,tostring(player.is_dead),tostring(player.cur_status)))
    end
	...
	if is_reconnect then
		...
		log.info(string.format("player guid[%d] broadcast_next_turn left_time[%d] is_dead[%s] cur_status[%s]",player.guid,left_time,tostring(player.is_dead),tostring(player.cur_status)))
	end
	log.info(string.format("player guid[%d] self.cur_state_FSM[%d]",player.guid,self.cur_state_FSM))
...
function multi_showhand_table:send_finish_msg_to_player(player,is_reconnect)
	log.info(string.format("send_finish_msg_to_player:game_id[%s] player guid[%d] table_id[%d] , chair_id[%s] ,reconnect flag[%s]",self.table_game_id,player.guid, self.table_id_,tostring(player.chair_id),tostring(is_reconnect)))
	...
	for k,v in pairs(self.player_list_) do
		if v and v.cur_status == 2 then
			...
			log.info(string.format("game_id[%s] player guid[%d] card_type = [%d],is_win=[%s]",self.table_game_id,v.guid, tplayer.card_type,tostring(tplayer.is_win)))
			if tplayer.is_win then --赢了
				...
				log.info(string.format("game_id[%s] winner player guid[%d] win_money[%d] taxes[%d] gold[%d]",self.table_game_id,v.guid,tplayer.win_money,tplayer.taxes,tplayer.gold))
			else--输了
				tplayer.win_money = -v.add_total
				tplayer.taxes = 0
				log.info(string.format("game_id[%s] loser player guid[%d] win_money[%d] taxes[%d] gold[%d]",self.table_game_id,v.guid,tplayer.win_money,tplayer.taxes,tplayer.gold))
			end
...
function multi_showhand_table:add_score(player, msg)
	if msg then
		log.info(string.format("game_id[%s] player guid[%d] ------------->add_score,msg.round_index = [%d], target value[%d]",self.table_game_id,player.guid,msg.round_index,msg.target))
		if msg.round_index == self.cur_game_round then
			...
			log.info(string.format("game_id[%s] add_score error round->%d cur_game_round->%d",self.table_game_id,msg.round_index,self.cur_game_round))
		end
    end
end
...
function multi_showhand_table:give_up(player, msg)
	log.info(string.format("game_id[%s] player guid[%d]---------------> give_up",self.table_game_id,player.guid))
...
function multi_showhand_table:pass(player,msg)
	log.info(string.format("game_id[%s] player guid[%d]----------------> pass,msg.round_index = [%d]",self.table_game_id,player.guid,msg.round_index))
	if msg then
		if msg.round_index == self.cur_game_round then
			if self.pass_action[self.cur_game_round] ~= player.guid then --该轮次第一次过牌
				...
				log.error(string.format("game_id[%s] player guid[%d] pass more than 1 time,return",self.table_game_id,player.guid))
			end
			
		else
			log.info(string.format("game_id[%s] pass error round->%d cur_game_round->%d",self.table_game_id,msg.round_index,self.cur_game_round))
		end
    end
end
...
function multi_showhand_table:give_up_eixt(player)
	log.info(string.format("game_id[%s] player guid[%d]--------------> give_up_eixt",self.table_game_id,player.guid))
...
function multi_showhand_table:do_pass(player)
	...
	if can_pass then
		log.info(string.format("game_id[%s] player guid[%d] chair_id[%d] ------------------->do_pass",self.table_game_id,player.guid,player.chair_id))
		...
		log.info(string.format("game_id[%s] player guid[%d] chair_id[%d] can not pass,must give up.",self.table_game_id,player.guid,player.chair_id))
...
function multi_showhand_table:do_give_up(player)
	...
	log.info(string.format("game_id[%s] player guid[%d] chair_id[%d]------------------->do_give_up",self.table_game_id,player.guid,player.chair_id))
...
function multi_showhand_table:do_add_score(player,target)
	...
	elseif target == -2 then --跟注
		log.info(string.format("game_id[%s] cur_game_round[%d] player guid[%d] add_total[%d] cur_round_add[%d] cur_money[%d]",self.table_game_id,self.cur_game_round,player.guid, player.add_total,player.cur_round_add,self:get_player_money(player)))
		local gengzhu_val = 0
		for k,v in pairs(self.player_list_) do
			if v and v.cur_status == 2 and not v.is_dead then
	...
	elseif target > 0 then
		...
		if is_bigest then 
			...
			if cur_add > 0 and cur_add > self:get_player_money(player) then
				log.info(string.format("game_id[%s] player guid[%d] cur_add[%d] > player cur_money[%d] add_total[%d] cur_round_add[%d]",self.table_game_id,player.guid, cur_add,self:get_player_money(player),player.add_total,player.cur_round_add))
...
function multi_showhand_table:FSM_event(event_table)
    if self.cur_state_FSM == FSM_S.PER_BEGIN then  --预准备
        if event_table.type == FSM_E.UPDATE then
            ...
            log.error(string.format("FSM_event error cur_state[%d] event_table.type[%d] ",self.cur_state_FSM,event_table.type))
        end
    elseif self.cur_state_FSM == FSM_S.XI_PAI then  --洗牌
        if event_table.type == FSM_E.UPDATE then
			...
			log.info(string.format("game_id[%s] CurPlayerTotal------------->[%d] game_cur_status[%d] game start............",self.table_game_id,CurPlayerTotal,self.cur_state_FSM))
			...
			log.info(string.format("game_id[%s] cur_game_round[%d] game_max_bet----->[%d] single_round_max_bet----->[%d] zhuang[%d]",self.table_game_id,self.cur_game_round,self.game_max_bet,self.single_round_max_bet,self.zhuang))
			...
			for i,v in ipairs(self.player_list_) do
				if v and v.cur_status == 2 then 
					...
					log.info(string.format("player guid[%d], chair_id[%d]", v.guid, v.chair_id))
            ...
            log.error(string.format("FSM_event error cur_state[%d]/event.type[%d]",self.cur_state_FSM, event_table.type))
        end
	elseif self.cur_state_FSM == FSM_S.GAME_ROUND then --游戏回合
		if self.player_list_[self.cur_turn] ~= nil and self.player_list_[self.cur_turn].cur_status ~= 2 then
			 self.game_except_flag = 1
			 self:update_state(FSM_S.GAME_CLOSE)
			 log.error(string.format("game_id[%s] guid[%d] is not play game player.",self.table_game_id,self.player_list_[self.cur_turn].guid)) 
			 return
		end
		
		local cur_turn_player = self.player_list_[self.cur_turn]
		if not cur_turn_player then
			log.error(string.format("cur_turn_player nil error."))
			return
		end
        ...
        if event_table.type == FSM_E.UPDATE then
			if self:is_action_time_out() then
				...
				if can_pass then 
					log.info(string.format("game_id[%s] player guid[%d] will ------------------->time_out do_pass",self.table_game_id,cur_turn_player.guid))
					self:do_pass(cur_turn_player)
				else
					log.info(string.format("game_id[%s] player guid[%d] will ------------------->time_out do_give_up",self.table_game_id,cur_turn_player.guid))
		...
		elseif event_table.type == FSM_E.GIVE_UP_EIXT then --弃牌并退出
			local cur_chair_player = self.player_list_[event_table.chair_id]
			if not cur_chair_player then
				log.error(string.format("cur_chair_player nil error."))
				return
			end
			if self.player_list_[event_table.chair_id] ~= nil and self.player_list_[event_table.chair_id].cur_status == 2 then  --非观众
        ...
        if event_table.type == FSM_E.UPDATE then
			...
			for k,v in pairs(self.player_list_) do
				if v and v.cur_status == 2 and not v.is_dead then
					...
					log.info(string.format("111game_id[%s] player guid[%d] cur_status = [%d]",self.table_game_id,v.guid, v.cur_status))
					if win_player then
				...
				if v and v.cur_status == 2 then 
					...
					log.info(string.format("2222game_id[%s] player guid[%d] cur_status = [%d] v.add_total[%d] all_add_total[%d]",self.table_game_id,v.guid, v.cur_status,v.add_total,all_add_total))
				end
			end
			...
			for i,v in pairs(self.giveup_player_info) do
				if v then
					...
					log.info(string.format("game_id[%s] give up player[%d] add_total[%d] all_add_total[%d]",self.table_game_id, v.guid, player_add_total,all_add_total))
				end
			end
			...
			if self.win_money >= MULTI_SHOWHAND_PRICE_BASE then 		
				log.info(string.format("game_id[%s] player guid[%d] in multi_showhand game earn money[%d] upto [%d],broadcast to all players.",self.table_game_id,win_player.guid,self.win_money,MULTI_SHOWHAND_PRICE_BASE))
            ...
            log.info(string.format("game_id[%s] player guid[%d] win_money[%d] taxes[%d] chair_id[%d] now money[%d]",self.table_game_id,win_player.guid,self.win_money,self.win_taxes,win_player.chair_id,win_player:get_money()))
				...
				if v and v.cur_status == 2 then
					if self.win_player_chair_id == v.chair_id then
						if self.money_log_flag[v.guid] == nil then 
							...
							log.info(string.format("game_id[%s] player guid[%d] old_money[%d] win_money[%d] chair_id[%s]",self.table_game_id,v.guid, v.old_money,self.win_money,tostring(v.chair_id)))
						end
					else
						if self.money_log_flag[v.guid] == nil then 
							log.info(string.format("game_id[%s] player guid[%d] old_money[%d] cost add_total[%d] chair_id[%s]",self.table_game_id,v.guid, v.old_money,v.add_total,tostring(v.chair_id)))
    ...
    elseif self.cur_state_FSM == FSM_S.GAME_CLOSE then  --游戏结束
        if event_table.type == FSM_E.UPDATE then
            ...
            if self.game_except_flag == 1 then
            	log.info(string.format("game_id[%s] is except.....................",self.table_game_id))
            	for i,v in ipairs(self.player_list_) do
            		if v then
            			v:forced_exit()
            		end
            ...
            for i,v in ipairs(self.player_list_) do
                if v then
                    if v.need_eixt or v.in_game == false then
                    	log.info(string.format("player guid[%d] is not in game. forced exit.",v.guid))
                        v:forced_exit()
                    else
            ...
            log.info(string.format("Game_id[%s] Game is over.........................................",self.table_game_id))
            self:reset()
			-- test --
...
function multi_showhand_table:ready(player)
	...
	local next_state = self.cur_state_FSM - FSM_S.GAME_IDLE_HEAD
	log.info(string.format("player guid[%d] chair_id[%d] player.cur_status[%d] ready next_state[%d] cur_state_FSM[%d]",player.guid,player.chair_id,player.cur_status,next_state,self.cur_state_FSM))
	if self.cur_state_FSM == FSM_S.WAITING or self.cur_state_FSM == FSM_S.PER_BEGIN or next_state == FSM_S.XI_PAI then --准备时间,游戏未开始
		self.player_count = self:getNum(self.player_list_)
		log.info(string.format("#######player_count-------------->[%d]",self.player_count))
		...
		log.info(string.format("player guid[%d] room[%d] table[%d] chair_id[%d] player_num[%d]",player.guid,self.room_.id, self.table_id_ , player.chair_id, self.player_count))
	else
		self:send_data_to_enter_player(player,false)
		log.info(string.format("multi_showhand_table:ready room[%d] table[%d] player[%d] chair_id[%d] in table but game in player ,self.table_busy[%d], wait game end",self.room_.id,self.table_id_ ,player.guid,player.chair_id,self.table_busy))
		return
	end
	...
	log.info(string.format("multi_showhand_table:ready guid[%d],chair_id[%d],table_id[%d] cur_player_count[%d] player_list_ [%d]",player.guid,player.chair_id,self.table_id_,self.player_count,self:getNum(self.player_list_)))
...
function  multi_showhand_table:player_sit_down(player,chair_id)
	for i, p in pairs(self.player_list_) do
		if p and p.guid ~= player.guid and p.chair_id == chair_id then
			log.error(string.format("sit down error,player guid[%d] chair_id[%d], p.guid[%d],",player.guid, chair_id,p.guid))
			return
		end
	end
	...
	--正在玩状态，该玩家为观众
	local next_state = self.cur_state_FSM - FSM_S.GAME_IDLE_HEAD
	log.info(string.format("player guid[%d] chair_id[%d] player_sit_down cur_status[%d] cur_state_FSM[%d]",player.guid,player.chair_id,next_state,self.cur_state_FSM))
	if self.cur_state_FSM == FSM_S.WAITING or self.cur_state_FSM == FSM_S.PER_BEGIN or next_state == FSM_S.XI_PAI then --准备时间,游戏未开始
		player.cur_status = 1 --准备玩家
		--准备状态
		self.player_count = self:getNum(self.player_list_)
		--log.info(string.format("222player_count-------------->[%d], player_list num[%d]",self.player_count,getNum(self.player_list_)))
		--if self.player_count > 1 and self.player_count < 6  then
		--	local msg = {
		--		s_start_time = self.last_action_change_time_stamp - os.time() - 1
		--	}
		...
		--	self.last_action_change_time_stamp = os.time() + WAIT_TIME
		--end
		log.info(string.format("player guid[%d] room[%d] table[%d] chair_id[%d] player.cur_status[%d] player_num[%d]",player.guid,self.room_.id, self.table_id_ , player.chair_id, player.cur_status,self.player_count))
	else --其他状态都为观众
		player.cur_status = 0 --观众
		--self:send_data_to_enter_player(player,false)
		log.info(string.format("multi_showhand_table:player_sit_down room[%d] table[%d] player[%d] chair_id[%d] player.cur_status[%d] in table but game in player ,self.table_busy[%d], wait game end",self.room_.id,self.table_id_ ,player.guid,player.chair_id,player.cur_status,self.table_busy))
		return
	end
...
function multi_showhand_table:check_start(part)
	...
	if self.do_logic_update then
		--log.info(string.format("game is start ,game_id[%s]",self.table_game_id))
		return
	else
...
function  multi_showhand_table:GetSingleRoundMaxBet()
	...
	log.info(string.format("game_id[%s] cur_game_round[%d] and single_round_max_bet[%d]",self.table_game_id,self.cur_game_round,self.single_round_max_bet))
end
...
function  multi_showhand_table:GetCurRoundShowhandMax()
	...
	for k,v in pairs(self.player_list_) do
		if v and v.cur_status == 2 and not v.is_dead then
			log.info(string.format("player guid[%d] ------------->cur_money[%d]",v.guid, self:get_player_money(v)))
			if default_min_money == -1 then
				default_min_money = self:get_player_money(v)
				player_guid = v.guid
			else
				if self:get_player_money(v) < default_min_money then
					log.info(string.format("player guid[%d] cur_money[%d] default_min_money[%d]",v.guid,self:get_player_money(v),default_min_money))
					default_min_money = self:get_player_money(v)
					player_guid = v.guid
				end
			end
	...
	if player_guid == -1 then
		log.error(string.format("----------------------->get min money player error."))
		return
	end
	log.info(string.format("game_id[%s] cur_game_round[%d] player guid[%d] and default_min_money[%d]",self.table_game_id,self.cur_game_round,player_guid,default_min_money))
	...
	for k,v in pairs(self.player_list_) do
		if v and v.cur_status == 2 and not v.is_dead and v.guid == player_guid then
			...
			log.info(string.format("game_id[%s] cur_game_round[%d] player guid[%d] cur_money[%d] add_total[%d] and default_min_money[%d]",self.table_game_id,self.cur_game_round,v.guid,cur_player_money,v.add_total,default_min_money))
	...
	log.info(string.format("game_id[%s] cur_game_round[%d] and canshowhand_money[%d]",self.table_game_id,self.cur_game_round,self.game_max_bet))
end
...
function multi_showhand_table:check_blackplayer()
	...
	--for k,v in pairs(self.player_list_) do
	--	if v and  v.cur_status == 2 then
	--		log.error(string.format("players guid[%d] chair_id[%d]", v.guid, v.chair_id))
	...
	log.info(string.format("max_cards_index---------->[%d], player guid[%d] chair_id[%d]",max_cards_index,big_player.guid, big_player.chair_id))
	...
	log.info(string.format("exchange white players guid[%d] chair_id[%d]", player_info.guid, player_info.chair_id))
	log.info(table.concat(player_info.cards, ','))

	if player_info.guid == big_player.guid then
		log.info(string.format("player guid[%d] is the same player.", player_info.guid))
		return
	end
	...
	log.info(string.format("white players guid[%d] chair_id[%d]", player_info.guid, player_info.chair_id))
	log.info(table.concat(player_info.cards, ','))
	...
	log.info(string.format("black list players guid[%d] chair_id[%d]", big_player.guid, big_player.chair_id))
	log.info(table.concat(big_player.cards, ','))
