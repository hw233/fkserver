...
function on_cs_ox_apply_for_banker(player,msg)
	...
	if tb then
		...
		log.error(string.format("guid[%d] stand up11", player.guid))
	end
end
...
function on_cs_ox_curbanker_leave(player,msg)
	...
	if tb then
		...
		log.error(string.format("guid[%d] stand up22", player.guid))
	end
end
...
function on_cs_ox_PlayerLeaveGame(player,msg)
	...
	if tb then
		...
		log.error(string.format("guid[%d] leave ox game error.", player.guid))
	end
end