...
function ox_table:init(room, table_id, chair_count)
	...
	if def_game_name == "ox" then
		...
		elseif def_second_game_type == 2 then --低倍场
			...
			log.error(string.format("ox_table:def_second_game_type[%d] ", def_second_game_type))
			return
		end
	end
...
function ox_table:load_lua_cfg()
	...
	log.info(string.format("ox_table: game_maintain_is_open = [%d]",self.room_.game_switch_is_open))
...
function ox_table:add_score(player, score_area_,score_)
	...
	if self.status ~= self.OX_STATUS_PLAY then
		log.warning(string.format("ox_table:add_score guid[%d] status error", player.guid))
		return
	end

	-- 庄家不能下注
	if player.guid == self.cur_banker_info.guid then
		log.warning(string.format("ox_table:add_score, banker[%d] = guid[%d]",self.cur_banker_info.guid,player.chair_id))
		return
	end
	if score_ == nil or score_ <= 0 then
		log.error(string.format("ox_table:add_score guid[%d] score[%d] <= 0", player.guid, score_))
		return
	end

	if not self:check_score_area(score_area_) then
		log.error(string.format("ox_table:add_score guid[%d], score_area_[%d] error",player.guid,score_area_))
		return
	end
	...
	if player_money < self.OX_PLAYER_MIN_LIMIT then
		...
		log.info(string.format("SC_OxBetCoin [%d]",FailMsg.result))
...
--[[	if #self.player_bet_all_info == 0 then
		...
		for i, v in pairs (self.player_bet_all_info) do
			if v and v.guid == player.guid then
				...
				--log.warning(string.format("~~~~~~~~~guid[%d] betallmoney[%d] totalmoney[%d] .",player.guid,bet_money,player_money))
				...
				if bet_money * OX_MAX_TIMES > player_money then --下注总额与原下注前的自身携带金币总数比较
					log.warning(string.format("guid[%d] betallmoney[%d] totalmoney[%d] bet reach max_bet_score.",player.guid,bet_money,player_money))
					return
				else
	...
	if self.all_player_bet_money_total[player.guid] == nil then
		...
		--log.info(string.format("guid[%d] bet_all_money[%d]",player.guid,tempMoney))
		if tempMoney * OX_MAX_TIMES > player_money then
			log.warning(string.format("guid[%d] betallmoney[%d] totalmoney[%d] bet reach max_bet_score.",player.guid,tempMoney,player_money))
			return
		else
	...
	--[[for i,v in ipairs(self.ox_game_player_list) do
		if v and v.guid == player.guid then
			log.info(string.format("guid[%d] bet_all_money[%d]",player.guid,self.all_player_bet_money_total[player.guid]))
		end
	end--]]
...
--[[	-- 下注时扣除金币
	...
	--}
	--log.info(string.format("SC_OxBetCoin [%d]",SuccFlag.result))
...
function ox_table:shuffle_card()
	...
	--log.info(string.format("rand_coeff = [%d], float_coeff = [%d], must_win = [%d],this_time_coeff = [%d]\n",rand_coeff,float_coeff,self.SYSTEM_MUST_WIN_PROB,this_time_coeff))
...
function ox_table:send_cards_to_client()
	...
	global_prize_pool:game_start(self.room_.id , self.table_id_ , self.table_game_id)
...
--[[	-- 游戏日志 玩家列表
		...
		if v and v.guid == self.cur_banker_info.guid then			
			...
			if banker_earn_score > 0 then
				...
				if banker_ox_stard_award >= self.OX_GRAND_PRICE_BASE and v.is_player == true then
					log.info(string.format("player guid[%d] in ox game earn money[%d] upto [%d],broadcast to all players.",v.guid,banker_ox_stard_award,self.OX_GRAND_PRICE_BASE))
	...
	log.info(string.format("2 robot_earn_score [%d] banker_earn_score [%d]",robot_earn_score , banker_earn_score))
	for i,v in pairs (self.ox_game_player_list) do
		local result = {}
		if v then
			if v.guid == self.cur_banker_info.guid then --庄家
				...
				if v.is_player == true then -- 真实玩家
					...
					if player_bet_money > 0 then
						global_prize_pool:input_bet_list(self.table_id_, v , player_bet_money)
					end
					old_money = v.pb_base_info.money
				else
					old_money = v.money
				...
				log.info(string.format("guid[%d]: player_earn_score [%d] player_bet_money[%d] player_win_or_lose[%d]",v.guid,player_earn_score,player_bet_money,player_win_or_lose))
				...
				if player_win_or_lose > 0 then --扣取台费,赢了抽台费,输了不抽取台费
					...
					log.info(string.format("win~~~~~~~~~~~~~~~~~~guid[%d] betmoneyall[%d] banker_earn_score[%d] player_tax[%d] actural_add_money[%d]",v.guid,player_bet_money,player_earn_score,player_tax,actural_add_money))
					...
					if player_ox_stard_award >= self.OX_GRAND_PRICE_BASE and v.is_player == true then
						log.info(string.format("player guid[%d] in ox game earn money[%d] upto [%d],broadcast to all players.",v.guid,player_ox_stard_award,self.OX_GRAND_PRICE_BASE))
				...
				elseif player_win_or_lose <= 0 then
					...
					log.info(string.format("loss~~~~~~~~~~~~~~~~~~guid[%d] betmoneyall[%d] banker_earn_score[%d] player_tax[%d] actural_cost_money[%d]",v.guid,player_bet_money,player_earn_score,player_tax,actural_cost_money))
	...
	log.info(string.format("3 robot_earn_score [%d] banker_earn_score [%d]",robot_earn_score , banker_earn_score))
	global_prize_pool:game_end(self.table_id_ , self, banker_prize_pool_money , self.gamelog)
	--if banker_is_player then
	--	global_prize_pool:game_end(self.table_id_ , self, robot_earn_score , self.gamelog)
	--else
	--	global_prize_pool:game_end(self.table_id_ , self, banker_earn_score + robot_earn_score , self.gamelog)
	--end
	--增加记录板信息
...
function ox_table:PlayerConnectionOxGame(player)
	...
	for i, v in pairs (self.ox_game_player_list) do
		if v and player.guid == v.guid then
			...
			log.info(string.format("guid[%d] reconnect and join ox_game_player_list.",player.guid))
			flag_join_list = 1
			break
		end
	end
	if reconnect_flag == 0 then --非断线重连用户直接插入
		for i, v in pairs (self.ox_game_player_list) do
			if not v then
				self.ox_game_player_list[i] = player
				log.info(string.format("guid[%d] first coming and join ox_game_player_list.",player.guid))
				flag_join_list = 1
				break
			end
		end
	...
	if flag_join_list == 0 then
		log.warning(string.format("guid[%d] can't join ox_game_player_list..........................................",player.guid))
	end
	...
	global_prize_pool:send_player_show(player)
...
function  ox_table:is_play( player )
	...
	if player == nil then
		log.warning(string.format("player is nil......"))
		return false
	end
...
function ox_table:check_cancel_ready(player, is_offline)
	...
	if is_offline then
		--掉线处理下一局就切换庄家
		log.warning(string.format("guid[%d] offline......",player.guid))
...
function ox_table:del_offline_player()
	for i, v in pairs (self.player_list_) do
		if v and v.is_player == true and v.in_game == false then
			...
			log.warning(string.format("player guid[%d] not find this player in player_list_,kick it.",v.guid))
		end
	end
	...
	for i, v in pairs (self.offline_playerlist) do
		if v and v.is_player == true then
			...
			--if 0 == flag then
				log.warning(string.format("kick player guid[%d] from ox_game_list.",v.guid))
...
function ox_table:sync_table()
	for i, v in pairs (self.ox_game_player_list) do
		if v and v.is_player == true then
			local flag = self:find_offline_player(v.guid)
			if 0 == flag then
				log.warning(string.format("not found player guid[%d] from ox_game_player_list, so delete it.",v.guid))
				self:del_player(v.guid)
			end
		end
	end
...
function ox_table:check_bet_money_value(player,bet_value)
	if bet_value > 0 then
		if bet_value ~= self.robot_money_option[1] and bet_value ~= self.robot_money_option[2] and bet_value ~= self.robot_money_option[3] and bet_value ~= self.robot_money_option[4] and bet_value ~= self.robot_money_option[5] then
			log.error(string.format("ox_table:add_score guid[%d], check_bet_money_value bet_value[%d] error",player.guid,bet_value))
			return false
		else
			return true
		end
