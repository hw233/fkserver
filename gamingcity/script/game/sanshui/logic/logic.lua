...
function logic:balance_scores(player_cards)
    ...
    local function  caculate_scores(type_cards_ps)
		local cmp_ret_scores = table.elements_operation(type_cards_ps,{},function(last_res,p,k) 
			last_res[p.chair_id] = {}
    ...
    local function caculate_flag_and_scores(p_type_cards)
		...
		table.walk(cmp_scores,function(s_l,k_l) 
			local shoot_count = table.elements_operation(s_l,0,function(last_shoot_count,s_r,k_r)
				local win_dao_count = table.elements_operation(s_r.normal,0,function(last,i_s,i_k)  
	...
	local type_cards = table.elements_operation(player_cards,{},function(last_res,p,k)
		local p_type_cards = table.elements_operation(p.cards,{},function(last_cards,cds,k) 
