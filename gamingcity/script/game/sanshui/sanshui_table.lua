...
sanshui_table = class("sanshui_table",base_table)
...
--    log.warning(string.format("sanshui_table:load_lua_cfg,%s",self.room_.room_cfg))
...
--	player_cards[4] = {chair_id = 4,cards = {{56,58,29},{2,4,20,21,36},{22,53,9,40,41}}}
    ...
    local scores = table.elements_operation(results,{},function(last_scores,v,k)
        local score = table.elements_operation(v,0,function(last_p_s,v1,k_c)
			local s = table.elements_operation(v1.normal,0,function(last_dao_s,p_s,k_p) return last_dao_s + p_s.win + p_s.extra  end)
			if v1.shoot ~= 0 then  s = s * math.abs(v1.shoot) end
			if v1.qld ~= 0 then s = s * math.abs(v1.qld) end
			s = s + table.elements_operation(v1.special,0,function(last_dao_s,p_s,k_p) return last_dao_s + p_s.win + p_s.extra  end)
			return last_p_s + s
		end)
...
--	end
	...
	table.walk(results,function(v,k)
		normal_count = normal_count + table.elements_operation(v,0,function(last_normal_count,v1,k1) return last_normal_count + (#v1.normal > 0 and 1 or 0)  end) / 2
		special_count = special_count + table.elements_operation(v,0,function(last_special_count,v1,k1) return last_special_count + (#v1.special > 0 and 1 or 0) end) / 2
		shoot_count = shoot_count + table.elements_operation(v,0,function(last_shoot_count,v1,k1)  return last_shoot_count + (v1.shoot ~= 0 and 1 or 0) end) / 2
		qld_count = qld_count + table.elements_operation(v,0,function(last_qld_count,v1,k1)  return last_qld_count + (v1.qld ~= 0 and 1 or 0) end) / 2
	end)
...
function sanshui_table:bi_pai_accomplish(player,msg)
    ...
    table.walk(player_balances,function(val,chair_id)
        ...
        log.warning(string.format("player change money [%d] [%d] [%d] [%d]",player.guid,chair_id,diff_money,l_tax))
...
function sanshui_table:reconnection_play_msg(player)
...
--        end
        ...
        log.info(string.format("deal_cards,guid:%d,cards:%s",v.guid,cards_array_str(cards)))
        dump(cards)
    ...
    elseif self.status == TableStatus.WAIT_SELECT_CARDS then
        self:foreach(function(v) 
            if v.status == PlayerStatus.SELECT_CARDS and ((os.clock() - v.time_elaspe) >= select_cards_elasped_time  + 3 or v.trusteeship == true) then
	            local l = logic:new()
                log.info(string.format("tick,splite_cards,guid:%d,cards:%s",v.guid,cards_array_str(v.cards)))
                ...
                if #types == 0 then
                    ...
                    log.error(string.format("tick,splite_cards failed,guid:%d,cards:%s",v.guid,cards_array_str(v.cards)))
                else
                    dump(types[1])
                    if #types[1] == 3 then 
    ...
    elseif self.status == TableStatus.WAIT_BI_PAI then
        ...
        self:foreach(function(v) 
            if v.status == PlayerStatus.BI_PAI and 
                ...
                log.warning(string.format("wait bipai elasped:%d,bi_pai_accomplish",v.guid))
