...
require "game.shaibao.shaibao_table"

-- enum GAME_SERVER_RESULT
local GAME_SERVER_RESULT_SUCCESS = pb.enum("GAME_SERVER_RESULT", "GAME_SERVER_RESULT_SUCCESS")


shaibao_room = base_room

-- 初始化房间
function shaibao_room:init(tb, chair_count, ready_mode, room_lua_cfg)
	base_room.init(self, tb, chair_count,ready_mode,room_lua_cfg)

end

-- 创建桌子
function shaibao_room:create_table()
	return shaibao_table:new()
end

-- 坐下处理
function shaibao_room:on_sit_down(player)
...
-- 快速坐下
function shaibao_room:auto_sit_down(player)
...
-- 坐下
function shaibao_room:sit_down(player, table_id_, chair_id_)
	if player.disable == 1 then
...
-- 站起
function shaibao_room:stand_up(player)
	print "test shaibao stand up ....................."
