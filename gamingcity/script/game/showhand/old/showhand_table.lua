...
local SHOWHAND_CARD_TYPE_ZILCH = pb.enum("SHOWHAND_CARD_TYPE", "SHOWHAND_CARD_TYPE_ZILCH")
local SHOWHAND_CARD_TYPE_ONE_PAIRS = pb.enum("SHOWHAND_CARD_TYPE", "SHOWHAND_CARD_TYPE_ONE_PAIRS")
local SHOWHAND_CARD_TYPE_TWO_PAIRS = pb.enum("SHOWHAND_CARD_TYPE", "SHOWHAND_CARD_TYPE_TWO_PAIRS")
local SHOWHAND_CARD_TYPE_THREE = pb.enum("SHOWHAND_CARD_TYPE", "SHOWHAND_CARD_TYPE_THREE")
local SHOWHAND_CARD_TYPE_STRAIGHT = pb.enum("SHOWHAND_CARD_TYPE", "SHOWHAND_CARD_TYPE_STRAIGHT")
local SHOWHAND_CARD_TYPE_FLUSH = pb.enum("SHOWHAND_CARD_TYPE", "SHOWHAND_CARD_TYPE_FLUSH")
local SHOWHAND_CARD_TYPE_FULLHOUSE = pb.enum("SHOWHAND_CARD_TYPE", "SHOWHAND_CARD_TYPE_FULLHOUSE")
local SHOWHAND_CARD_TYPE_BOMB = pb.enum("SHOWHAND_CARD_TYPE", "SHOWHAND_CARD_TYPE_BOMB")
local SHOWHAND_CARD_TYPE_STRAIGHT_FLUSH = pb.enum("SHOWHAND_CARD_TYPE", "SHOWHAND_CARD_TYPE_STRAIGHT_FLUSH")

local SHOWHAND_ADD_SCORE_GIVEUP = pb.enum("SHOWHAND_ADD_SCORE_TYPE", "SHOWHAND_ADD_SCORE_GIVEUP")
local SHOWHAND_ADD_SCORE_NO = pb.enum("SHOWHAND_ADD_SCORE_TYPE", "SHOWHAND_ADD_SCORE_NO")
local SHOWHAND_ADD_SCORE_CALL = pb.enum("SHOWHAND_ADD_SCORE_TYPE", "SHOWHAND_ADD_SCORE_CALL")
local SHOWHAND_ADD_SCORE_ADD = pb.enum("SHOWHAND_ADD_SCORE_TYPE", "SHOWHAND_ADD_SCORE_ADD")
local SHOWHAND_ADD_SCORE_ALLIN = pb.enum("SHOWHAND_ADD_SCORE_TYPE", "SHOWHAND_ADD_SCORE_ALLIN")
...
function showhand_table:add_score(player, type_ ,score_)
	if self.status ~= SHOWHAND_STATUS_PLAY then
		log.warning(string.format("showhand_table:add_score guid[%d] status error", player.guid))
		return
	end

	if player.chair_id ~= self.cur_turn then
		log.warning(string.format("showhand_table:add_score guid[%d] turn[%d] error, cur[%d]", player.guid, player.chair_id, self.cur_turn))
		return
	end

	if self.is_dead_[player.chair_id] then
		log.error(string.format("showhand_table:add_score guid[%d] is dead", player.guid))
		return
	end
	...
	if type_ ~= SHOWHAND_ADD_SCORE_GIVEUP and  type_ ~= SHOWHAND_ADD_SCORE_NO and type_ ~= SHOWHAND_ADD_SCORE_CALL and type_ ~= SHOWHAND_ADD_SCORE_ADD and type_ ~= SHOWHAND_ADD_SCORE_ALLIN then
		log.warning(string.format("showhand_table:add_score guid[%d] type error", player.guid))
		return
	end
	...
	if type_ == SHOWHAND_ADD_SCORE_NO then
		if self.call_type ~= nil and self.call_type ~= SHOWHAND_ADD_SCORE_NO then
			log.warning(string.format("showhand_table:add_score guid[%d] cannot SHOWHAND_ADD_SCORE_NO lastType [%d]", player.guid , (self.call_type or 0)))
			return
		end
		score_ = 0
	end
	if type_ == SHOWHAND_ADD_SCORE_CALL then
		if self.call_type == nil or self.call_type == SHOWHAND_ADD_SCORE_NO then
			log.warning(string.format("showhand_table:add_score guid[%d] cannot SHOWHAND_ADD_SCORE_CALL lastType [%d]", player.guid , (self.call_type or 0)))
			return
		end
		score_ = self.last_score - self.player_score[player.chair_id]
		if score_ < 0 then
			-- 本身 下注 比记录最大注还大，这不可能
			log.error(string.format("guid[%d] SHOWHAND_ADD_SCORE_CALL error  last_score[%d] player_score[%d]", player.guid , score.last_score , self.player_score[player.chair_id]))
			return
		end
		if score_ > player:get_money() then
			log.warning(string.format("showhand_table:add_score guid[%d] SHOWHAND_ADD_SCORE_CALL faild player_money[%d] call_score[%d]", player.guid, player:get_money(), score_))
			return
		end
	end
	if type_ == SHOWHAND_ADD_SCORE_ADD then --加注 1 不能比之前低 2 不能钱不够 3 不能在ALLIN的时候加注 
		if self.call_type ~= nil and self.call_type == SHOWHAND_ADD_SCORE_ALLIN then
			log.warning(string.format("showhand_table:add_score guid[%d] cannot SHOWHAND_ADD_SCORE_ALLIN , It is ShowHand Time", player.guid ))
			return
		end
		if score_ < 0 then
			log.warning(string.format("showhand_table:add_score guid[%d] faild score_[%d] < 0", player.guid,score_))
			return
		end
		if score_ == 0 then
		...
		if score_ > player:get_money() then
			log.warning(string.format("showhand_table:add_score guid[%d] SHOWHAND_ADD_SCORE_ADD faild player_money[%d] call_score[%d]", player.guid, player:get_money(), score_))
			return
		end
	end
...
function showhand_table:give_up(player)
	if self.status ~= SHOWHAND_STATUS_PLAY then
		log.warning(string.format("showhand_table:give_up guid[%d] status error", player.guid))
		return
	end

	if player.chair_id ~= self.cur_turn then
		log.warning(string.format("showhand_table:give_up guid[%d] turn[%d] error, cur[%d]", player.guid, player.chair_id, self.cur_turn))
		return
	end
...
function  showhand_table:player_offline( player )
	-- body
...
--请求玩家数据
function showhand_table:reconnection_play_msg(player)
	-- body self.round_
