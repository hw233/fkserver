...
function slotma_table:select_line(player,msg)
	...
	for i,v in ipairs(msg.lines) do
		if self.slotma_line_list[v] ~= nil then
			...
			log.info(string.format("error line->%d",v))
...
function slotma_table:load_lua_cfg()
	log.info(string.format("slotma_table: game_maintain_is_open = [%d]",self.room_.game_switch_is_open))
...
function slotma_table:PlayerConnectionSlotmaGame(player)
	log.info(string.format("playerr[%d] coming slotma game",player.guid))
end

-- 玩家离开游戏
function slotma_table:playerLeaveSlotmaGame(player)
	log.info(string.format("player[%d]  leave slotma game",player.guid))
end
...
function slotma_table:RandomEffectiveTimes(user_random_count,lineNum,min,max)
	...
	log.info(string.format("RandomEffectiveTimes random_count->%d",random_count))
...
function slotma_table:slotma_start(player,msg)
	...
	if self.room_.game_switch_is_open == 1 then
		if player and player.vip ~= 100 then
			...
			log.warning(string.format("slotma_table: game_switch_is_open[%d],game will maintain ,exit server.",self.room_.game_switch_is_open))
			return
		end
	end
	--入房条件金钱是否足够
	if player:check_room_limit(self.room_:get_room_limit()) then
		log.warning(string.format("slotma_table:check_room_limit guid[%d] chairid[%d]", player.guid, player.chair_id))
		self:respones(player,SLOTMA_TYPE_NOMONEY)
		return
	end	

	log.info(string.format("player slotma_start---------> [%d]",player.guid))
	...
	log.info(string.format("lineNum-> %d",lineNum))

	--判断线数
	if lineNum < 1 or lineNum > 9 then
		log.info(string.format("error line-> %d",lineNum))
		self:respones(player,SLOTMA_TYPE_LINERROR)
		return
	end
	...
	if player:get_money() < cost then
		log.warning(string.format("slotma_table:select_line guid[%d] chairid[%d] error, Money shortage", player.guid, player.chair_id))
		self:respones(player,SLOTMA_TYPE_NOMONEY)
		return
	end	
	...
	if get_bonus_money == true then
		...
		log.info(string.format("guid[%d] bonus_money[%d] times[%d], cell_times_[%d]",player.guid, bonus_money, bonus_times,self.cell_times_))
		...
		if SLOTMA_BONUS_OPEN == 1 and sys_earn > 0 then
			...
			log.info(string.format("guid[%d] to_bonus_pool_money[%d]",player.guid,to_bonus_pool_money))
...
function slotma_table:reconnect(player)
	log.info(string.format("slotma_table:reconnect--------------------->guid[%d] charid[%d]",player.guid,player.chair_id))
end
