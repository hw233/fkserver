...
texas_room = base_room  

-- 初始化房间
function texas_room:init(tb, chair_count, ready_mode, room_lua_cfg)
...
function texas_room:change_table(player)
	...
	local tb = self:find_table_by_player(player)
	if tb then
		local newTable, newChair, newTbID = self:get_suitable_table(player,true)
		if newTable then
			--离开当前桌子
			...
			self:player_enter_room(player, 0)
