...
function on_cs_operation(player,msg)
    ...
    if not tb then 
        log.error(string.format("on_cs_operation error:not find table [%d] ",player.guid))
        return 
    end
...
function on_cs_get_player_infos(player,msg)
    local tb = room:find_table_by_player(player)
    if not tb then 
        log.error(string.format("on_cs_get_player_infos error:not find table [%d] ",player.guid))
        return 
    end
