...
function zhajinhua_table:init(room, table_id, chair_count)
	...
	if def_game_name == "zhajinhua" then
		...
		elseif def_second_game_type == 99 then
			...
			log.error(string.format("zhajinhua_table:init def_second_game_type[%d] ", def_second_game_type))
			return
		end
	end
	...
	if zhajinhua_room_score[6] ~= nil then
		...
		log.info(string.format("zhajinhua_table:basic_config_coeff[%d][%d][%d][%d][%d][%d][%d] ", self.basic_config_coeff[1],self.basic_config_coeff[2],self.basic_config_coeff[3],self.basic_config_coeff[4],self.basic_config_coeff[5],self.basic_config_coeff[6],self.basic_config_coeff[7]))
	...
	for k=1,10 do
		...
		for i = 1 ,100000 do
			...
			if flag == true then
				test_num = test_num + 1
				--log.info(string.format("[%d] check_spec_cards ok.",i))
			end
			if test_num == 100000 then
				log.info(string.format("~~~~~~~~~~~~~~times[%d]:[%d] check_spec_cards ok.",k,test_num))
			end
		end
	end
	--]]

	local str = string.format("del %s_%d_%d_players",def_game_name,def_first_game_type,def_second_game_type)
	log.info(str)
	redis_command(str)
	self:robot_init()
end
...
function zhajinhua_table:check_cancel_ready(player, is_offline)
	log.info(string.format("check_cancel_ready guid [%d] status [%d] is_offline[%s] isDead[%s]",player.guid and player.guid or 0,self.status,tostring(is_offline),tostring(self.is_dead_[player.chair_id])))
...
function zhajinhua_table:all_compare()
	log.info(string.format("game_id[%s]------->all compare cards",self.gamelog.table_game_id))
	local player = nil
	...
	for i = 1, self.player_count_, 1 do
		...
		log.info(string.format("compare_card:table_id[%d] player guid[%d]",self.table_id_,player.guid))
	...
	repeat
		...
		if old == self.cur_turn then
			...
			for i = 1, #self.ready_list_ do
				log.error(string.format("gameid[%s] self.ready_list_[i] :",self.gamelog.table_game_id, self.ready_list_[i] ))
			end

			for i = 1, #self.is_dead_ do
				log.error(string.format("gameid[%s] self.is_dead_[i] :",self.gamelog.table_game_id, self.is_dead_[i] ))
			end

			log.error(string.format("turn error gameid[%s] old is %d  #self.ready_list_ is %d" ,self.gamelog.table_game_id, old, #self.ready_list_))
			log.error(string.format("%s",debug.traceback()))
			log.error(string.format("turn error gameid[%s]",self.gamelog.table_game_id))
			return
		end
...
function zhajinhua_table:get_sit_down(player)
	log.info(string.format("table_id[%d]: player guid[%d]------------->get_sit_down",self.table_id_,player.guid))
...
function zhajinhua_table:reconnect(player)
	if self.gamelog and self.gamelog.table_game_id then
		log.info(string.format("zhajinhua_table:game_id[%s] reconnect---------->guid[%d],chair_id[%d]",self.gamelog.table_game_id,player.guid,player.chair_id))
	else
		log.info(string.format("zhajinhua_table:reconnect---------->guid[%d],chair_id[%d]",player.guid,player.chair_id))
	end
...
function zhajinhua_table:load_lua_cfg()
	...
	log.info(string.format("zhajinhua_table:game_switch_is_open ---------------------------->[%d]",self.room_.game_switch_is_open))
	...
	log.info(string.format("robot_switch = [%d] black_rate = [%d]", self.robot_switch,self.black_rate))
...
function zhajinhua_table:start(player_count)
	...
	local spec_cards = {}
	--log.info(string.format("zhajinhua_table:basic_config_coeff[%d][%d][%d][%d][%d][%d] ", self.basic_config_coeff[1],self.basic_config_coeff[2],self.basic_config_coeff[3],self.basic_config_coeff[4],self.basic_config_coeff[5],self.basic_config_coeff[6]))
	...
	for i,v in ipairs(self.player_list_) do
		if v then
			...
			log.info(string.format("game_id[%s]:player guid[%d] --------------> cards:[%d,%d,%d]",self.gamelog.table_game_id,v.guid,tonumber(cards[1]),tonumber(cards[2]),tonumber(cards[3])))
	...
	self:broadcast2client("SC_ZhaJinHuaStart", msg)
    print("cell_score:", cell_score, " self.max_score_:",self.max_score_)
	log.info(string.format("game start ID =%s   guid=%s   timeis:%s", self.gamelog.table_game_id, self.log_guid, os.date("%y%m%d%H%M%S")))
	self.time0_ = get_second_time()
end

-- 加注
function zhajinhua_table:add_score(player, score_)
	log.info(string.format("game_id[%s] player guid[%d]--------> add score[%d]",self.gamelog.table_game_id,player.guid,score_))
	...
	if (not self.add_score_[score_])then
		if(score_ ~= 1 ) then
			log.warning(string.format("zhajinhua_table:game_id[%s] add_score guid[%d] status error", self.gamelog.table_game_id,player.guid))
			return
		end
		--第一个全压的人
		if self.ball_begin == false then
			...
			if playernum == 2 then
				...
				--end
				--if player_money_temp2 > self:get_player_money(self.player_list_[otherplayer]) then
				--	player_money_temp2 = self:get_player_money(self.player_list_[otherplayer])
				--end

				--print(self:get_player_money(player), self:get_player_money(self.player_list_[otherplayer]))
				...
				if all_add_score > self:get_player_money(self.player_list_[otherplayer]) then
					all_add_score = self:get_player_money(self.player_list_[otherplayer])
				end
				...
				log.info(string.format("game_id [%s]: guid[%d] add_score self.max_score_[%d]:", self.gamelog.table_game_id,player.guid,self.max_score_))
			else
				log.warning(string.format("game_id[%s]: add_score guid[%d] status error",self.gamelog.table_game_id, player.guid))
				return
			end
		else
			score_ = self.max_score_
	...
	if self.ball_score_[player.chair_id] then
			log.warning(string.format("game_id[%s]: add_score guid[%d] status error  is all_score_  true", self.gamelog.table_game_id, player.guid))
			return
	end
	if self.status ~= ZHAJINHUA_STATUS_PLAY then
		log.warning(string.format("game_id[%s]: add_score guid[%d] status error", self.gamelog.table_game_id,player.guid))
	...
	if player.chair_id ~= self.cur_turn then
		log.warning(string.format("game_id[%s]:add_score guid[%d] turn[%d] error, cur[%d]",self.gamelog.table_game_id, player.guid, player.chair_id, self.cur_turn))
		return
	end

	if self.is_dead_[player.chair_id] then
		log.error(string.format("game_id[%s]:add_score guid[%d] is dead", self.gamelog.table_game_id,player.guid))
		return
	end

	if score_ < self.last_score and not b_all_score_ then
		log.error(string.format("game_id[%s]:add_score guid[%d] score[%d] < last[%d]",self.gamelog.table_game_id, player.guid, score_, self.last_score))
		return
	end

	local money_ = score_
	if money_ > self.max_score_ then
		log.error(string.format("game_id[%s]:add_score guid[%d] score[%d] > max[%d]",self.gamelog.table_game_id, player.guid, money_, self.max_score_))
		return
	end
	...
	log.info(string.format("game_id[%s]: player guid[%d]----->socre[%d],money[%d].",self.gamelog.table_game_id,player.guid,score_, money_))

	money_ = math.ceil(money_)

	if money_ <= 0 or self:get_player_money(player) < money_ then
		log.error(string.format("game_id[%s]:add_score guid[%d] money[%d] > player_money[%d]",self.gamelog.table_game_id, player.guid, money_, self:get_player_money(player)))
		return false
	end
	...
	if b_all_score_ == true then
		log.info(string.format("game_id[%s]:player guid[%d]--------->all score money score[%d]", self.gamelog.table_game_id,player.guid,score_))
...
function zhajinhua_table:give_up(player)
	log.info(string.format("game_id[%s]:player guid[%d]------> give_up", self.gamelog.table_game_id,player.guid))
	if self.status ~= ZHAJINHUA_STATUS_PLAY then
		log.warning(string.format("game_id[%s]:give_up guid[%d] status error",self.gamelog.table_game_id, player.guid))
		return
	end
	...
	if self.ball_begin and self.cur_turn ~= player.chair_id then
		log.error(string.format("game_id[%s]:give_up is ball_begin guid[%d] can not giveup charid [%d] cur_turn[%d]",self.gamelog.table_game_id, player.guid, player.charid , self.cur_turn))
	end
...
function zhajinhua_table:look_card(player)
	log.info(string.format("game_id[%s]:player guid[%d]-------------->look cards",self.gamelog.table_game_id,player.guid))
	if self.status ~= ZHAJINHUA_STATUS_PLAY then
		log.warning(string.format("game_id[%s]:look_card guid[%d] status error", self.gamelog.table_game_id,player.guid))
		return
	end
	...
	if player.chair_id ~= self.cur_turn then
		log.warning(string.format("game_id[%s]:look_card guid[%d] turn[%d] error, curturn[%d]", self.gamelog.table_game_id, player.guid, player.chair_id, self.cur_turn))
		return
	end
--	if self.is_look_card_[player.chair_id] then
--		log.error(string.format("zhajinhua_table:look_card guid[%d] has look", player.guid))
--		return
--	end

	--全压已经取了钱最少玩家的所有钱，不受看牌×2规则影响，所以可以看牌
	--if self.ball_begin and self:get_player_money(player) < (self.max_score_  * 2)  then
	--	log.error(string.format("zhajinhua_table:look_card guid[%d] player_money[%d] max_money[%d] ball_begin and player money error", player.guid,self:get_player_money(player),self.max_score_))
	--	return
	--end
...
function zhajinhua_table:compare_card(player, compare_chair_id, allcompare, nosendflag)
	log.info(string.format("game_id[%s]:player guid[%d] chair_id[%d]-------------->compare_chair_id[%d] COMPARE_CARD",self.gamelog.table_game_id,player.guid,player.chair_id,compare_chair_id))
	if self.status ~= ZHAJINHUA_STATUS_PLAY then
		log.warning(string.format("game_id[%s]:compare_card guid[%d] status error", self.gamelog.table_game_id,player.guid))
		return
	end

	if player.chair_id ~= self.cur_turn then
		log.warning(string.format("game_id[%s]:compare_card guid[%d] turn[%d] error, cur[%d]",self.gamelog.table_game_id, player.guid, player.chair_id, self.cur_turn))
		return
	end

 	local target = self.player_list_[compare_chair_id]
 	if not target then
		log.error(string.format("game_id[%s]:compare_card guid[%d] compare[%d] error",self.gamelog.table_game_id, player.guid, compare_chair_id))
 		return
 	end

	if self.is_dead_[player.chair_id] then
		log.error(string.format("game_id[%s]:compare_card error guid[%d] is dead",self.gamelog.table_game_id, player.guid))
		return
	end

	if self.is_dead_[compare_chair_id] then
		log.error(string.format("game_id[%s]:compare_card error guid[%d] is dead",self.gamelog.table_game_id, target.guid))
		return
	end
	...
	if not allcompare  then
		...
		log.info(string.format("game_id[%s]: player guid[%d] compare card must cost money[%d]",self.gamelog.table_game_id,player.guid,money_))
		if money_ > self:get_player_money(player) then
			--money_ = player:get_money()
			money_ = self:get_player_money(player)
			bRetAllCompare = true
			log.info(string.format("game_id[%s]: player guid[%d] do not have enough money[%d]",self.gamelog.table_game_id,player.guid,money_))
		end
		money_ = math.ceil(money_)
		log.info(string.format("game_id[%s]: player guid[%d] last_score is [%d] cost_money[%d]",self.gamelog.table_game_id,player.guid,self.last_score,money_))
	...
	if ret then
		log.info(string.format("game_id[%s]:player guid[%d],target guid[%d]---------->first is win.",self.gamelog.table_game_id,player.guid,target.guid))
	else
		log.info(string.format("game_id[%s]:player guid[%d],target guid[%d]---------->second is win.",self.gamelog.table_game_id,player.guid,target.guid))
	end
	...
	if ret then
		...
		loser = self.player_list_[compare_chair_id]
		winner = player
	else
		loser = player
		winner = self.player_list_[compare_chair_id]
	...
	log.info(string.format("game_id[%s]: player guid[%d] charid[%d] , target guid[%d] ,turn [%d] , otherplayer [%d] money[%d] win [%s]" ,self.gamelog.table_game_id,player.guid,compare.chair_id,target.guid,compare.turn,compare.otherplayer,compare.money,tostring(compare.win)))
	...
	if self:check_end("SC_ZhaJinHuaCompareCard", notify) then -- 结束
		...
		log.info(string.format("game_id[%s]:------------->This Game Is  Over!",self.gamelog.table_game_id))
		return true
	end
...
function zhajinhua_table:check_end(sendname, fmsg)
	...
	if win then
		log.info(string.format("game_id[%s]: check_end---->Game is Over !!",self.gamelog.table_game_id))
		...
		for i,v in pairs(self.gamer_player) do
			if v then
				...
				if i == win then
					...
					log.info(string.format("guid[%d] to_bonus_pool_money[%d]",v.player.guid,to_bonus_pool_money))
					...
					if bonus_money > 0 then
						...
						log.info(string.format("winner guid[%d] extra_prize[%d]",v.player.guid,bonus_money))
						--得到全额奖金才广播
					...
					log.info(string.format("game_id[%s]: guid[%d]------> tax[%d] add money[%d]",self.gamelog.table_game_id,v.player.guid,money_tax,money_temp))
					...
					log.info(string.format("game_id[%s]: guid[%d]------> bet money[%d] change money[%d] now money[%d]",self.gamelog.table_game_id,item.guid,item.bet_money,money_change,item.money))
					--跑马灯游戏公告
					...
					if bonus_money > 0 then
						...
						log.info(string.format("loser guid[%d] extra_prize[%d]",v.player.guid,bonus_money))
						--得到全额奖金才广播
						if bonus_money_full == bonus_money then
							local bonus_money_str = string.format("%.02f",bonus_money/100)
							if v.player.guid > 0 then
				...
				log.info(string.format("game_id[%s]:guid[%d]:chair_id = [%d] status[%d] bet_money = [%d] now_money = [%d], score = [%d] ip_area = [%s] get_bonus_money[%d] to_bonus_money[%d]",self.gamelog.table_game_id,item.guid,item.chair_id,item.status,item.bet_money,item.money,item.score,item.ip_area,bonus_money,to_bonus_pool_money))
...
function zhajinhua_table:clear_ready( ... )
	...
	log.info(string.format("game end ID =%s   guid=%s   timeis:%s", self.gamelog.table_game_id, self.log_guid, os.date("%y%m%d%H%M%S")))
	...
	for i = 1, self.chair_count  do
		...
		if self.player_list_[i] then
			...
			if self.player_list_[i].is_offline == true or self.player_list_[i].in_game == false or player.isTrusteeship then
...
function zhajinhua_table:compare_cards(first, second)
	log.info(string.format("game_id[%s]:------------->COMPARE_CARDS!",self.gamelog.table_game_id))
...
function zhajinhua_table:player_sit_down(player, chair_id_)
	...
	if self.status == ZHAJINHUA_STATUS_FREE or self.status == ZHAJINHUA_STATUS_READY then
		...
		if self.player_list_[chair_id_] then
			log.warning(string.format("zhajinhua_table:player_sit_down !ZHAJINHUA_STATUS_FREE guid[%d]", player.guid))
			return
		end
		player.table_id = self.table_id_
		player.chair_id = chair_id_
...
function zhajinhua_table:player_stand_up(player, is_offline)
	--扣除未扣的钱
	if self.gamelog and self.gamelog.table_game_id then
		log.info(string.format("game_id[%s]:player guid[%d]----------->stand up and cost money real.",self.gamelog.table_game_id,player.guid))
	else
		log.info(string.format("player guid[%d]----------->stand up and cost money real.",player.guid))
	end
...
function zhajinhua_table:cost_player_money(player,money_cost)
	...
	if self.gamelog and self.gamelog.table_game_id then
		log.info(string.format("game_id[%s]: player guid[%d] cur money[%d],money_cost[%d].",self.gamelog.table_game_id,player.guid,money,money_cost))
	else
		log.info(string.format("player guid[%d] cur money[%d],money_cost[%d].",player.guid,money,money_cost))
	end

	if money_cost <= 0 or money < money_cost then
		log.error(string.format("game_id[%s]:guid[%d] cost_player_money error.curmoney[%d], must cost money[%d]",self.gamelog.table_game_id,player.guid,money,money_cost))
		return false
	end
	...
	if self.gamelog and self.gamelog.table_game_id then
		log.info(string.format("game_id[%s]: player guid[%d] new_money[%d],money_cost[%d].",self.gamelog.table_game_id,player.guid,new_money,money_cost))
	else
		log.info(string.format("player guid[%d] new_money[%d],money_cost[%d].",player.guid,new_money,money_cost))
	end
...
function zhajinhua_table:cost_player_money_real(player)
	...
	if self.gamelog and self.gamelog.table_game_id then
		log.info(string.format("game_id[%s]: player guid[%d] must cost money[%d]",self.gamelog.table_game_id,player.guid,cost))
	else
...
function zhajinhua_table:tick()
	...
	if self.status == ZHAJINHUA_STATUS_PLAY then
		...
		if curtime - self.time0_ >= 17 then
			...
			if player then
				log.info(string.format("Time out give_up  guid[%d]",player.guid))
...
function zhajinhua_table:check_robot_leave()
	...
	if self.robot_switch == 1 then
		...
		for _,v in pairs(leave) do
			log.info(string.format("guid[%s] tableid[%s] chairid[%s]",tostring(v.guid),tostring(v.table_id),tostring(v.chair_id)))
			if v.table_id and v.chair_id then
				local notify = {
						table_id = v.table_id,
						chair_id = v.chair_id,
...
function zhajinhua_table:check_black_user()
	...
	-------------
	print("----------------------------------------------------------------------------------------------------------")
	print(self.player_list_[max_chair_id].guid,"===>",self.player_list_[swap_chair_id].guid)
	print("----------------------------------------------------------------------------------------------------------")
end