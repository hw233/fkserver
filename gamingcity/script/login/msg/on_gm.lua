...
local function on_MSG_DELETE(fd,asyncid,doc)
	...
	channel.publish("db.?","LD_DelMessage",{
		msg_type = doc.type,
		msg_id = doc.id,
		retid = fd,
		asyncid = asyncid,
...
local function on_feedback(fd,asyncid,doc)
	...
	local function update_feedback(fd,asyncid,doc)
		...
		channel.publish("gate."..tostring(gateid),"LG_FeedBackUpdate",{
			guid = guid,
			type = type,
...
local function on_newserver(fd,asyncid,doc)
	...
	channel.publish("gate.*","LG_AddNewGameServer",{
		ip = doc.ip,
		port = doc.port,
		retid = fd,
	})
...
local function on_edit_alipay(fd,asyncid,doc)
	...
	local function edit_alipay(fd,asyncid,doc)
		...
		channel.publish("db.?","LD_AlipayEdit",{
			guid = guid,
			alipay_name = alipay_name,
			alipay_name_y = alipay_name_y,
			alipay_account = alipay_account,
...
local function on_edit_bankcard(fd,asyncid,doc)
	...
	local function edit_bank_card(fd,asyncid,doc)
		...
		if gateid then
			local gameid = reddb.hget("player_online_gameid",guid)
			if gameid then
				channel.publish("game."..tostring(gameid),"LS_BankcardEdit",{
					guid = guid,
		...
		channel.publish("db.?","LD_BankcardEdit",{
			guid = guid,
			bank_card_name = bank_card_name,
			bank_card_num = bank_card_num,
			bank_name = bank_name,
...
local function on_freeze_account(fd,asyncid,doc)
	...
	create_do_Sql( keyid, "account", strsql, json.encode(stData), function( retCode, retData, strData )
		...
		if gateid then
			local gameid = reddb.hget("player_online_gameid",guid)
			if gameid then
				channel.publish("game."..tostring(gameid), stDataR )
			else
...
local function on_update_proxy(fd,asyncid,doc)
	...
	channel.publish("db.?","S_RequestProxyInfo",{
...
local function on_create_proxy_account(fd,asyncid,doc)
	...
	channel.publish("db.?","LD_CreateProxyAccount",{
		web_id = fd,
		guid = creater_guid,
...
local function on_update_promoter_msg(fd,asyncid,doc)
	...
	local gameid = reddb.hget("player_online_gameid",guid)
	if gameid then
		channel.publish("game."..tostring(gameid),"LS_Player_Identiy",{
			guid = guid,
			identity_type = type,
...
local function on_update_risk(fd,asyncid,doc)
	...
	local gameid = reddb.hget("player_online_gameid",guid)
	if gameid then
		channel.publish("game."..tostring(gameid),"LS_Update_Risk",{
			guid = guid,
			risk = risk,
		})
	end
...
local function on_update_sensior_promoter(fd,asyncid,doc)
	...
	local gameid = reddb.hget("player_online_gameid",guid)
	if gameid then
		channel.publish("game."..tostring(gameid),"LS_Player_SeniorPromoter",{
			guid = guid,
