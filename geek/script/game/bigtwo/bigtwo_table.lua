...
local bigtwo_table = base_table:new()
...
function bigtwo_table:load_lua_cfg()
	...
	log.info("bigtwo_table: game_maintain_is_open = [%d]",self.room_.game_switch_is_open)
	...
	if bigtwo_config then
		if bigtwo_config.black_rate then
			...
			log.info("#########black_rate:[%f]",self.black_rate)
		end
	else
...
function bigtwo_table:shuffle_cheat_cards( bad_num)
	...
	if ilen ~= (52 - (13 * bad_num)) then		
		log.error("bigtwo_cards:shuffle_cheat_cards black_num[%d] ilen[%d] cards[%s]  black1[%s] black2[%s]",bad_num, ilen,  table.concat(arr_last_Cards, ','), table.concat(bad_cards_list[1], ','), table.concat(bad_cards_list[2], ','))
	end
...
function bigtwo_table:out_card(player, cardslist, flag)
	...
	if self.status ~= BIGTWO_STATUS_PLAY then
		log.warning("bigtwo_table:out_card guid[%d] status error", player.guid)
		return
	end

	if player.chair_id ~= self.cur_turn then
		log.warning("bigtwo_table:out_card guid[%d] turn[%d] error, cur[%d]", player.guid, player.chair_id, self.cur_turn)
		return
	end
	...
	if  not cardslist or #cardslist > 5 or  #cardslist == 0 then
		log.error("bigtwo_table:out_card guid[%d] out cards[%s] error, has[%s]", player.guid, table.concat(cardslist, ','), table.concat(playercards.cards_, ','))
		return
	end
	...
	if not cardstype then
		log.error("bigtwo_table:out_card guid[%d] get_cards_type error, cards[%s]", player.guid, table.concat(cardslist, ','))
		return
	end	
...
function bigtwo_table:pass_card(player, flag)
	if self.status ~= BIGTWO_STATUS_PLAY then
		log.warning("bigtwo_table:pass_card guid[%d] status error", player.guid)
		return
	end

	if player.chair_id ~= self.cur_turn then
		log.warning("bigtwo_table:pass_card guid[%d] turn[%d] error, cur[%d]", player.guid, player.chair_id, self.cur_turn)
		return
	end

	if not self.last_out_cards then
		log.error("bigtwo_table:pass_card guid[%d] first turn", player.guid)
		return
	end
...
function  bigtwo_table:recoveryplayercard(player)
	...
	log.info("---------recoveryplayercard-----------1")
		...
		log.info("chairid[%d] cards[%s]",player.chair_id,table.concat( self.bigtwo_player_cards[player.chair_id].cards_, ", "))
...
function  bigtwo_table:finishgame(player)
	...
	for i,v in ipairs(self.player_list_) do
		if v then
			...
			log.error("========player_list_ [%d] is nil or false",i)
		end
	end
	...
	for i,v in ipairs(self.player_list_) do
		if v then
			...
			if math.abs(tmp) > v.money then
				is_proportion = true
				trigger[v.chair_id] = true
				print("change score:", tmp, v.money)
				if tmp > 0 then
					tmp = v.money
				else				
					tmp = -v.money
	...
	if is_proportion then
		...
		log.info("molecular[%d]  denominator[%d]",molecular, denominator)
	else
		molecular = 1			  
		denominator = 1        
	end
	...
	for i,v in ipairs(self.player_list_) do
		if v then
    		if (is_change_md and notify.pb_conclude[v.chair_id].score > 0 ) or (is_change_md == false and notify.pb_conclude[v.chair_id].score < 0)then
    			log.info("-----------------score run A chair_id[%d]  score[%d]", v.chair_id, notify.pb_conclude[v.chair_id].score)
				notify.pb_conclude[v.chair_id].score = math.floor(notify.pb_conclude[v.chair_id].score * molecular / denominator)
    			log.info("-----------------score run B chair_id[%d]  score[%d]", v.chair_id, notify.pb_conclude[v.chair_id].score)
	...
	log.info("molecular_var:[%d]   denominator_var:[%d] ",molecular_var, denominator_var)
	--补差值
	if is_proportion and molecular_var ~= denominator_var then
		...
		if chair_id_m ~= nil then
			log.info("supplement------------------A score[%d]  chair_id[%d] temp[%d]",notify.pb_conclude[chair_id_m].score, chair_id_m, temp )
			notify.pb_conclude[chair_id_m].score = notify.pb_conclude[chair_id_m].score + temp
			log.info("supplement------------------A score[%d]", notify.pb_conclude[chair_id_m].score)
		end
	end
		...
		if v then
			...
			if notify.pb_conclude[v.chair_id].score > 0 then
				...
				if bigtwo_player_stdard_award >= BIGTWO_GRAND_PRICE_BASE and v.is_player ~= false then
					log.info("player guid[%d] nickname[%s]in bigtwo game earn money[%d] upto [%d],broadcast to all players.",v.guid,v.nickname,bigtwo_player_stdard_award,BIGTWO_GRAND_PRICE_BASE)
	...
	for _,var in ipairs(notify.pb_conclude) do 
		log.info("score [%d] , cards[%s]",var.score,table.concat( var.cards, ", "))
	end	
...
function bigtwo_table:tick()
	...
	elseif self.status == BIGTWO_STATUS_PLAYOFFLINE  and self.private_vote_status ~= true then
		...
		-- 游戏结束 进行结算
			log.info("BIGTWO_TIME_WAIT_OFFLINE time out time0[%d] curtime[%d]",self.time0_ ,curtime)
