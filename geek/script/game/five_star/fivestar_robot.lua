...
function fivestar_robot:init(guid_, account_, nickname_)
...
function fivestar_robot:creat_robot(guid, money)
	local frobot = fivestar_robot:new()
	frobot:init(guid, "test_fivestar_robot", "system_fivestar_robot")
	frobot.money = money
	return frobot
end

function fivestar_robot:get_header_icon()
	-- body
	return self.header_icon
end

function fivestar_robot:get_money()
	-- body
	return self.money
end

-- 加钱
function fivestar_robot:add_money(price, opttype)
...
-- 花钱
function fivestar_robot:cost_money(price, opttype, bRet)
	local money = self.money
	local oldmoney = money
	local iRet = true
	for _, p in ipairs(price) do
...
function fivestar_robot:dispatch_msg(msgname, msg)
	-- body
end