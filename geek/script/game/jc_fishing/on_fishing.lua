...
send2client_pb_str = function(player_or_guid, msgid, msg_str)
	...
	if not player.online then
		log.info("game[send2client_pb] offline, guid:%d  msgid:%d",player.guid,msgid)
		return
	end
...
send2client_pb = function(player_or_guid, msgname, msg)
	...
	if type(player) ~= "table" then
		player = base_players[player_or_guid]
		if not player then
			log.warning("game[send2client_pb] not find player:[%d],return",player_or_guid)
			return
		end
	end
	...
	if not player.online then
		log.info("game[send2client_pb] offline, guid:%d  msg:%s",player.guid,msgname)
		return
	end
...
function update_player_fire_log(guid,multiple,cost)
    ...
    if os.clock() - last_print_time >= 3 then
        log.info("======current storage:%d   ",global_storage)
	...
	if not player then
		log.error("update_player_fire_log guid[%d] not find in game" ,guid)
		return
	end

    local tb = room:find_table_by_player(player)
	if not tb then
		log.error("update_player_fire_log tableID[%d] not find in game" , player.table_id)
		return
	end
...
function update_player_hit_log(guid,fish_type,connon_score)
	...
	local player = base_players[guid]	
	if not player then
		log.error("[update_player_catch_log] guid[%d] not find in game" ,guid)
		return
	end

    local tb = room:find_table_by_player(player)
	if not tb then
		log.error("[update_player_catch_log] tableID[%d] not find in game" , player.table_id)
		return
	end
...
function write_player_money(guid,change_money)
	...
	local player = base_players[guid]	
	if not player then
		log.error("[write_player_money] guid[%d] not find in game",guid)
		return
	end

    local tb = room:find_table_by_player(player)
	if not tb then
		log.error("[write_player_money] tableID[%d] not find in game" , player.table_id)
		return
	end
...
function on_cs_fishing_treasureend(player, msg)
	...
	if tb then
		...
		log.error("guid[%d] treasureend", player.guid)
	end
end
...
function on_cs_fishing_changecannonset(player, msg)
	...
	if tb then
		...
		log.error("guid[%d] changecannonset", player.guid)
	end
end
...
function on_cs_fishing_netcast(player, msg)
	...
	if tb then
		...
		log.error("guid[%d] netcast", player.guid)
	end
end
...
function on_cs_fishing_lockfish(player, msg)
	...
	if tb then
		...
		log.error("guid[%d] lockfish", player.guid)
	end
end
...
function on_cs_fishing_lockspecfish(player,msg)
	...
	if tb then
		...
		log.error("guid[%d] lockspecfish", player.guid)
	end
end
...
function on_cs_fishing_fire(player, msg)
	...
	if tb then
		...
		log.error("guid[%d] fire", player.guid)
	end
end
...
function on_cs_fishing_changecannon(player, msg)
	...
	if tb then
		...
		log.error("guid[%d] changecannon", player.guid)
	end
end
...
function on_cs_fishing_timesync(player, msg)
	...
	if tb then
		...
		log.error("guid[%d] timesync", player.guid)
	end
end
