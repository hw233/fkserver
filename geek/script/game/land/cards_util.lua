...
function cards_util.seek_great_than(kcards,ctype,cvalue,ccount)
	...
	if ctype == LAND_CARD_TYPE.SINGLE then
		for i = cvalue + 1,15 do
			if valuecards[i] then return LAND_CARD_TYPE.SINGLE,i end
		end
	end

	if ctype == LAND_CARD_TYPE.DOUBLE then
		for i = cvalue + 1,15 do
			if valuecards[i] and #valuecards[i] >= 2 then  return LAND_CARD_TYPE.DOUBLE,i end
		end
	end

	if ctype == LAND_CARD_TYPE.THREE then
		for i = cvalue + 1,15 do
			if valuecards[i] and #valuecards[i] >= 3 then  return LAND_CARD_TYPE.THREE,i end
		end
	end

	if ctype == LAND_CARD_TYPE.THREE_WITH_ONE then
		for i = cvalue + 1,15 do
			if valuecards[i] and #valuecards[i] >= 3    then return LAND_CARD_TYPE.THREE_WITH_ONE,i end
		end
	end

	if ctype == LAND_CARD_TYPE.THREE_WITH_TWO then
		for i = cvalue + 1,15 do
			if valuecards[i] and #valuecards[i] >= 3  then return LAND_CARD_TYPE.THREE_WITH_TWO,i end
		end
	end

	if ctype == LAND_CARD_TYPE.BOMB then
		for i = cvalue + 1,15 do
			if valuecards[i] and #valuecards[i] == 4  then return  LAND_CARD_TYPE.BOMB,i end
		end
	end

	if ctype == LAND_CARD_TYPE.FOUR_WITH_DOUBLE then
		for i = cvalue + 1,15 do
			if valuecards[i] and #valuecards[i] == 4 and table.sum(valuecards,function(cs,j) return j == i and 0 or #cs end) >= 2  then 
				return LAND_CARD_TYPE.FOUR_WITH_DOUBLE,i 
			end
		end
	end

	if ctype == LAND_CARD_TYPE.FOUR_WITH_THREE then
		for i = cvalue + 1,15 do
			if valuecards[i] and #valuecards[i] == 4  and table.sum(valuecards,function(cs,j) return j == i and 0 or #cs end) >= 3 then 
				return LAND_CARD_TYPE.FOUR_WITH_THREE ,i 
			end
		end
	end
	...
	if ctype == LAND_CARD_TYPE.SINGLE_LINE then
		local lian_count,first_value = continuity_cards_count(cvalue + 1,1)
		if lian_count >= ccount then
			return LAND_CARD_TYPE.SINGLE_LINE,first_value
		end
	end

	if ctype == LAND_CARD_TYPE.DOUBLE_LINE then
	...
	if ctype == LAND_CARD_TYPE.PLANE then
	...
	if ctype == LAND_CARD_TYPE.PLANE_WITH_ONE then
		local lian_count,first_value = continuity_cards_count(cvalue + 1,3)
	...
	if ctype == LAND_CARD_TYPE.PLANE_WITH_TWO then
		local lian_count,first_value = continuity_cards_count(cvalue + 1,3)
