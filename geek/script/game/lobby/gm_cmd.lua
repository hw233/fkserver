...
function gm_change_player_bank(web_id_,login_id_, guid_,banktype_,order_id_,money_)
	log.info("gm_change_player_bank: web_id[%d] login_id[%d] guid[%d] banktype[%d] order_id[%s] money[%d]",web_id_,login_id_, guid_,banktype_,tostring(order_id_),money_)
	...
	log.info("ret[%d] acturl_cost_money_[%d] reason_[%d]",ret,acturl_cost_money_,reason_)
	if tonumber(ret) == 1 then --success
		log.info("gm_change_player_bank: guid[%d] cost money[%d] success",guid_,money_)
	else --failed
...
function gm_set_slotma_rate(guid,count)
	...
	print ("old random_count-> is :",player.slotma_addition)
	player.slotma_addition = count
