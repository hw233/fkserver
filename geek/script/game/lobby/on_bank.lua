...
-- 转账
function on_cs_bank_transfer(msg,guid)
	local player = base_players[guid]
	if msg.account == player.account then
		log.error("on_cs_bank_transfer guid[%d] target = self", player.guid)
		return
	end

	if not player.enable_transfer then
		log.error("on_cs_bank_transfer enable_transfer=false guid[%d] target = self", player.guid)
		return
	end
	...
	if target then -- 在该服务器情况
		...
		statement_.bank_balance = target.bank
		send2db_pb("SD_SaveBankStatement", {
			pb_statement = statement_,
		})]]
	else -- 不在该服务器情况
...
function on_ls_bank_transfer_target(msg)
	...
	if not target then 
		log.warning("on_es_bank_transfer_target account[%s] not find in game", msg.target)
		return
	end
...
function on_ds_bank_transfer(msg)
	if msg.result == BANK_OPT_RESULT_SUCCESS then
		...
		local player = base_players[msg.guid]
		if not player then
			log.warning("on_ds_bank_transfer guid[%d] not find in game", msg.guid)
			return
		end
		
		player.bank = player.bank + msg.money
...
function on_CS_CheckBankTransferEnable(msg,guid)
	local player = base_players[guid]
	if msg.guid == player.guid then
		log.error("on_CS_CheckBankTransferEnable guid[%d] target = self", player.guid)
...
function on_DS_CheckBankTransferEnable(msg)
	...
	if not player then
		log.error("on_DS_CheckBankTransferEnable guid[%d] not find in game", msg.pay_guid)
		return
	end
...
function do_player_transfer(player,msg, remain_money)
	-- body
	log.info("do_player_transfer  begin----------------- player  guid[%d]  money[%d] ", msg.guid, msg.money )
	...
	local fmsg = {
		...
		bef_money = player.money,
		aft_money = player.money,
...
-- 玩家通过guid转账给代理商
function on_cs_bank_transfer_by_guid(msg,guid)
	local player = base_players[guid]
	if not player or msg.guid == player.guid then
		log.error("on_cs_bank_transfer_by_guid guid[%d] target = self", msg.guid)
		send2client_pb(player,"SC_BankTransfer", {result = 7})
		return
	end
 		...
 		if switch == false then
			...
			redis_cmd_query(string.format("GET cash_remain_money"), function (reply)
				if reply:is_string() then
					...
					do_player_transfer(msg,guid,
			 remain_money)
				else
					log.error("on_cs_bank_transfer_by_guid player[%d] get cash_remain_money error..............................",player.guid)
					send2client_pb(player,"SC_BankTransfer", {result = 7})
				end
			end)
		end
...
function on_DS_PlayerBankTransfer(msg)
	log.info ("on_DS_PlayerBankTransfer begin  guid[%d]  ret[%d]", msg.guid, msg.result)
	local player = base_players[msg.guid]	
	if player and  player.pb_base_info  then
		...
		log.warning("on_DS_PlayerBankTransfer..............................[%d] no find player", msg.guid)
	end
...
function on_ls_bank_transfer_by_guid(msg)
	local player = base_players[msg.guid]
	if not player then 
		log.warning("on_ls_bank_transfer_by_guid guid[%d] not find in game", msg.guid)
		return
	end
...
function on_ds_bank_transfer_by_guid(msg)
	local player = base_players[msg.guid]
	if not player then
		log.warning("on_ds_bank_transfer_by_guid guid[%d] not find in game", msg.guid)
		return
	end
...
-- 银行流水记录
function on_cs_bank_statement(msg,guid)
	local player = base_players[guid]
	if player.b_bank_statement then
		log.warning("on_cs_bank_statement guid[%d] repeated", player.guid)
		return
	end
...
function on_ds_bank_statement(msg)
	local player = base_players[msg.guid]
	if not player then
		log.warning("on_ds_bank_statement guid[%d] not find in game", msg.guid)
		return
	end
