local rule = require "game.maajan_hongzhong.base.rule"
...
function mj_util.get_actions(pai,mo_pai,in_pai,si_dui,qi_dui)
	local actions = {}
	...
	if in_pai and rule.is_hu(pai,in_pai,si_dui,qi_dui)  then
		actions[ACTION.HU] = {[in_pai] = true,}
	end

	if mo_pai and rule.is_hu(pai,nil,si_dui,qi_dui) then
...
function mj_util.get_actions_first_turn(pai,mo_pai,si_dui,qi_dui,qishou_4hz_kehu)
	local actions = {}
	local counts = pai.shou_pai
	local ty_num = counts[TY_VALUE] or 0
	for t,c in pairs(counts) do
	...
	-- 起手四红中可胡
	if qishou_4hz_kehu and ty_num == 4 then
		actions[ACTION.ZI_MO] = {[mo_pai] = true,}
	end
	log.info("========= get_actions_first_turn 11111")
	if not actions[ACTION.ZI_MO] then
		if rule.is_hu(pai,nil,si_dui,qi_dui) then
			actions[ACTION.ZI_MO] = {[mo_pai] = true,}
		end
	end
	log.info("========= get_actions_first_turn ")
	return actions
end
function mj_util.get_actions_gang_huan_pai(pai,mo_pai,si_dui,qi_dui)
	local actions = {}
	local counts = pai.shou_pai
	local ty_num = counts[TY_VALUE] or 0
	for t,c in pairs(counts) do
...
function mj_util.is_hu(pai,in_pai,si_dui,qi_dui)
	return rule.is_hu(pai,in_pai,si_dui,qi_dui)
end

function mj_util.hu(pai,in_pai,mo_pai,si_dui,qi_dui)
	return rule.hu(pai,in_pai,mo_pai,si_dui,qi_dui)
end
...
function mj_util.is_ting(pai,si_dui,qi_dui)
	return rule.ting(pai,si_dui,qi_dui)
end

function mj_util.is_ting_full(pai,si_dui,qi_dui)
	return rule.ting_full(pai,si_dui,qi_dui)
end
