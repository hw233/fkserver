...
local mj_util 	= require "game.maajan_hongzhong.base.mang_jiang_util"
...
function maajan_table:on_started(player_count)
    ...
    self.bTest = false
    -- 鸟牌(码牌)
    self.niao_tiles = {}       -- { pai = nil,zhong = nil,}
    self.zhong_niao = 0
    self.niao_tiles_count = 0
    if self.rule.zhuaNiao then
        if self.rule.zhuaNiao.niao_option == 0 then  -- 一码全中
            self.niao_tiles_count = 1
        elseif self.rule.zhuaNiao.niao_option == 1 then  -- 159中码
    ...
    -- 是否选择了飘分，没有飘分就直接洗牌发牌
    if self.rule.piao and table.nums(self.rule.piao) ~= 0 then
        self:piao_fen()
    else
        self:pre_begin()
...
--     local counts = clone(player.pai.shou_pai)
--     for _,tile in pairs(tiles) do
--         local c = counts[tile]
--         if not c or c == 0 or tile == TY_VALUE then
--             send2client(player.guid,"SC_HuanPai",{
...
function maajan_table:on_piao_fen(player,msg)
    ...
    elseif self.rule.piao.piao_option == 1 then
        piao_fens = piao_private_conf.piao.piao_option_2
    elseif self.rule.piao.piao_option == 2 then
        piao_fens = piao_private_conf.piao.piao_option_3
    end
    local bcheck = false
    if piao ~= 0 then
        for _, value in pairs(piao_fens) do
            if value == piao then
                bcheck = true
                break
            end
...
function maajan_table:piao_fen()
    ...
    if trustee_type and (trustee_type == 1 or (trustee_type == 2 and self.cur_round > 1))  then
        local function auto_piao_fen(p)
            ...
            -- local min_men,c = table.min(men_count,function (count,men)
            --     return  men == mj_util.tile_men(TY_VALUE) and 14 or count
            -- end)
            local min_piao = 0
            log.info("%s",min_piao)
            self:on_piao_fen(p,{
...
function maajan_table:on_action_after_mo_pai(player,msg,auto)
    ...
    -- 是否有胡必胡玩法
    if action and self.rule.play.youhu_bihu then
        log.dump(self.waiting_actions,"waiting_actions")
        log.dump(action,"on_action_after_mo_pai")
        log.info("ZI_MO %d,action %d",ACTION.ZI_MO,msg.action)
        if action.actions[ACTION.ZI_MO] and msg.action ~= ACTION.ZI_MO then
            log.info("on_action_after_mo_pai pass youhu_bihu guid: %d",player.guid)
            return
        end
    end
    self:cancel_clock_timer()
...
function maajan_table:on_action_after_chu_pai(player,msg,auto)
    ...
    if action and self.rule.play.youhu_bihu then
        log.dump(self.waiting_actions,"waiting_actions")
        log.dump(action,"on_action_after_chu_pai")
        log.info("HU %d,action %d",ACTION.HU,msg.action)
        if action.actions[ACTION.HU] and msg.action ~= ACTION.HU then
            log.info("on_action_after_chu_pai pass youhu_bihu guid: %d",player.guid)
            return
        end
    end
...
function maajan_table:fake_mo_pai()
    ...
    if len == self.niao_tiles_count then
        self:do_balance()
        return
    end
...
function maajan_table:ting(p)
    -- if not self:is_piao(p) then return {} end

    local si_dui = self.rule.play and self.rule.play.si_dui
    local qi_dui = self.rule.play and self.rule.play.qidui_hu
    local ting_tiles = mj_util.is_ting(p.pai,si_dui,qi_dui) or {}
    -- if p.piao and ting_tiles then
    --     table.filter(ting_tiles,function(_,tile) return mj_util.tile_men(tile) ~= p.piao end)
    -- end

    return ting_tiles
end

-- function maajan_table:is_piao(p)
--     if not p.piao then return true end
...
--     local piao_fen_count = men_counts[p.piao] or 0
--     return piao_fen_count == 0
-- end
...
function maajan_table:ting_full(p)
    ...
    local si_dui = self.rule.play and self.rule.play.si_dui
    local qi_dui = self.rule.play and self.rule.play.qidui_hu
    local ting_tiles = mj_util.is_ting_full(p.pai,si_dui,qi_dui)
    -- if p.piao then
    --     ting_tiles = table.map(ting_tiles,function(tiles,discard)
    --         local hu_tiles = table.map(tiles,function(_,tile)
    --             return tile,mj_util.tile_men(tile) ~= p.piao and tile or nil
    --         end)
...
function maajan_table:on_action_chu_pai(player,msg,auto)
    ...
    if not mj_util.check_tile(chu_pai_val) or chu_pai_val ==  TY_VALUE then
    ...
    -- local piao_fen = player.piao
    -- if  piao_fen and
    --     table.Or(shou_pai,function(c,tile) return c > 0 and mj_util.tile_men(tile) == piao_fen end) and
    --     piao_fen ~= mj_util.tile_men(chu_pai_val)
    -- then
    --     log.error("piao exists and tile is not piao when chu guid:%s,tile:%s",player.guid,chu_pai_val)
    --     return
    -- end
...
function maajan_table:chu_pai()
    ...
    if trustee_type then
        local function auto_chu_pai(p)
            ...
            --     log.dump(men_tiles)
            --     if men_tiles[p.piao] and table.sum(men_tiles[p.piao]) > 0 then
            --         local c
            --         repeat
            --             chu_tile,c = table.choice(men_tiles[p.piao])
            --             log.info("%d,%d",chu_tile,c)
            --         until c and c > 0
            --     end
            -- end

            if not chu_tile or not p.pai.shou_pai[chu_tile] or p.pai.shou_pai[chu_tile] <= 0 or chu_tile==TY_VALUE then
                local c
                repeat
...
function maajan_table:zhuaNiao()
    -- 抓鸟牌(荒庄不抓鸟)
    local hu_count = table.sum(self.players,function(p) return p.hu and 1 or 0 end)
    if self.rule.zhuaNiao and hu_count >= 1 then
        local niao_pai,val
        if self.rule.zhuaNiao.niao_option == 0 then  -- 一码全中
            if self.bTest then
                if table.nums(self.pre_gong_tiles or {}) > 0 then
                    for i,tile in pairs(self.pre_gong_tiles) do
                        niao_pai = self.dealer:deal_one_on(function(t) return t == tile end)
                        self.pre_gong_tiles[i] = nil
                        break
                    end
                else
                    niao_pai = self.dealer:deal_one()
                end
            else 
                niao_pai = self.dealer:deal_one()
            end            
            val = niao_pai == 35 and 10 or mj_util.tile_value(niao_pai)
            self.zhong_niao = val
            local mapai = {
                pai = niao_pai,
                zhong = true,
            }
            tinsert(self.niao_tiles,mapai)
        elseif self.rule.zhuaNiao.niao_option == 1 then  -- 159中码
            for i = 1, self.rule.zhuaNiao.niao_count, 1 do
                if self.bTest then
                    if table.nums(self.pre_gong_tiles or {}) > 0 then
                        for i,tile in pairs(self.pre_gong_tiles) do
                            niao_pai = self.dealer:deal_one_on(function(t) return t == tile end)
                ...
                val = mj_util.tile_value(niao_pai)
                local mapai = {
                    pai = niao_pai,
                    zhong = false,
                }
                if val == 1 or val == 5 or val == 9 or niao_pai == 35 then
                    if self.rule.play.yima_yifen then
                        self.zhong_niao = self.zhong_niao + 1  -- 玩法选择 一码1分
                    else
                        self.zhong_niao = self.zhong_niao + 2  -- 中码，默认一码2分
                    end
                    mapai.zhong = true
                end
                tinsert(self.niao_tiles,mapai)
    ...
    log.dump(self.niao_tiles,"niao_tiles_zhong_"..self.zhong_niao)
end
...
function maajan_table:do_balance()
    ...
    for chair_id,p in pairs(self.players) do
        ...
        tinsert(msg.player_balance,{
            ...
            piao_score		= p.piao,	-- 选择的飘分
            hupiao_score	= p.hupiaoscore,	    -- 胡牌结算的飘分	
            gang_score		= p.gangscore,	-- 杠分
            zhongniao_score	= p.niaoscore,	-- 码分
            hu_score        = p.huscore,    -- 输家胡分
        })
    ...
    for _, mp in pairs(self.niao_tiles) do
        tinsert(msg.mapais,{
            pai = mp.pai,
            zhong = mp.zhong,
        })
    end
    log.dump(msg,"SC_MaajanHongZhongGameFinish")


    chair_money = self:balance(chair_money,enum.LOG_MOENY_OPT_TYPE_MAAJAN_HONGZHONG)
    ...
    self:broadcast2client("SC_MaajanHongZhongGameFinish",msg)
    ...
    self.game_log.niao_tiles = self.niao_tiles
...
function maajan_table:types_fan(ts)
    ...
    for _,t in pairs(ts) do
        ...
        if t.type == HU_TYPE.QIANG_GANG_HU then
            t.fan = zimo_fan
        end
...
function maajan_table:get_actions_first_turn(p,mo_pai)
    local si_dui = self.rule.play and self.rule.play.si_dui
    local qi_dui = self.rule.play and self.rule.play.qidui_hu
    local qishou_4hz_kehu = self.rule.play and self.rule.play.qishou_4hz_kehu
    local actions = mj_util.get_actions_first_turn(p.pai,mo_pai,si_dui,qi_dui,qishou_4hz_kehu)
    ...
    --     for act,tiles in pairs(actions) do
    --         for tile,_ in pairs(tiles) do
    --             if mj_util.tile_men(tile) == p.piao then
    --                 tiles[tile] = nil
    --             end
    --         end
    ...
    -- if not self:is_piao(p) and (actions[ACTION.HU] or actions[ACTION.ZI_MO]) then
...
function maajan_table:get_actions_gang_huan_pai(p,mo_pai)
    local si_dui = self.rule.play and self.rule.play.si_dui
    local qi_dui = self.rule.play and self.rule.play.qidui_hu
    local actions = mj_util.get_actions_gang_huan_pai(p.pai,mo_pai,si_dui,qi_dui)
...
function maajan_table:get_actions(p,mo_pai,in_pai,qiang_gang)
    local si_dui = self.rule.play and self.rule.play.si_dui
    local qi_dui = self.rule.play and self.rule.play.qidui_hu
    local actions = mj_util.get_actions(p.pai,mo_pai,in_pai,si_dui,qi_dui)
    ...
    if remain == self.niao_tiles_count and (actions[ACTION.AN_GANG] or actions[ACTION.MING_GANG] or actions[ACTION.BA_GANG] 
   ...
   -- 是否有胡必胡玩法
   if self.rule.play.youhu_bihu and (actions[ACTION.HU] or actions[ACTION.ZI_MO]) then
        actions[ACTION.PENG] = nil
        actions[ACTION.AN_GANG] = nil
        actions[ACTION.MING_GANG] = nil
        actions[ACTION.BA_GANG] = nil
...
function maajan_table:game_balance()
    local typefans,scores,piaoscors = {},{},{}
    self:foreach(function(p)
        local hu
        if p.hu then
    ...
    self:foreach(function(p)
        ...
        if p.hu then
            p.huscore = fan_score
            if not p.hu.zi_mo and not p.hu.qiang_gang then -- 点炮
                local whoee = p.hu.whoee
                local pi = self.players[whoee]
                -- 胡分
                ...
                -- 码分
                scores[whoee] = (scores[whoee] or 0) - zhongma_score
                scores[chair_id] = (scores[chair_id] or 0) + zhongma_score
                pi.niaoscore = (pi.niaoscore or 0) - zhongma_score
                p.niaoscore = (p.niaoscore or 0) + zhongma_score
                -- 飘分
                if self.rule.piao and table.nums(self.rule.piao) ~= 0 then                    
                    scores[whoee] = (scores[whoee] or 0) - (p.piao + pi.piao)
                    scores[chair_id] = (scores[chair_id] or 0) + (p.piao + pi.piao)
                    pi.hupiaoscore = (pi.hupiaoscore or 0) - (p.piao + pi.piao)
                    p.hupiaoscore = (p.hupiaoscore or 0) + (p.piao + pi.piao)
                end
                -- 庄闲
                for _, v in pairs(type) do
            ...
            if not p.hu.qiang_gang then -- 自摸胡 
                self:foreach_except(p,function(pi)
                    ...
                    -- 码分
                    scores[chair_i] = (scores[chair_i] or 0) - zhongma_score
                    scores[chair_id] = (scores[chair_id] or 0) + zhongma_score
                    pi.niaoscore = (pi.niaoscore or 0) - zhongma_score
                    p.niaoscore = (p.niaoscore or 0) + zhongma_score
                    -- 飘分
                    if self.rule.piao and table.nums(self.rule.piao) ~= 0 then
                        scores[chair_i] = (scores[chair_i] or 0) - (p.piao + pi.piao)
                        scores[chair_id] = (scores[chair_id] or 0) + (p.piao + pi.piao)
                        pi.hupiaoscore = (pi.hupiaoscore or 0) - (p.piao + pi.piao)
                        p.hupiaoscore = (p.hupiaoscore or 0) + (p.piao + pi.piao)
                    end
                ...
                self:foreach_except(p,function(pi)
                    ...
                    scores[whoee] = (scores[whoee] or 0) - fan_score
                    scores[chair_id] = (scores[chair_id] or 0) + fan_score
                    self.players[whoee].huscore = (self.players[whoee].huscore or 0) - fan_score
                    for _, v in pairs(type) do
                        if chair_id == self.zhuang then
                            if v.type == HU_TYPE.ZHUANG then
                                v.score = (v.score or 0) + 1
                    ...
                    scores[whoee] = (scores[whoee] or 0) - zhongma_score
                    scores[chair_id] = (scores[chair_id] or 0) + zhongma_score
                    self.players[whoee].niaoscore = (self.players[whoee].niaoscore or 0) - zhongma_score
                    p.niaoscore = (p.niaoscore or 0) + zhongma_score
                    -- 飘分
                    if self.rule.piao and table.nums(self.rule.piao) ~= 0 then
                        scores[whoee] = (scores[whoee] or 0) - (p.piao + pi.piao)
                        scores[chair_id] = (scores[chair_id] or 0) + (p.piao + pi.piao)
                        self.players[whoee].hupiaoscore = (self.players[whoee].hupiaoscore or 0) - (p.piao + pi.piao)
                        p.hupiaoscore = (p.hupiaoscore or 0) + (p.piao + pi.piao)
                    end
                    -- 庄闲
                    for _, v in pairs(type) do
...
function maajan_table:ext_hu(player,mo_pai,qiang_gang)
    ...
    local chu_pai_player = self:chu_pai_player()
    -- 红中麻将默认有天胡、对对碰、清一色玩法
    if not self.rule.play.tian_di_hu then
    ...
    -- 海底捞
    -- if self.dealer.remain_count == self.niao_tiles_count then
    ...
    local is_zi_mo,qiang_gang_hu = mo_pai and true or false,false
    ...
    -- end
    -- 自摸
    if is_zi_mo and self.rule.ziMoFen then -- self.rule.ziMoFen
        types[HU_TYPE.ZI_MO] = 1
    elseif qiang_gang and self.rule.play.qian_gang_hu then
        types[HU_TYPE.QIANG_GANG_HU] = 1
        qiang_gang_hu = true
    elseif self.rule.play.dian_pao_hu and not is_zi_mo and not qiang_gang then 
        types[HU_TYPE.DIAN_PAO] = 1
    end
    -- 庄闲
    if self.rule.play.zhuang_xian then
        if player.chair_id == self.zhuang or chu_pai_player.chair_id == self.zhuang or is_zi_mo then
            types[HU_TYPE.ZHUANG] = 1
        elseif qiang_gang_hu then
            if not self.players[self.zhuang].hu then
                types[HU_TYPE.ZHUANG] = 1
            end
        end
    end
...
function maajan_table:rule_hu_types(pai,in_pai,mo_pai)
    ...
    return table.series(hu_types,function(ones)
        ...
        for t,c in pairs(ones) do
            ...
            -- elseif t == HU_TYPE.QING_DA_DUI then -- and play_opt == "er_ren_yi_fang"
            ...
            bHave = true
            -- 七对、碰碰胡、天胡、清一色加一分
            if not self.rule.play.qd_pph_th_qys_jia1fen then
                if t == HU_TYPE.TIAN_HU then
                    ts[t] = nil
                    bHave = false
                end
                if t == HU_TYPE.DA_DUI_ZI then
                    ts[t] = nil
                    bHave = false
                end
                if t == HU_TYPE.QING_YI_SE then
                    ts[t] = nil
                    bHave = false
                end
                if t == HU_TYPE.QI_DUI then
                    ts[t] = nil
                    bHave = false
                end
            end
            if not self.rule.play.wuhongzhong_jia2fen then
                if t == HU_TYPE.WU_JI then
                    ts[t] = nil
                    bHave = false
                end
            end
...
function maajan_table:is_hu(pai,in_pai)
    local si_dui = self.rule.play and self.rule.play.si_dui
    local qi_dui = self.rule.play and self.rule.play.qidui_hu
    return mj_util.is_hu(pai,in_pai,si_dui,qi_dui)
end

function maajan_table:can_hu(player,in_pai,mo_pai,qiang_gang)
    -- 是否允许吃胡、抢杠胡
    local dian_pao_hu = self.rule.play.dian_pao_hu
    log.dump(self.rule.play,"can_hu rule_play")
    if qiang_gang then
        log.info("qiang_gang_hu 判断 %t",qiang_gang)
        local qian_gang_hu = self.rule.play.qian_gang_hu
        if not qian_gang_hu then
            log.info("qiang_gang_hu 判断 %t 不允许抢杠胡",qiang_gang)
            return false
        end
    else
        log.info("dian_pao_hu 判断 %t",dian_pao_hu)
        if not dian_pao_hu and in_pai then 
            log.info("dian_pao_hu 判断 %t 不允许点炮胡",dian_pao_hu)
            return false
        end
    end
