local rule = require "game.maajan_zigong.base.rule"
...
function mj_util.get_actions(pai,mo_pai,in_pai,si_dui,lai_zi)
	local actions = {}
	...
	if in_pai and rule.is_hu(pai,in_pai,si_dui,lai_zi)  then
		actions[ACTION.HU] = {[in_pai] = true,}
	end

	if mo_pai and rule.is_hu(pai,nil,si_dui,lai_zi) then
...
function mj_util.get_actions_first_turn(pai,mo_pai,si_dui)
	...
	local counts = pai.shou_pai
	local ty_num = counts[TY_VALUE] or 0
	log.dump(counts,"get_actions_first_turn_"..ty_num)
	for t,c in pairs(counts) do
		if t ~= TY_VALUE then 
			if c == 4 then
				actions[ACTION.AN_GANG] = actions[ACTION.AN_GANG] or {}
...
function mj_util.get_actions_gang_huan_pai(pai,mo_pai,si_dui,lai_zi)
	local actions = {}
	local counts = pai.shou_pai
	local ty_num = counts[TY_VALUE] or 0
	for t,c in pairs(counts) do
	...
	if rule.is_hu(pai,nil,si_dui,lai_zi) then
		actions[ACTION.ZI_MO] = {[mo_pai] = true,}
	end
	
	return actions
end
function mj_util.is_hu(pai,in_pai,si_dui,lai_zi)
	return rule.is_hu(pai,in_pai,si_dui,lai_zi)
end

function mj_util.hu(pai,in_pai,mo_pai,si_dui,lai_zi)
	return rule.hu(pai,in_pai,mo_pai,si_dui,lai_zi)
end
...
function mj_util.is_ting(pai,si_dui,lai_zi)
	return rule.ting(pai,si_dui,lai_zi)
end

function mj_util.is_ting_full(pai,si_dui,lai_zi)
	return rule.ting_full(pai,si_dui,lai_zi)
end
