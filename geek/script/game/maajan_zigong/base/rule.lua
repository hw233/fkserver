...
local TY_VALUE = 37
local maajian_is_hu = require "game.maajan_zigong.base.maajian_is_hu"
...
function rule.is_hu(pai,in_pai,with_si_dui,lai_zi)
	local cache = {}
	for i=1,37 do
...
local function is_ka_er_tiao(pai,in_pai)
	...
	table.decr(all_pai.shou_pai,21)
	table.decr(all_pai.shou_pai,22)
	table.decr(all_pai.shou_pai,23)
	local cache = {}
	for i = 1,37 do cache[i] = all_pai.shou_pai[i] or 0 end
	-- if in_pai then table.incr(cache,in_pai) end
	local state = {  counts = clone(cache) }
	if is_hu(state) then
		is_ka_er_tiao = true
	end
	log.dump(is_ka_er_tiao,"is_ka_er_tiao")
	return is_ka_er_tiao
end
...
function rule.hu(pai,in_pai,mo_pai,si_dui,lai_zi)
	...
	local qi_dui = is_qi_dui(cache)
	local si_dui = false -- is_si_dui(cache)
...
--未摸牌判听
function rule.ting(pai,si_dui,lai_zi)
	return rule.ting_tiles(pai,si_dui,lai_zi)
end

--全部牌判听
function rule.ting_full(pai,si_dui,lai_zi)
	local all_pai = clone(pai)
	local discard_then_ting_tiles = table.map(all_pai.shou_pai,function(c,tile)
		...
		local tiles = rule.ting_tiles(all_pai,si_dui,lai_zi)
