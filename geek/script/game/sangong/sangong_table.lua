...
local sangong_table = base_table:new()
...
function sangong_table:init(room, table_id, chair_count)
	...
	if def_first_game_type == 14 then
		...
		log.info("sangong_ox:-------->type[%d]:bet_times_option:[%d,%d,%d,%d]",def_second_game_type,bet_times_option[1],bet_times_option[2],bet_times_option[3],bet_times_option[4])
	end
	self:robot_init()
end
...
function sangong_table:load_lua_cfg()
	...
	log.info("sangong_table: game_maintain_is_open = [%d]",self.room_.game_switch_is_open)
	...
	if sangong_config then
		...
		if sangong_config.sangong_OX_GRAND_PRICE_BASE then
			...
			log.info("#########sangong_OX_GRAND_PRICE_BASE:[%d]",sangong_OX_GRAND_PRICE_BASE)
		end

		if sangong_config.BASIC_BET_TIMES then
			bet_times_option = sangong_config.BASIC_BET_TIMES
			log.info("######### sangong_ox:bet_times_option:[%d,%d,%d,%d]",bet_times_option[1],bet_times_option[2],bet_times_option[3],bet_times_option[4])
		end
...
function sangong_table:decide_sangong()
	...
	if getNum(self.b_contend_count) == 0 then
		...
		for _guid, b_player in pairs(self.b_player) do
			...
			self.game_log.sangong_contend[b_player.chair] = 1
			log.info("not find  player contend sangong.")
	...
	log.info("sangong_candidate is [%d]", getNum(sangong_candidate))
	local msg = {}
	...
	log.info("self.b_sangong.ratio = [%d],guid = [%d]", self.b_sangong.ratio,self.b_sangong.guid)
	...
	log.info("Cur sangong guid = [%d], ratio = [%d]",self.b_sangong.guid,self.b_sangong.ratio)
end
...
function sangong_table:send_result()
	...
	local sangong_player = self:get_player(self.b_sangong.chair)
	local sangong_old_money = sangong_player.money
	...
	for _guid, b_player in pairs(self.b_player) do
		...
		if _guid ~= self.b_sangong.guid and b_player.cards then
			...
			if win == true then
				...
				log.info("1111player[%d] betmoney = [%d],win_money_ = [%d] sangong_win_lose_money = [%d]",l_player.guid,b_player.bet,win_money_,sangong_win_lose_money)
			else
				...
				log.info("2222player[%d] betmoney = [%d],lose_money_ = [%d] sangong_win_lose_money = [%d]",l_player.guid,b_player.bet,lose_money_,sangong_win_lose_money)
			end
		end
	end

    log.info("sangong_lose_money [%d] sangong_win_money [%d] sangong_old_money [%d] sangong_win_lose_money[%d]", sangong_lose_money, sangong_win_money, sangong_old_money, sangong_win_lose_money)
	...
	for _guid, b_player in pairs(self.b_player) do
		if _guid ~= self.b_sangong.guid and b_player.cards then
			...
			if win == true then
				...
				log.info("3333player guid[%d] win_money = [%d] sangong_win_lose_money = [%d] sangong_old_money = [%d]",l_player.guid,win_money,sangong_win_lose_money,sangong_old_money)
				if sangong_old_money < 2 then
					...
					--  按比例获得 金币
						...
						log.info("4444sangong_ox:guid[%d] win_money [%d] sangong_old_money [%d] sangong_win_lose_money [%d]",_guid,win_money ,sangong_old_money ,sangong_win_lose_money)
					end
				end
				...
				log.info("55555player[%d] betmoney = [%d],win_money = [%d] sangong_win_lose_money = [%d]",l_player.guid,b_player.bet,win_money,sangong_win_lose_money)
				...
				if sangong_ox_player_stdard_award >= sangong_OX_GRAND_PRICE_BASE and l_player.is_player ~= false then
					log.info("player guid[%d] nickname[%s]in sangong ox game earn money[%d] upto [%d],broadcast to all players.",l_player.guid,l_player.nickname,sangong_ox_player_stdard_award,sangong_OX_GRAND_PRICE_BASE)
				...
				--  按比例获得 金币
					...
					log.info("66666sangong_ox:guid[%d] lose_money [%d] sangong_old_money [%d] sangong_win_lose_money [%d]",_guid,lose_money ,sangong_old_money ,sangong_win_lose_money)
				end
				...
				log.info("77777player[%d] betmoney = [%d],acturl lose_money = [%d] sangong_win_lose_money = [%d]",l_player.guid,b_player.bet,lose_money,sangong_win_lose_money)
				pb_player = {
					chair = b_player.chair,
					money = 0,
					tax = 0,
	...
	log.info("==================>b_pool = [%d] sangong_win_lose_money = [%d]",self.b_pool,sangong_win_lose_money)
	if self.b_pool > 0 and sangong_win_lose_money < 0 then
		log.info("88888b_pool = [%d] sangong_win_lose_money = [%d]",self.b_pool,sangong_win_lose_money)
		...
		log.info("99999player[%d] increment_money = [%d] tax = [%d]",l_player.guid,pb_sangong.increment_money,pb_sangong.tax)
		if pb_sangong.increment_money > s_old_money then
			log.info("00000guid[%d] sangong win_money [%d] s_old_money[%d]",l_player.guid,pb_sangong.increment_money ,s_old_money)
			pb_sangong.increment_money = s_old_money
		end
		log.info("aaaplayer[%d] increment_money = [%d] s_old_money = [%d]",l_player.guid,pb_sangong.increment_money,s_old_money)
		if l_player then
			l_player:add_money(
				{{ money_type = ITEM_PRICE_TYPE_GOLD, 
		...
		if sangong_ox_sangong_stdard_award >= sangong_OX_GRAND_PRICE_BASE and l_player.is_player ~= false then
			log.info("player guid[%d] nickname[%s]in sangong ox game earn money[%d] upto [%d],broadcast to all players.",l_player.guid,l_player.nickname,sangong_ox_sangong_stdard_award,sangong_OX_GRAND_PRICE_BASE)
	...
	log.info("record: sangong[%d] header_icon[%d] nickname[%s] cards_type[%d] money_change[%d]",sangong_record.guid,sangong_record.header_icon,sangong_record.nick_name,sangong_record.cards_type,sangong_record.money_change)
...
function sangong_table:sangong_bet(player, msg)
	...
	if self.b_sangong.guid == player.guid then
		log.warning("sangong guid[%d] can't bet money.",player.guid)
		return
	end

	
	local cur_player_money = player:get_money()
	log.info("player guid[%d] bet_money = [%d] bet_times = [%d] cur_player_money = [%d] ,sangong ratio = [%d]",player.guid,msg.bet_money,msg.bet_times,cur_player_money,self.b_sangong.ratio)
	...
	if msg.bet_times == player_cur_max_times then --玩家选择最低倍数始终可选不用判断
		...
		msg.bet_money = msg.bet_times * self.b_bottom_bet * self.b_sangong.ratio 
		log.info("player guid[%d] bet_money = [%d] acturl_bet_times = [%d] cur_player_money = [%d] ,sangong ratio = [%d], acturl_player_bet_max_times = [%d]",player.guid,msg.bet_money,msg.bet_times,cur_player_money,self.b_sangong.ratio,acturl_player_bet_max_times)
	end
...
function sangong_table:sangong_contend(player, ratio)
	if player and player.guid then
		log.info("sangong_table:sangong_contend ========== player [%d] ratio[%d]" , player.guid, ratio)
	end
	...
	if not self:isPlayer(player) then
		if player then
			log.info("sangong_contend player is not in game guid[%d]" , player.guid)
		else
			log.info("sangong_contend player is nil")
		end
		return
	end
	if ratio < 0 then
	...
	if self.b_player[player.guid].ratio ~= -1 then
		log.info("sangong_contend player ratio is not nil guid[%d] ratio[%d]" , player.guid , self.b_player[player.guid].ratio)
		return
	end
	...
	log.info("sangong_ox:sangong_contend ========== player [%d] ratio[%d] curMoney = [%d] cur_max_ratio = [%d]" , player.guid, ratio,player_curMoney,cur_max_ratio)
	if ratio > cur_max_ratio then
		ratio = cur_max_ratio
	end
	--抢庄倍数异常时，默认为1
	if ratio > 4 then
		log.warning("sangong_ox:player guid[%d] sangong contend expection ratio[%d] will be set ratio = 1" , player.guid, ratio)
		ratio = 1
	end
...
function sangong_table:player_sit_down(player, chair_id_)
	...
	for i,v in pairs(self.player_list_) do
		if v == player then
			log.info("GameInOutLog,sangong_table:player_sit_down return, guid %s,chair_id %s",tostring(player.guid),tostring(chair_id_))
...
function sangong_table:playerReady( player )
	...
	if player then
		...
		elseif self.b_table_busy == 1 and player.sangong_enterflag == true and self.b_status ~= STATUS_WAITING then
			if player.is_player ~= false then
				log.error("playerReady error player sangong_enterflag is true guid[%d]",player.guid)
			end
			return
		end
	else
	...
	log.info("sangong_table:playerReady room[%d] table[%d] chair[%d] player[%d]",self.room_.id, self.table_id_ , player.chair_id, player.guid)
