...
local shelongmen_table = class("shelongmen_table",base_table)
...
function shelongmen_table:init(room, table_id, chair_count)
	...
	timer_manager:new_timer(3,function()  --?????????????
		self:game_start() 
	end)
end
...
function shelongmen_table:on_bet(player,msg)
	...
	if self.status ~= TableStatus.WaitBet then
		log.warning("shelongmen_table:on_bet self.status ~= TableStatus.WaitBet guid[%d]",player.guid)
		error_num = 1
	end

	if player.standby then
		log.warning("shelongmen_table:on_bet standby player guid[%d]",player.guid)
		error_num = 2
	end

	if not table.logic_or(bet_chips,function(cell) return cell == msg.money end) then
		log.warning("shelongmen_table:on_bet bet amount not in bet_chips guid[%d] [%d]",player.guid,msg.money)
		error_num = 3
	end

	if not self:can_bet(player,msg.money,msg.area) then
		log.warning("shelongmen_table:on_bet no enought money guid[%d] total[%d] money[%d]",player.guid,player.bet.total,msg.money)
		error_num = 4
	end
	...
	log.info("shelongmen_table:on_bet guid[%d] [%d]",player.guid,player.bet.total + msg.money)
...
--	local cards_area = self:get_cards_area(cards_dipai,card_shechu)
	...
	if cards_area then 
		...
		log.error("shelongmen_table:wait_game_over deal cards error %s",self:get_now_game_id())
	end
