...
function showhand_table:add_score(player, type_ ,score_)
	if self.status ~= SHOWHAND_STATUS_PLAY then
		log.warning("showhand_table:add_score guid[%d] status error", player.guid)
		return
	end

	if player.chair_id ~= self.cur_turn then
		log.warning("showhand_table:add_score guid[%d] turn[%d] error, cur[%d]", player.guid, player.chair_id, self.cur_turn)
		return
	end

	if self.is_dead_[player.chair_id] then
		log.error("showhand_table:add_score guid[%d] is dead", player.guid)
		return
	end
	...
	if type_ ~= SHOWHAND_ADD_SCORE_GIVEUP and  type_ ~= SHOWHAND_ADD_SCORE_NO and type_ ~= SHOWHAND_ADD_SCORE_CALL and type_ ~= SHOWHAND_ADD_SCORE_ADD and type_ ~= SHOWHAND_ADD_SCORE_ALLIN then
		log.warning("showhand_table:add_score guid[%d] type error", player.guid)
		return
	end
	...
	if type_ == SHOWHAND_ADD_SCORE_NO then
		if self.call_type ~= nil and self.call_type ~= SHOWHAND_ADD_SCORE_NO then
			log.warning("showhand_table:add_score guid[%d] cannot SHOWHAND_ADD_SCORE_NO lastType [%d]", player.guid , (self.call_type or 0))
			return
		end
		score_ = 0
	end
	if type_ == SHOWHAND_ADD_SCORE_CALL then
		if self.call_type == nil or self.call_type == SHOWHAND_ADD_SCORE_NO then
			log.warning("showhand_table:add_score guid[%d] cannot SHOWHAND_ADD_SCORE_CALL lastType [%d]", player.guid , (self.call_type or 0))
			return
		end
		score_ = self.last_score - self.player_score[player.chair_id]
		if score_ < 0 then
			-- 本身 下注 比记录最大注还大，这不可能
			log.error("guid[%d] SHOWHAND_ADD_SCORE_CALL error  last_score[%d] player_score[%d]", player.guid , score.last_score , self.player_score[player.chair_id])
			return
		end
		if score_ > player:get_money() then
			log.warning("showhand_table:add_score guid[%d] SHOWHAND_ADD_SCORE_CALL faild player_money[%d] call_score[%d]", player.guid, player:get_money(), score_)
			return
		end
	end
	if type_ == SHOWHAND_ADD_SCORE_ADD then --加注 1 不能比之前低 2 不能钱不够 3 不能在ALLIN的时候加注 
		if self.call_type ~= nil and self.call_type == SHOWHAND_ADD_SCORE_ALLIN then
			log.warning("showhand_table:add_score guid[%d] cannot SHOWHAND_ADD_SCORE_ALLIN , It is ShowHand Time", player.guid )
			return
		end
		if score_ < 0 then
			log.warning("showhand_table:add_score guid[%d] faild score_[%d] < 0", player.guid,score_)
			return
		end
		if score_ == 0 then
		...
		if score_ > player:get_money() then
			log.warning("showhand_table:add_score guid[%d] SHOWHAND_ADD_SCORE_ADD faild player_money[%d] call_score[%d]", player.guid, player:get_money(), score_)
			return
		end
	end
...
function showhand_table:give_up(player)
	if self.status ~= SHOWHAND_STATUS_PLAY then
		log.warning("showhand_table:give_up guid[%d] status error", player.guid)
		return
	end

	if player.chair_id ~= self.cur_turn then
		log.warning("showhand_table:give_up guid[%d] turn[%d] error, cur[%d]", player.guid, player.chair_id, self.cur_turn)
		return
	end
