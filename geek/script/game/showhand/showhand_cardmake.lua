...
require "data.showhand_data"
local showhand_card_rate = showhand_card_rate

local random = require "random"

local showhand_cardmake = {}

local showhand_cards = {}
...
-- 存储原始cards ,cards必须有序
function showhand_cardmake.init_cards(cards)
	-- body
	showhand_cards = {}
	for i,v in ipairs(cards) do
		table.insert(showhand_cards,v)
	end
end

--转为二维数组便于计算 
function showhand_cardmake.create_my_cards()
	-- body
	local mycards = {}
	for i,v in ipairs(showhand_cards) do
		-- 得到牌花色1-4
		local color = (v-1) % 4 + 1
...
function showhand_cardmake.need_make_card()
	-- body
	--概率等于基础概率加上浮动概率
	local rate = showhand_card_rate.base_rate + random.boost_integer(0,showhand_card_rate.float_rate)
...
function showhand_cardmake.make_cards(player_count)
	
	local make_success = true
	local players_cards = {}
	local mycards = showhand_cardmake.create_my_cards()

	for i=1,player_count do
		local success,player_cards = showhand_cardmake.do_make_cards(mycards)
		if success then
			--换2张牌，混淆一下
			local x = math.random(#player_cards)
			local y = math.random(#player_cards)
			if x ~= y then
				player_cards[x], player_cards[y] = player_cards[y], player_cards[x]
			end

			for _,v in ipairs(player_cards) do
				if v then
					table.insert(players_cards,v)
				end
			end
		else
			make_success = false
...
function showhand_cardmake.set_card_rate(card_rate)
	-- body
	showhand_card_rate = card_rate
end

function showhand_cardmake.do_make_cards(mycards)

	local card_rates = {
	{rate = showhand_card_rate.rate_tonghua_shun, func = showhand_cardmake.make_tonghua_shun},
	{rate = showhand_card_rate.rate_four,         func = showhand_cardmake.make_four},
	{rate = showhand_card_rate.rate_hulu,         func = showhand_cardmake.make_hulu},
	{rate = showhand_card_rate.rate_tonghua,      func = showhand_cardmake.make_tonghua},
	{rate = showhand_card_rate.rate_shunzi,       func = showhand_cardmake.make_shunzi},
	{rate = showhand_card_rate.rate_three,        func = showhand_cardmake.make_three},
	{rate = showhand_card_rate.rate_double_two,   func = showhand_cardmake.make_double_two},
	{rate = showhand_card_rate.rate_double,       func = showhand_cardmake.make_double}
	}
...
function  showhand_cardmake.make_tonghua_shun(mycards)
	-- body
	local player_cards = {}
...
function  showhand_cardmake.make_four(mycards)
	-- body
	local player_cards = {}
...
function  showhand_cardmake.make_hulu(mycards)
	-- body
	local player_cards = {}
	--随机找两张牌
...
function  showhand_cardmake.make_tonghua(mycards)
	-- body
	local player_cards = {}
...
function  showhand_cardmake.make_shunzi(mycards)
	-- body
	local player_cards = {}
...
function  showhand_cardmake.make_three(mycards)
	-- body
	local player_cards = {}
	--随机找三张牌
	local color_three,card_index_three = get_random_card(mycards,1,#mycards[1])
...
function  showhand_cardmake.make_double_two(mycards)
	-- body
	local player_cards = {}
...
function  showhand_cardmake.make_double(mycards)
	-- body
	local player_cards = {}
	--随机找一张牌
