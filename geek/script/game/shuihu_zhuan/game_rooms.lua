...
local game_rooms = base_room:new()
...
function game_rooms:auto_sit_down(player)
    if not base_room.auto_sit_down(self, player) then
        return false
    end
...
function game_rooms:sit_down(player, table_id_, chair_id_)
    if not base_room.sit_down(self, player, table_id_, chair_id_) then
        return false
    end
...
function game_rooms:stand_up(player)
    ...
    if not base_room.stand_up(self, player) then
        return false
    end
    return true
end
