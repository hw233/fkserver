...
local texas_table = base_table:new()
...
function texas_table:reset()
	...
	self.t_min_bet = self.blind_small_bet		--read from config
	self.t_max_bet = 100000	                    --read from config
	self.t_bet = {}
	--self.t_cur_pot = 0
...
function texas_table:init(room, table_id, chair_count)
	...
	-- self.t_card_set = {
	-- 	--k 7 3 8 2   - 5 5   -3 9
	-- 	0x0D,0x07,0x23,0x08,0x32,0x15,0x05,0x25,0x35,0x0A,0x0B,0x0C,0x0D,	--方块 A - K
	-- 	0x11,0x12,0x13,0x14,0x15,0x16,0x17,0x18,0x19,0x1A,0x1B,0x1C,0x1D,	--梅花 A - K
	-- 	0x21,0x22,0x23,0x24,0x25,0x26,0x27,0x28,0x29,0x2A,0x2B,0x2C,0x2D,	--红桃 A - K
	-- 	0x31,0x32,0x33,0x34,0x35,0x36,0x37,0x38,0x39,0x3A,0x3B,0x3C,0x3D,	--黑桃 A - K
	-- }
...
function texas_table:show_down_and_award()	
	...
	for _guid, _final_money in pairs(self.t_final_pool) do
		if self.t_player[_guid] then
			...
			local l_player = self:get_player(l_chair)
			local l_tax = _final_money * self.t_tax
			if l_tax < 1 then
				l_tax = 0 
			else 
			...
			if l_player then
				msg.pb_user[l_chair].win_money = l_win_money
...
function texas_table:show_down_with_one_player(msg, t_side_pool_array)
	...
	--独享主池子
	local last_one_win_money = self.t_pot
	for _guid, t_player in pairs(self.t_player) do
		...
		if player and self.t_player[_guid] then
			...
			if self.t_player[_guid].status == PLAYER_STATUS_GAME or 
				...
				local l_tax = last_one_win_money * self.t_tax
				if l_tax < 1 then
					l_tax = 0 
				else 
					l_tax = math.floor(l_tax + 0.5)
				...
				pb_player.win_money = last_one_win_money - l_tax
				pb_player.tax = l_tax
...
function texas_table:player_sit_down(player, chair_id_)
	...
	log.info(string.format("GameInOutLog,texas_table:player_sit_down, guid %s, table_id %s, chair_id %s",
...
function texas_table:sit_on_chair(player, _chair_id)
	...
	local result_, table_id_, chair_id_ = base_room.sit_down(self, player, self.table_id_, _chair_id)
...
function texas_table:player_stand_up(player, is_offline)
	log.info(string.format("GameInOutLog,texas_table:player_stand_up, guid %s, table_id %s, chair_id %s, is_offline %s",
