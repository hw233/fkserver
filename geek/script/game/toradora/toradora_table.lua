...
local toradora_table = base_table:new()
...
function toradora_table:init(room, table_id, chair_count)
    ...
    self.android_list = {}
    self.android_guid = 0
    self.android_guid_title = string.format("%d%d%d000",def_game_id, self.room_.id, table_id)
...
function toradora_table:load_lua_cfg()
    ...
    if toradora_config then
        ...
        -- 显示玩家数量
        if toradora_config.show_playersmax then
            self.show_playersmax = toradora_config.show_playersmax
        end
        -- 必杀机率
        ...
        -- 显示幸运星玩家数量
        if toradora_config.show_luck_playersmax then
            self.show_luck_playersmax = toradora_config.show_luck_playersmax
        end
        -- 大赢家阀值
        ...
        self:log_error_msg("toradora_config is nil")
    end

    self:log_important(string.format([[
        TORADORA_TIME_START_COUNTDOWN [%d] TORADORA_TIME_BETTING_COUNTDOWN[%d] TORADORA_TIME_SETTLEMENT_CANNOT_EXIT_COUNTDOWN[%d] TORADORA_TIME_SETTLEMENT_CAN_EXIT_COUNTDOWN[%d] history_max[%d] bet_limit[%d] bet_all_max[%d] logLevel[%d] show_playersmax[%d]
        kill_Chance[%d] draw_Chance[%d] dragon_times[%f] tiger_times[%f] draw_times[%f] ,big_win_limit[%d] , grand_base[%d] ,show_luck_playersmax[%d] ,luck_count_num [%d]
        ...
        , self.TORADORA_TIME_START_COUNTDOWN , self.TORADORA_TIME_BETTING_COUNTDOWN , self.TORADORA_TIME_SETTLEMENT_CANNOT_EXIT_COUNTDOWN , self.TORADORA_TIME_SETTLEMENT_CAN_EXIT_COUNTDOWN, self.history_max , self.bet_limit , self.bet_all_max , self.logLevel , self.show_playersmax,
         self.kill_Chance , self.draw_Chance ,
         self.toradora_times[toradora_area_dragon] , self.toradora_times[toradora_area_tiger] , self.toradora_times[toradora_area_draw] , self.big_win_limit , self.grand_base, self.show_luck_playersmax,self.luck_count_num,
...
function toradora_table:player_stand_up(player, is_offline)
    ...
    self:log_msg(string.format(" player_stand_uptable id[%d] guid[%s]",self.id,tostring(player.guid)))
    if is_offline then
...
function toradora_table:check_player_num()
    self:log_msg(string.format("toradora_table:check_player_num androidNO [%d] #self.players[%d]  self.robot_threshold.playerless[%d]" , 
        getNum(self.android_list) , getNum(self.players) ,  self.robot_threshold.playerless))
    -- body
