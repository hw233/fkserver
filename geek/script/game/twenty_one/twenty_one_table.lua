...
local logic = require "game.twenty_one.logic"
...
local twenty_one_table = class("twenty_one_table",base_table)
...
function twenty_one_table:start(player_count)
    log.info("twenty_one_table:start: %d",player_count)
...
function twenty_one_table:player_sit_down(player, chair_id_)
    log.warning("twenty_one_table:player_sit_down [%d] [%d]",player.guid,chair_id_)
...
function twenty_one_table:player_stand_up(player, is_offline)
	...
	if can then
		player:reset()
		log.warning("twenty_one_table:player_stand_up normally [%d]",player.guid)
...
function twenty_one_table:ready(player)
    log.info("twenty_one_table:ready [%d]",player.guid)
...
function twenty_one_table:next_player_and_opers(current_operator)
	...
	local cur = nil
	local opers = nil
	table.logic_or(self.player_list_,function(p,k) 
...
function twenty_one_table:on_operation(player,msg)
	...
	if player.standby and not player.sub_player then
		log.warning("twenty_one_table:on_operation standby player guid[%d]",player.guid)
		return 
	end

	if msg.operation == Operation.BuySecurity then
		if self.status ~= TableStatus.WaitBuySecurity then
			log.warning("twenty_one_table:on_operation buy security when table is not on WaitBuySecurity [%d]",player.guid)
			return
		end
		...
		if self.status ~= TableStatus.WaitOperate then
			log.warning("twenty_one_table:on_operation operate when table is not on WaitOperate [%d]",player.guid)
			return
		end

		if not self.cur_operator  then
			log.warning("twenty_one_table:on_operation cur_operator got nil,player [%d]",player.guid)
			return 
		end

		if player ~= self.cur_operator and player.sub_player ~= self.cur_operator then 
			log.warning("twenty_one_table:on_operation cur_oper ~= player,cur[%d] player[%d]",self.cur_operator.guid,player.guid)
			return 
		end
		...
		if not table.logic_or(self.allow_operations,function(v,k) return v == msg.operation end) then
			log.warning("twenty_one_table:on_operation not check right  [%d]",player.guid)
			return
		end
...
function twenty_one_table:on_buy_security(player,is_cancel)
	...
	then
		...
		if table.logic_or(self.player_list_,function(p) return p and p.bet_amount > 0 and p.security and p.security > 0 end) then
...
function twenty_one_table:on_bet(player,msg)
	if self.status ~= TableStatus.WaitBet then
		log.warning("twenty_one_table:on_bet self.status ~= TableStatus.WaitBet guid[%d]",player.guid)
		return 
	end

	if player.standby then
		log.warning("twenty_one_table:on_bet standby player guid[%d]",player.guid)
		return 
	end

	if not table.logic_or(bet_money_units,function(cell) return cell == msg.amount end) then
		log.warning("twenty_one_table:on_bet bet amount not in bet_money_units guid[%d] [%d]",player.guid,msg.amount)
		return
	end

	log.info("twenty_one_table:on_bet guid[%d] [%d]",player.guid,player.bet_amount + msg.amount)
	...
	if table.logic_and(self.player_list_,function(p) return not p or p.standby or p.bet_amount > 0 end) then
		...
		timer_manager:new_timer((deal_cards_player_counts + 1) * deal_one_card_time * 2,function()
			if	table.logic_or(self.player_list_,function(p)  return p and p.bet_amount > 0 and self:can_buy_security(p) end) then 
...
function twenty_one_table:reconnect(player)
    log.info("twenty_one_table:reconnect %d",player.guid)
	player.trusteeship = false
...
function twenty_one_table:real_start_game()
	log.info("twenty_one_table:real_start_game [%s]",self:get_now_game_id())
...
function twenty_one_table:balance_player(player)
	...
	log.warning("balance player [%d] [%d] [%d]",player.guid,player.chair_id,player.balance_money)
...
function twenty_one_table:balance()
	...
	self:foreach(function(p)
		...
		log.warning("player change money [%d] [%d] [%d] [%d]",p.guid,p.chair_id,l_total_balance or 0,l_tax or 0)
...
function twenty_one_table:begin_operations(player,opers)
	...
	log.warning("twenty_one_table:begin_operations [%d] %s",player.guid,to_string(opers))
...
--		end
        ...
        log.info("deal_cards,guid:%d,cards:%s",p.guid,to_string(p_cards))
        p.cards = p_cards
...
function twenty_one_table:game_over()
	self:foreach(function(p)
		...
		if p.trusteeship then 
			log.warning("offline kick out,guid:[%d]",p.guid)
			p:forced_exit() 
		end
	end)
